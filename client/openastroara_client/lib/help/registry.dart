...
const Map<String, Help> helpRegistry = {
  ...
  'eq.auto_connect_on_boot': Help(
    ...
    body:
        ...
        '* Filter wheel — the first time the wheel connects after Ara starts up on the rig (auto or manual) Ara itself parks it on slot 1, conventionally L, unless a filter change is requested first or "Park on slot 1 when the wheel first connects" is off; a later reconnect leaves the wheel where it is. Drivers may also reposition on connect (driver-dependent).\n'
  ...
  'eq.filterwheel.home_on_first_connect': Help(
    ...
    title: 'Park on slot 1 when the wheel first connects',
    body:
        'The first time your filter wheel connects after Ara starts up on the rig — whether it auto-connected on boot or you connected it by hand — Ara parks it on slot 1, conventionally your luminance filter, so a session always starts from a known filter.\n\n'
