...
final rotatorApiFactoryProvider =
  ...
  (ref) => (server) => EquipmentDeviceApi<RotatorStatus>(
        server,
        path: 'rotator',
...
final rotatorApiProvider =
    Provider<EquipmentDeviceClient<RotatorStatus>?>((ref) {
...
class RotatorNotifier extends EquipmentDeviceNotifier<RotatorStatus> {
  ...
  Future<bool> move(double targetAngleDeg, {bool useSkyAngle = false}) =>
      performAction((api) => api.command('move', {
  ...
  Future<bool> sync(double skyAngleDeg) =>
      performAction((api) => api.command('sync', {'sky_angle_deg': skyAngleDeg}));
}

final rotatorProvider =
    AsyncNotifierProvider<RotatorNotifier, RotatorStatus?>(RotatorNotifier.new);
