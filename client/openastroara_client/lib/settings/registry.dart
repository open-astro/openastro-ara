...
const List<Setting> settingsRegistry = [
  ...
  Setting(
    id: 'timesync.client_gps.enabled',
    label: 'GPS on this computer',
    description: 'Read a USB GPS dongle plugged into the computer running the app and send its time and position to the rig. Frees a USB port on the Pi; the receiver fix also feeds Fill from GPS.',
    keywords: ['gps', 'dongle', 'usb', 'serial', 'time', 'clock', 'location', 'site', 'latitude', 'longitude'],
    path: ['Settings', 'Safety', 'Site', 'Time sync'],
    type: SettingType.bool(),
    defaultValue: false,
    relatedSettings: ['timesync.client_gps.port'],
  ),
  Setting(
    id: 'timesync.client_gps.port',
    label: 'GPS serial port',
    description: 'The serial port the USB GPS dongle appears on (for example /dev/cu.usbserial-1420 on a Mac, COM3 on Windows, /dev/ttyUSB0 on Linux).',
    keywords: ['gps', 'serial', 'port', 'com', 'tty', 'usb'],
    path: ['Settings', 'Safety', 'Site', 'Time sync'],
    type: SettingType.string(),
    defaultValue: '',
    relatedSettings: ['timesync.client_gps.enabled'],
  ),
  Setting(
  ...
  Setting(
    id: 'safety.policies.weather_triggers',
    label: 'React to weather-station thresholds',
    description: 'With an ObservingConditions weather station connected, a breached wind / humidity / dew-delta threshold makes conditions unsafe and runs your on-unsafe action — same reaction path as the safety monitor. Default off so an upgraded rig never surprise-parks.',
    keywords: ['weather', 'thresholds', 'wind', 'humidity', 'dew', 'station', 'observing conditions', 'unsafe'],
    path: ['Settings', 'Unattended running', 'When to stop'],
    type: SettingType.bool(),
    defaultValue: false,
    profilePath: 'safety.weather_triggers_enabled',
  ),
  Setting(
    id: 'safety.policies.max_wind_kmh',
    label: 'Max wind (km/h)',
    description: 'Wind above this — sustained speed or gust, whichever is worse — is a breach. Typical guided setups misbehave well before 40 km/h.',
    keywords: ['wind', 'gust', 'speed', 'kmh', 'threshold', 'weather'],
    path: ['Settings', 'Unattended running', 'When to stop'],
    type: SettingType.intRange(min: 1, max: 200),
    defaultValue: 36,
    profilePath: 'safety.max_wind_kmh',
  ),
  Setting(
    id: 'safety.policies.max_humidity_pct',
    label: 'Max humidity (%)',
    description: 'Relative humidity above this is a breach — condensation territory for optics and electronics.',
    keywords: ['humidity', 'moisture', 'percent', 'threshold', 'weather', 'condensation'],
    path: ['Settings', 'Unattended running', 'When to stop'],
    type: SettingType.intRange(min: 1, max: 100),
    defaultValue: 85,
    profilePath: 'safety.max_humidity_pct',
  ),
  Setting(
    id: 'safety.policies.min_dew_delta_c',
    label: 'Min dew delta (°C)',
    description: 'When ambient temperature drops to within this many degrees of the dew point, optics are about to fog — breach. Needs a station that reports both temperature and dew point.',
    keywords: ['dew', 'dew point', 'delta', 'fog', 'condensation', 'threshold', 'weather'],
    path: ['Settings', 'Unattended running', 'When to stop'],
    type: SettingType.doubleRange(min: 0, max: 20, step: 0.5),
    defaultValue: 2.0,
    profilePath: 'safety.min_dew_delta_c',
  ),
  Setting(
  ...
  Setting(
    ...
    type: SettingType.bool(),
    defaultValue: true,
    profilePath: 'safety.unattended_shutdown_enabled',
    relatedSettings: ['safety.policies.unattended_shutdown_wait_minutes', 'safety.policies.unattended_escalation'],
  ),
  Setting(
    id: 'safety.policies.unattended_shutdown_wait_minutes',
    label: 'Unattended shutdown wait (min)',
    description: 'Minutes of unattended silence after a paused-awaiting-you failure before the graceful shutdown executes. 10 is a good default — long enough to grab a phone and acknowledge, short enough not to drain a field battery; 5 suits battery rigs, 30 wall-powered observatories.',
    keywords: ['unattended', 'shutdown', 'wait', 'minutes', 'countdown', 'timeout', 'battery', 'observatory'],
    path: ['Settings', 'Unattended running', 'When to stop'],
    type: SettingType.intRange(min: 1, max: 120),
    defaultValue: 10,
    profilePath: 'safety.unattended_shutdown_wait_minutes',
    relatedSettings: ['safety.policies.unattended_shutdown_enabled'],
  ),
  Setting(
  ...
  Setting(
    ...
    path: ['Settings', 'Unattended running', 'Extra logging'],
  ...
  Setting(
    ...
    defaultValue: 30000,
    profilePath: 'safety.flat_target_adu',
  ),
  Setting(
    id: 'session.calibration.flat_target_adu_tolerance_pct',
    label: 'Flat target tolerance (%)',
    description: 'How close the probed mean ADU must land to the target before the flat set captures. Tighter costs more probe exposures; looser risks less-consistent flats.',
    keywords: ['flats', 'tolerance', 'adu', 'target', 'percent'],
    path: ['Settings', 'Your night', 'Calibration'],
    type: SettingType.doubleRange(min: 0.1, max: 50),
    defaultValue: 5.0,
    profilePath: 'safety.flat_target_adu_tolerance_pct',
  ),
  Setting(
    id: 'session.calibration.flat_frames_per_filter',
    label: 'Flat frames per filter',
    description: 'Saved FLAT frames each generated per-filter set captures once its exposure converges. 20-30 stacks to a clean master flat on most cameras.',
    keywords: ['flats', 'frames', 'count', 'filter', 'per filter'],
    path: ['Settings', 'Your night', 'Calibration'],
    type: SettingType.intRange(min: 1, max: 500),
    defaultValue: 30,
    profilePath: 'safety.flat_frames_per_filter',
  ),
  Setting(
    id: 'session.calibration.post_flat_park_mount',
    label: 'Park the mount after flats',
    description: 'Append a park step to the generated flats sequence so the rig ends the night parked once the last flat set completes.',
    keywords: ['flats', 'park', 'mount', 'end of night'],
    path: ['Settings', 'Your night', 'Calibration'],
    type: SettingType.bool(),
    defaultValue: true,
    profilePath: 'safety.post_flat_park_mount',
  ),
  Setting(
    ...
    type: SettingType.intRange(min: 1, max: 65535),
    defaultValue: 25000,
    profilePath: 'safety.sky_flat_target_adu',
  ),
  Setting(
    ...
    type: SettingType.intRange(min: 1, max: 500),
    defaultValue: 20,
    profilePath: 'safety.sky_flat_frames_per_filter',
  ),
  Setting(
    id: 'session.calibration.sky_flat_target_azimuth',
    label: 'Sky flat patch azimuth (deg)',
    description: 'Compass bearing of the blank sky patch the mount slews to for twilight flats. 90 points east; the anti-solar sky opposite the sunset or sunrise gives the most even illumination.',
    keywords: ['sky flats', 'twilight', 'azimuth', 'pointing', 'slew', 'patch', 'compass'],
    path: ['Settings', 'Your night', 'Calibration'],
    type: SettingType.doubleRange(min: 0, max: 360),
    defaultValue: 90.0,
    profilePath: 'safety.sky_flat_target_azimuth',
  ),
  Setting(
    id: 'session.calibration.sky_flat_target_altitude',
    label: 'Sky flat patch altitude (deg)',
    description: 'Height above the horizon of the sky patch for twilight flats. 75 degrees keeps the mount well clear of horizon gradients and obstructions while staying off the zenith.',
    keywords: ['sky flats', 'twilight', 'altitude', 'elevation', 'pointing', 'slew', 'patch'],
    path: ['Settings', 'Your night', 'Calibration'],
    type: SettingType.doubleRange(min: 0, max: 90),
    defaultValue: 75.0,
    profilePath: 'safety.sky_flat_target_altitude',
  ),
  Setting(
    ...
    profilePath: 'safety.sky_flat_stop_at_max_adu',
  ),
  Setting(
    ...
    defaultValue: 5000.0,
    profilePath: 'safety.sky_flat_stop_at_min_adu',
  ),
  Setting(
    id: 'session.calibration.sky_flat_sun_altitude',
    label: 'Sky flat wait-for sun altitude (deg)',
    description: 'The sun altitude the generated sky-flat sequence waits for before it starts. Around -9 degrees (nautical twilight) the sky is dim enough for stars yet bright enough to reach the target within the exposure bounds. Negative is below the horizon.',
    keywords: ['sky flats', 'twilight', 'sun', 'altitude', 'nautical', 'dusk', 'dawn', 'wait'],
    path: ['Settings', 'Your night', 'Calibration'],
    type: SettingType.doubleRange(min: -90, max: 90),
    defaultValue: -9.0,
    profilePath: 'safety.sky_flat_sun_altitude',
  ),
  Setting(
  ...
  Setting(
    id: 'session.storage.backup_stream_folder',
    label: 'Backup folder',
    description: 'Where streamed frames are written on this desktop, organized as <server>/<session>/<frame>.fits. Defaults to Documents/OpenAstroAra/Backups.',
    keywords: ['backup', 'folder', 'path', 'directory', 'stream', 'destination', 'local'],
    path: ['Settings', 'Your night', 'Storage'],
    type: SettingType.string(),
    defaultValue: '',
  ),
  ...
  Setting(
    ...
    path: ['Settings', 'Location & sky', 'Where you observe'],
    type: SettingType.string(),
  ...
  Setting(
    ...
    path: ['Settings', 'Your night', 'Filenames'],
  ...
  Setting(
    ...
    description: 'Connect to the filter wheel when Ara starts. On its first connect after Ara starts, Ara parks the wheel on slot 0 unless "Park on slot 0 on first connect" is off.',
  ...
  // §63.17 guider equipment selection — device picks pushed to the guider
  // daemon inside its disconnected window. Values are the daemon's own choice
  // strings (GET /equipment/guider/choices); "" / 0 = unset (the daemon keeps
  // its own selection).
  Setting(
    ...
    type: SettingType.string(),
    defaultValue: '',
    profilePath: 'phd2.guider_camera',
  ),
  Setting(
    ...
    type: SettingType.string(),
    defaultValue: '',
    profilePath: 'phd2.guider_camera_id',
  ),
  Setting(
  ...
  Setting(
    ...
    type: SettingType.string(),
    defaultValue: '',
    profilePath: 'phd2.guider_aux_mount',
  ),
  Setting(
    ...
    type: SettingType.string(),
    defaultValue: '',
    profilePath: 'phd2.guider_rotator',
  ),
  Setting(
    ...
    type: SettingType.string(),
    defaultValue: '',
    profilePath: 'phd2.guider_alpaca_host',
  ),
  Setting(
    ...
    defaultValue: 0,
    profilePath: 'phd2.guider_alpaca_port',
  ),
];
