...
namespace OpenAstroAra.Test {
    ...
    public class ImageDataTest {
        ...
        [Test]
        [TestCase(OpenAstroAra.Core.Enum.FileTypeEnum.XISF, ".xisf")]
        [TestCase(OpenAstroAra.Core.Enum.FileTypeEnum.FITS, ".fits")]
        [TestCase(OpenAstroAra.Core.Enum.FileTypeEnum.TIFF, ".tif")]
        public async Task SaveToDiskSimpleTest(OpenAstroAra.Core.Enum.FileTypeEnum fileType, string extension) {
        ...
        [Test]
        [TestCase(OpenAstroAra.Core.Enum.FileTypeEnum.XISF)]
        [TestCase(OpenAstroAra.Core.Enum.FileTypeEnum.FITS)]
        [TestCase(OpenAstroAra.Core.Enum.FileTypeEnum.TIFF)]
        public async Task SaveToDiskForceExtensionTest(OpenAstroAra.Core.Enum.FileTypeEnum fileType) {
            dataFactoryUtility.ProfileServiceMock.Setup(x => x.ActiveProfile.ImageFileSettings.FITSAddFzExtension).Returns(false);
            var data = new ushort[] {
                3,1,1,
                3,4,5,
        ...
        public async Task SaveToDiskPatternMetaDataTest() {
            ...
            var fileSaveInfo = new FileSaveInfo {
                FilePath = TestContext.CurrentContext.TestDirectory,
                FilePattern = pattern,
                FileType = OpenAstroAra.Core.Enum.FileTypeEnum.XISF
            };
            ...
            var expectedPattern = $"{MetaData.FilterWheel.Filter}" +
                ...
                $"#{OpenAstroAra.Core.Utility.CoreUtil.ApplicationStartDate.ToString("yyyy-MM-dd")}";
        ...
        public async Task SaveToDiskPatternEmptyMetaDataTest() {
            ...
            var fileSaveInfo = new FileSaveInfo {
                FilePath = TestContext.CurrentContext.TestDirectory,
                FilePattern = pattern,
                FileType = OpenAstroAra.Core.Enum.FileTypeEnum.TIFF,
                TIFFCompressionType = OpenAstroAra.Core.Enum.TIFFCompressionTypeEnum.LZW
            };
            ...
            var expectedPattern = $"#{date.ToLocalTime():yyyy-MM-dd}#{date.AddHours(-12).ToLocalTime():yyyy-MM-dd}#{date.ToLocalTime():yyyy-MM-dd_HH-mm-ss}#{date.ToLocalTime():HH-mm-ss}#-0001##1x1#########{OpenAstroAra.Core.Utility.CoreUtil.ApplicationStartDate.ToString("yyyy-MM-dd")}.tif";
        ...
        //    var fileType = OpenAstroAra.Core.Enum.FileTypeEnum.XISF;
        //    var data = new ushort[] {
        //        3,1,1,
        //        3,4,5,
        //        3,2,3
