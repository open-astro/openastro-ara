...
- **Last merged:** PR **#807** (§36 framing position angle carried into runs — a dialed framing `rotationDeg` emits a `CenterAndRotate` in place of the blind slew, untouched 0° keeps the slew; 2 review rounds, r1 one stale-doc nit fixed; merged @ `68517b89c`. **ALL ROADMAP part-3 sequencer-instruction items closed.**) Before it: **#806** (§38 rotation fidelity @ `26ef90c54`), **the complete §42 epic #795–#805** (ROADMAP part 1 done).
- **Currently working on:** **§64/§59 StarDetector cap early-out** (branch `stardetector-cap-earlyout`, closes the last #775 PORT_TODO follow-up) — with `MaxNumberOfStars` set, `FindStars` collects candidate blobs + their fill-phase peak (the same `MaxBrightness` key the old path sorted on) and lazily `Measure`s brightest-first until the cap fills; rejected candidates pass their slot on; uncapped path measures inline as before (no candidate accumulation); `Measure`/`DonutGeometry` take `ReadOnlySpan<int>`. Selection equivalence pinned by test (capped == uncapped-then-trimmed). (Before it: **#809** §64 detect offload @ `bdd7506d9`, 1 round ✅; #808 OSC annotation; #807 §36 PA; #806 §38; the §42 epic #795–#805 — the #775 §64 follow-up trio is now fully closed.) Next: ROADMAP part 5 remainder (repo-frame annotation seam, OSC RenderBitmapSource display — both Live-View-gated/low-value; libraw is a native-dep slice) or the §59 Smart Focus remainder — re-read ROADMAP + PORT_TODO to pick
