...
- **Last merged:** PR #322 (§14e PR2 — daemon discovery integration test) on `master` — `AlpacaDiscoveryIntegrationTest` proves the daemon's UDP-broadcast discovery (port 32227) finds a live OmniSim's Camera + Telescope on a hosted runner (ran 2/2, not skipped). Builds on the #321 simulator harness. The §38 sequence-engine thread (#313–#320) remains functionally complete.
- **Currently working on:** **§14e PR3 — first real Alpaca-backed equipment service.** Replaces `PlaceholderSafetyMonitorService` with a real `SafetyMonitorService` that connects to a discovered Alpaca SafetyMonitor (via `ASCOM.Alpaca.Clients.AlpacaSafetyMonitor`) and exposes live `State` + `IsSafe` over the §6 REST surface, following the §60.5 202-Accepted lifecycle (connect on a background task; `GetAsync` reports `Connecting`→`Connected`/`Error`). Covered by sim-free unit tests (`SafetyMonitorServiceTest`: state machine + dead-port→Error, runs in the normal suite) **and** a live `[Category("Integration")]` test (`SafetyMonitorConnectIntegrationTest`: discover→connect→read→disconnect against the OmniSim). Next: unify this service with the Sequencer's `ISafetyMonitorMediator` (so `WaitUntilSafe` sees the live device), then roll the same pattern across the other device types.
