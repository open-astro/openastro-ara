...
- **Last merged:** PR **#344** (§14e capture-path PRb — `TakeExposure` re-port + camera/imaging mediator unification) on `master` @ `5ace6c5b4` — **15 review rounds.** `TakeExposure` re-ported with the NINA-verbatim JSON surface (resolves via §38k-6 remap); `CameraService` now also serves `ICameraMediator` + `IImagingMediator`, routing sequencer captures through the SAME `CaptureCoreAsync` pipeline as REST (in-flight gate: sequencer waits / REST rejects; cancellation checkpoints at entry/pre-exposure/pre-download + `TryAbortQuietly`; client re-snapshot under gate after a long queue wait; `ExposureCount` Interlocked+Volatile; `Validate` rejects non-positive exposure + unrecognized `ImageType`; `IMAGETYP` uppercased; `CameraOffset` cap-validated via new `MinOffset/MaxOffset` caps). **A saved sequence with capture nodes now produces real FITS frames in the catalog — the camera capture path (PRa #343 + PRb #344) is complete.** Deferred to PORT_TODO: shared "manual capture" session for sequencer frames, REST LIGHT-only, FITS keyword-convention audit, `ExposureSeconds` int / `Gain` sentinel widening. Follows #321–#343.
- **Currently working on:** **§63 guider/PHD2 — guider-a (real `GuiderService`), scoped + started, re-port deferred to a focused PR.** Discovery: the inherited `PHD2Guider.cs` (the `IGuider` impl that speaks the PHD2 JSON-RPC protocol) was **deleted in #242** (the net10.0 conversion) and is NOT in the tree — only the protocol primitives survive (`PHD2Methods`, `PhdEvents/*`, `PhdAppState`, `Phd2Profile`). Recovered the 1261-line class from `840893eb8^`. **WIP save-point pushed on branch `phase/14e-guider-a`** (does NOT compile yet — no PR). Mechanical re-port categories CLEARED there: WPF strip (`System.Windows.Media/Threading`, `IWindowServiceFactory`+`SetupDialog`+`OpenPHD2FileDialog`, `[RelayCommand]`/`CommunityToolkit.Mvvm`, the `ImageSource Image` UI property); `Notification`→`Notifier` (the class was *renamed* in the Phase 0.5p scrub, not removed — methods preserved); nullable annotations on the 11 connect-time fields/events (CS8618 cleared); settings `PHD2ServerUrl`→`PHD2ServerHost` (§63/12h.4 reshape — host string, clean rename); `IReadOnlyList.Length`→`.Count`. **REMAINING on that branch (~44 sites):** pure nullable-correctness (CS8602/8603/8604/8625/8601/8600/8629/8605) null-guards/`!`/`?.` on the connection + protocol path, + 2 `int?` conversions (L414 `durationMs`, L1057). Each needs per-site judgment (a wrong annotation = a runtime NRE only catchable with live PHD2). **Then `GuiderService`** (REST connect/status, §63.2 state machine, §32.4 cache, RMS from the `GuideEvent` stream → `GuiderStateDto`) replacing `PlaceholderGuiderService`. Resume on `phase/14e-guider-a`: finish the nullable pass → it compiles → add GuiderService + tests → open the PR.
- **Next substantive work (v0.0.1 queue, non-user-blocked):** **guider** (a: PHD2Guider re-port + `GuiderService` connect/status; b: StartGuiding/StopGuiding/Dither; c: `IGuiderMediator` unification replacing `HeadlessGuiderMediator`; d: §63.3 crash-detect + systemd-restart; e: §63.4/.5 profile + dark-library push — possibly the v0.0.1/v0.1.0 boundary) → **polar-align** (§45, gates on camera+plate-solve) → **Live View / §2105 image render** (OpenCvSharp4 + libraw un-stub in `Image/ImageData/` — Live-View-gated, NOT a capture-path blocker; previews already come from the §65 SkiaSharp stretch pipeline) → **`IXxxMediator → IXxxService` rename** (§8.1, cosmetic) → **Phase 15** (TODO sweep, `3rd-party-licenses.txt`). **User-blocked terminus:** the `v0.0.1-ara.1` tag on master is user-driven, and the RPi smoke test needs physical Pi hardware — so the final release step is NOT autonomously completable. **Phase 14e Alpaca simulator pinning** (§14.5.1 v0.4.0) already landed (#321).
