...
- **Last merged:** PR #325 (§14e PR5 — §32.4 cache for the SafetyMonitor read) on `master` — a background `Timer` refreshes the cached `IsSafe` every 2s while Connected; `GetAsync`/`GetInfo` serve it under a single lock with no per-poll blocking HTTP read. The SafetyMonitor is now **fully real, cached, and a hardened template**. Follows #321–#324. The §38 sequence-engine thread (#313–#320) remains functionally complete.
- **Currently working on:** **§14e PR6 — first replication of the real-service pattern: ObservingConditions.** `ObservingConditionsService` (real `IObservingConditionsService`) connects to a discovered Alpaca ObservingConditions device and serves live weather sensors (temperature/humidity/pressure/cloud/wind/rain) over the §6 REST surface, §32.4-cached (background timer reads all sensors every 2s; each sensor independently nullable since a device only implements the sensors it has). Read-only — REST-only, no mediator unification (`IWeatherDataMediator` isn't consumed by any sequence instruction). Mirrors the SafetyMonitor template (generation supersede, single-flight refresh, dispose timer). Covered by sim-free unit tests + a live `[Category("Integration")]` test (discover→connect→read temperature→disconnect). Next: roll the same pattern across the remaining read/control device types (Switch, FlatDevice, Focuser, Rotator, FilterWheel, Dome, Telescope, Camera, Guider).
