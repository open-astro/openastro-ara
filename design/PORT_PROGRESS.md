...
- **Last merged:** PR #330 (§14e PR10 — FilterWheel device, change slot) on `master` — slots + live current-slot over REST, slot-validated change. Follows #321–#329. The §38 sequence-engine thread (#313–#320) remains functionally complete.
- **Currently working on:** **§14e PR11 — FlatDevice / CoverCalibrator (cover + light).** `FlatDeviceService` (real `IFlatDeviceService`) connects to a discovered Alpaca CoverCalibrator and serves live runtime (cover state / light-on / brightness, §32.4-cached; `CoverState`/`CalibratorState` enums mapped to the DTO's `cover_open|cover_moving|cover_closed|light_on|error` summary) over REST, and drives the cover + calibrator light via `ApplyFlatPanelAsync` (202-Accepted background op — OpenCover/CloseCover + CalibratorOn/Off per the optional OpenCover/LightOn/Brightness fields; brightness validated via extracted+unit-tested `IsBrightnessOutOfRange` vs the read-once MaxBrightness). Mirrors the hardened control template. REST-only — mediator unification is a follow-up. Covered by 10 sim-free unit tests + a live `[Category("Integration")]` test (discover→connect→open cover→light on→verify→disconnect). Next: Dome/Telescope/Camera/Guider.
