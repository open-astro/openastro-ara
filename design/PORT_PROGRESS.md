...
- **Currently working on:** **§63 guider-c — `IGuiderMediator` unification** (next sub-PR). Make `GuiderService` (or a `.Mediator.cs` partial) also serve the Sequencer's `IGuiderMediator`, registered as one singleton for both interfaces (per §8.1), replacing `HeadlessGuiderMediator` so the sequencer's `StartGuiding`/`StopGuiding`/`Dither` instructions drive the **live** PHD2 guider instead of no-op stubs — mirrors the camera/telescope/etc. mediator-unification pattern. (guider-a folded in guider-b's RMS, so the queue skips straight to guider-c.)
