...
- **Last merged:** PR **#803** (§42.2 audit items 2+5 — persistent-op-fault escalation: `OpFaultEscalator` (3 definite op faults / 10-min window per device) feeds `FaultPolicyMatrix.Resolve(..., persistentOpFault:)`: Telescope → `AbortAndPark`, Camera → `PauseSequence`, peripherals notify-only; `ExecuteEscalationAsync` runs the terminal immediately (`escalated:*` stamps + WS + Error notification); camera capture path now publishes `StallTimeout`/`OpError`; 1 review round ✅ zero findings; merged @ `1287fb344`. **Every buildable §42.2 audit item done** — 7 is §59.7 territory, 8 an L advisory, 9 N/A by design.) Before it: **#802** (item 4 @ `8776aae56`), **#801** (items 3+6), **#800** (item 1), **#799**/#798/#797/#796/#795 (§42.5 + WILMA surfaces arc).
- **Currently working on:** **§42.6 `affected_frames` population** (branch `affected-frames-426`) — `SqliteFaultLogService` fills `affected_frames` the moment `resolved_at` is stamped (recovered-action update + insert paths, and `ResolveOnReconnectAsync`): the fault's session's frames whose exposure window `[captured_utc, captured_utc+exposure_seconds]` overlapped `[detected_at, resolved_at]`; best-effort, session-scoped, empty for out-of-run and never-resolved rows by design. **This is the LAST §42 epic item — merging it completes ROADMAP part 1.** (Before it: #804 closed §42.3 with the linger re-adopt + the 5-mediator disconnect-mid-wait harness @ `ad4311d87`.) Next after the epic: ROADMAP part 2 gates are user/hardware-blocked (apt pipeline needs infra decisions, RPi smoke test needs the Pi), so likely ROADMAP part 3 (§59 Smart Focus remainder) or part 5 (libraw DSLR RAW) — decide by re-reading ROADMAP + PORT_TODO next iteration
