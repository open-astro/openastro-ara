...
- **Currently working on:** **§29 low-disk-space warning** (branch `phase/29-disk-space-monitor`) — `DiskSpaceMonitor` (a warn-only `BackgroundService`) raises a §51 diagnostic + the §54 `OnDiskSpaceLow` notification when the image save volume runs low (Yellow <10 GiB / Red <2 GiB), opening one issue on a downward transition and clearing it on recovery via the new `IDiagnosticsService.ClearOpenEventsByTypeAsync`. Never blocks a capture or deletes. +pure-logic & clear-event tests. Recent completions: **guider-e-3 §63.4 profile-name mapping** (e-3a #375 + e-3b #376), **§39 dark-matching-by-temperature** (#377). Open threads (all needing user scope-calls or hardware): **§29 configurable thresholds / hard-stop policy** (PORT_TODO), **§58.9 four-layer unattended flip safety**, **guider-e-3c** (slug-collision disambiguation) **+ e-4** (dark-library), **§45 polar-align** (guider-daemon RPCs absent), hardware-gated live AF sweep / flip + centering validation.
