...
- **Last merged:** PR #327 (§14e PR7 — Switch device, first control action) on `master` — live ports over REST + a `SetValue` write. Follows #321–#326. The §38 sequence-engine thread (#313–#320) remains functionally complete.
- **Currently working on:** **§14e PR8 — Focuser device (Move).** `FocuserService` (real `IFocuserService`) connects to a discovered Alpaca Focuser, serves its static `Capabilities` (min/max position, step size, temp-comp, absolute — read once on connect) + live `Runtime` (state/position/temperature/temp-comp, §32.4-cached) over REST, and moves it via `MoveAsync` (a §60.5 202-Accepted background op — validates range vs capabilities, runs `Move` off-thread, refreshes the cache). Mirrors the template (generation supersede, single-flight refresh, dispose timer, cache reset on adopt). REST-only — the `IFocuserMediator` unification (MoveFocuser/autofocus instructions) is the follow-up. Covered by sim-free unit tests (incl. Move-when-not-connected→throws, dispose contracts) + a live `[Category("Integration")]` test (discover→connect→read caps→move→verify position→disconnect). Next: Rotator/FilterWheel/FlatDevice/Dome/Telescope/Camera/Guider.
