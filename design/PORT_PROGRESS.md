...
- **Last merged:** PR **#799** (§42.5 resolve-on-reconnect — `EquipmentEventPublisher` connected transition stamps `resolved_at` on the device type's unresolved `disconnected` rows via `IFaultLogService.ResolveOnReconnectAsync`; merged @ `64ab86c24`). Before it the §42 WILMA-surfaces arc: **#798** (fault feed panel + §42.6 session timeline @ `2a659188a`), **#797** (live fault chips + history seeding @ `5d0fe2ef1`), **#796** (has_more fix @ `69dae8032`), **#795** (§42.5 fault log + REST, **closes §42.5**, @ `707370300`).
- **Currently working on:** **§42.2 op-failure enforcement** (branch `op-failures-422`; the full matrix audit is recorded in PORT_TODO "§42.2 fault-matrix enforcement audit") — the six peripheral op mediators (mount slew/park, focuser move, filter change, rotator move, switch write, dome ops) published §42.4 faults but returned benign values the instructions discarded, so `Attempts` retries, `sequence.instruction_failed`, and `ErrorBehavior` never engaged (a jammed EFW read as success). Each definite op failure (stalled call, device error, dispatched-but-never-settled) now throws `SequenceEntityFailedException` after publishing; cancellation carve-outs and not-connected pre-checks untouched; the CenteringSolver finally-restore is guarded best-effort. Then the audit's items 2-6 (mount abort+park terminal, switch/rotator re-command, pause-on-persistent-op-fault, ASTAP exit code) → §42.3 generalized hot-reconnect → §42.6 `affected_frames`
