...
- **Last merged:** PR **#795** (§42.5 fault log + REST — `faults` table written at the `EquipmentFaultHub` choke point, §42.3 reaction outcomes stamped onto the same row via natural-key upsert (`recovered` resolves), run-session attribution via `ActiveRunSessionRegistry` (sole-active-run only — null over plausible-but-wrong, r2 review finding), `GET /api/v1/faults` (+ `/{id}`); 2 review rounds, both ✅ Approved; merged @ `707370300`. **Closes §42.5.**)
- **Currently working on:** **has_more pagination fix** (branch `fix-hasmore-pagination`) — the pre-existing sentinel-consumed-before-count-check bug that made notifications/diagnostics/frames lists report `has_more=false` on every page (found by the #795 test work; PORT_TODO entry now ✅); then §42 WILMA surfaces → §42.2 remaining matrix rows → §42.3 generalized hot-reconnect
