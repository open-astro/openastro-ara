...
- **Last merged:** PR #337 (§14e PR17 — Telescope mediator unification) on `master` — `TelescopeService` also serves `ITelescopeMediator` (largest mediator surface; epoch-aware slews + bounded terminal-condition waits + retried `EquatorialSystem` capture; SOFA/NOVAS-native fallback documented). Fourth control-device mediator unification. Follows #321–#336.
- **Currently working on:** **§14e PR18 — Switch mediator unification.** `SwitchService` now also implements `ISwitchMediator` (new `SwitchService.Mediator.cs` partial; one singleton backs both, replacing `HeadlessSwitchMediator`). The port cache was widened from REST-DTO shape to an internal `SwitchPortSnapshot` (adds `Description` + `StepSize`, which only the mediator needs); `GetAsync` projects the DTO view so the REST surface is unchanged. `GetInfo()` populates `WritableSwitches`/`ReadonlySwitches` with cache-backed `IWritableSwitch`/`ISwitch` wrappers (`CachedWritableSwitch`/`CachedSwitch`): range/step are the snapshot's (static per ASCOM), `Value` reads the live §32.4 cache so stored references stay current, `Poll()` nudges a refresh, `SetValue()` is a best-effort write-through. `SetSwitchValue(switchIndex, value, …)` maps the instruction's **writable-collection index → ASCOM port id** (cache order; `WritablePortIdAt` helper) and drives the device through the bounded write launcher (1-min hard timeout — switch writes are prompt; sequencer cancellation propagates; other failures log as a failed write). So the `SetSwitchValue` instruction executes against real hardware with real min/max/step in Validate. Covered by 11 sim-free mediator unit tests (incl. fabricated-snapshot collection/index-mapping tests) + a live mediator GetInfo+SetSwitchValue added to the `[Category("Integration")]` test. Next: FilterWheel/FlatDevice mediators need `IProfileService` (profile source-of-truth milestone) — that wiring is the §14e PR19 arc.
