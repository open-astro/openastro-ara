...
- **Last merged:** PR #326 (§14e PR6 — real ObservingConditions service) on `master` — live weather sensors over REST, §32.4-cached, read-only. Second application of the SafetyMonitor template. Follows #321–#325. The §38 sequence-engine thread (#313–#320) remains functionally complete.
- **Currently working on:** **§14e PR7 — Switch device (first control action).** `SwitchService` (real `ISwitchService`) connects to a discovered Alpaca Switch, serves its live ports (id/name/value/min/max/can-write) over REST §32.4-cached, and **writes** a port value via `SetValueAsync` (the first device control action — off-thread `SetSwitchValue`, then a cache refresh so the new value shows promptly). Mirrors the template (generation supersede, single-flight refresh, per-port skip-on-failure, dispose timer). REST-only — the `ISwitchMediator` unification (the `SetSwitchValue` sequence instruction) is the follow-up. Covered by sim-free unit tests (incl. SetValue-when-not-connected→throws, dispose contracts) + a live `[Category("Integration")]` test (discover→connect→read ports→write a writable port→verify→disconnect). Next: roll on to FlatDevice/Focuser/Rotator/FilterWheel/Dome/Telescope/Camera/Guider.
