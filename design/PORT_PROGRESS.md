...
- **Last merged:** PR #333 (§14e PR13 — Telescope / mount) on `master` — read-once caps + §32.4-cached RA/Dec/tracking runtime over REST + slew/park/unpark (202) + set-tracking/abort-slew (prompt sync, §57 panic stop ignores caller token). Follows #321–#332. The §38 sequence-engine thread (#313–#320) remains functionally complete.
- **Currently working on:** **§14e PR14 — mediator unification thread (start: Focuser).** With the 9 real REST device services shipped (#323–#333), the next thread wires each control device's *mediator* to the same singleton so the Sequencer's instructions drive live hardware (the pattern SafetyMonitor #324 set; the remaining Camera/Guider/PolarAlign devices gate on the image pipeline / PHD2 / plate-solve and come later). PR14: `FocuserService` now also implements `IFocuserMediator` (new `FocuserService.Mediator.cs` partial; one singleton backs both, replacing `HeadlessFocuserMediator`) — `GetInfo()` serves the §32.4 cache (never throws post-Dispose) and `MoveFocuser`/`MoveFocuserRelative`/`MoveFocuserByTemperatureRelative` drive the live device with a bounded settle-wait, returning the final position, so `MoveFocuserAbsolute`/`Relative`/`ByTemperature` validate + execute against real hardware. REST surface unchanged. Covered by 6 sim-free mediator unit tests + a live mediator-move added to the `[Category("Integration")]` test. Next mediators: Switch/Rotator/Dome/Telescope (FilterWheel's SwitchFilter + Dither need `IProfileService`).
