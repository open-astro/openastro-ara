...
- **Last merged:** PR **#805** (§42.6 `affected_frames` population — filled at resolve time (recovered action + reconnect hook), session-scoped exposure-window overlap against `[detected_at, resolved_at]`; 1 review round ✅ no bugs; merged @ `9438a1821`). **THE §42 HARDWARE-FAULT-SURFACES EPIC IS COMPLETE — ROADMAP part 1 done (eleven PRs, #795–#805):** fault log + REST (#795), has_more fix (#796), WILMA fault surfaces (#797/#798), resolve-on-reconnect (#799), §42.2 op-failure enforcement + audit items (#800–#803), §42.3 linger re-adopt + mid-wait harness (#804), §42.6 affected_frames (#805).
- **Currently working on:** **§38 CenterAndRotate rotation fidelity** (branch `center-rotate-fidelity`, ROADMAP part 3) — `ICenteringExecutor.CenterAndRotateAsync`: NINA's rotate-first order (capture-solve → `rotatorMediator.Sync(solvedPA)` → `MoveRelative` by `FoldRotationDelta` (deltas folded into (-90°,+90°] — a 180°-rotated frame is the same framing) looping to the profile's `RotationTolerance`, then the §28 centre loop), implemented in `CenteringService.Executor` under the centering gate; the instruction routes there when a rotator is connected, keeps NINA's preserved-but-skipped path otherwise. Also corrected stale tracking: RunAutofocus was ALREADY fully wired (`AutofocusSweepService : IAutofocusExecutor` via the factory) — ROADMAP part 3 updated. Next: §36 framing PA carried into runs (the framing flow can now emit a `CenterAndRotate`), then part 5 imaging-pipeline items (OSC annotation/display) or the §59 Smart Focus remainder
