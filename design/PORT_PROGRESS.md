...
- **Last merged:** PR #338 (§14e PR18 — Switch mediator unification) on `master` — `SwitchService` also serves `ISwitchMediator` (`IWritableSwitch` wrappers with real min/max/step; writable-collection-index→port-id mapping; bounded write). Fifth control-device mediator unification. Follows #321–#337.
- **Currently working on:** **§14e PR19 — FilterWheel mediator unification + profile filter import.** `FilterWheelService` now also implements `IFilterWheelMediator` (new `FilterWheelService.Mediator.cs` partial; one singleton backs both, replacing `HeadlessFilterWheelMediator`), so the `SwitchFilter` instruction drives the live Alpaca wheel. The `IProfileService` dependency is solved with **NINA's import-on-connect semantics** rather than the full profile source-of-truth reconciliation: on the first slot read of each connection the service imports the wheel's filter list into `ActiveProfile.FilterWheelSettings.FilterWheelFilters` (`SyncProfileFilters`: device is truth for slot count + names of new positions; existing entries at still-valid positions are preserved verbatim since they carry user-edited autofocus settings; stale positions removed) — so `SwitchFilter.MatchFilter` resolves its `[JsonProperty] Filter` by name/position on deserialize. `GetInfo()` serves Connected/IsMoving + `SelectedFilter` (cached position resolved profile-first, then device slot names — `ResolveFilter`). `ChangeFilter` drives the wheel through the bounded launcher (1-min hard timeout on the write) and blocks until the device reports the target slot (ASCOM `Position == -1` while rotating; ~60s settle bound), returning the filter actually reached. REST surface unchanged (`FilterWheelService` ctor gains an optional `IProfileService`). Covered by 10 sim-free mediator unit tests (fabricated-slot import/resolution helpers + not-connected/disposed contracts) + a live mediator import+ChangeFilter added to the `[Category("Integration")]` test. The full profile source-of-truth reconciliation (REST-edited `profile.json` ↔ `ActiveProfile`) remains the execution-engine milestone; FlatDevice mediator is next after this.
