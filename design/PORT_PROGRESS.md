...
- **Last merged:** PR **#817** (§37.5 max-sequence-runtime cap @ `3191d636a` — `SiteSettingsDto.MaxSequenceRuntimeMin` + the sequencer's `WatchRuntimeCapAsync` watchdog (graceful user-stop past the cap, one Warning notification, cap re-read per 60 s tick) + client field/panel/registries/wizard; clean review). Before it: **#816** (§37 clear-field), **#815** (§63.8 poll visibility), **#814** (§50 tiles), **#813** (§30 active-server), **#812** (§36.8 4b), **#811** (§42.5 — §42 fully closed), #806–#810, the §42 epic #795–#805.
- **Currently working on:** **§29.9 log-tail reverse byte-scan** (branch `logs-reverse-tail`, ROADMAP part 6 "Logs + bug report streaming", first half) — `LogService.TailAsync` now reads each file BACKWARDS (chunked from a length snapshot; splits on the newline byte with byte-level carry across chunk boundaries — UTF-8-safe since 0x0A never appears in a continuation byte) and early-exits the moment the window fills, instead of scanning every byte forward through a ring. Internal `ReverseScanChunkSize` seam. +2 tests (7-byte chunks force multi-boundary splits incl. mid-multibyte-char; CRLF + torn final line); 38/38 LogService tests pass. Still open in the same ROADMAP bullet: §54 ZIP downloads buffer→stream (separate slice). Next: remaining §37.5 fields or that streaming half.
