...
- **Last merged:** PR **#819** (§54 download streaming @ `0777edee3` — daemon-log + bug-report downloads stream to a destination-first path via `dio.download` + `pickStreamSavePath`; with **#818** (§29.9 reverse-tail @ `6cfe95523`) the "Logs + bug report streaming" ROADMAP bullet is CLOSED). Before them: **#817** (§37.5 max-runtime cap), **#816** (§37 clear-field), **#815** (§63.8 poll visibility), **#814** (§50 tiles), **#813** (§30 active-server), **#812** (§36.8 4b), **#811** (§42.5 — §42 fully closed), #806–#810, the §42 epic #795–#805.
- **Currently working on:** **§37.5 soft-warning altitude** (branch `soft-warning-altitude`, ROADMAP part 6, enforcement-first) — `SiteSettingsDto.SoftWarningAltitudeDeg` (double, ctor default 30; 0 disables) + its consumer: Tonight's Sky appends the zero-point advisory reason "stays below your N° soft-altitude mark (+0)" when an object's peak altitude tonight never clears the mark (the filter/moon advisory pattern — never a score input; hard horizon still gates inclusion). Client: `SiteSettings.softWarningAltitudeDeg` + JSON mapping, Settings → Safety → Site row (+ settings/help registries), wizard screen 16 field + `applyDraftToSite`. Test factory `Site(...)` gains `soft = 0` (advisory off) so pre-existing exact reason-count assertions stay meaningful; +2 tests (tag present below the mark + zero-point; absent when cleared or disabled). 43/43 TonightSky tests + 1484 client tests pass. Next: re-scan PORT_TODO/ROADMAP for the next buildable slice (remaining §37.5 fields need engine consumers).
