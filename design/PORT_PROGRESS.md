...
- **Last merged:** PR **#923** (2026-08-05, §29 storage & filenames — the whole arc in one branch: USB drive choose/mount/reformat via a sudoers-scoped helper (argv-only, UUID/dev-path validated, atomic fstab, full PKNAME parent-chain system-disk refusal), §29 template file naming server-expanded at capture, the full FITS header story (per-group switches + "See every header" live-values reference), `POST /storage/rescan`, notification inbox; 9 review rounds — the review caught a real hydrate-before-save data-loss path, the reformat confirm-label no-op, capture-time ephemeris, blank-disk onboarding, torn-fstab risk, and capture-vs-reformat races, all fixed in-branch). **Back-fill note (2026-08-05): every PR in the #877→#923 stretch except #918 skipped its CHANGELOG/PORT_PROGRESS updates — reconstructed from `gh pr list` in the docs-sync commit that adds this entry. The arcs: §63.17–.19 guider equipment mgmt (#877–#885), §45 polar alignment end-to-end (#888–#891), window-size + guiding-tune-dialog polish (#893/#894), §25 setup flow + §63.20 guider wizard (#897/#898), OpenAstro Guider rebrand (#900), §76 Wizard 2.0 (#901–#907), framing mosaics + SQM/sky-temp + discovery fallback (#912/#913/#916), settings truth-telling (#918), §77 relocation (#917).** Before #877: **#824** (§43-2 async backup create @ `94d2836e9`). Before it: **#823** (§30.7.4 e-4b-2 calibration_state record @ `0d4407f06` — the guider-slice `calibration_state` profile section (`dark_library`+`defect_map`, `{valid, last_built_at}`) wired through DTOs/normalizer/stores/snapshot/share-strip; both build-complete paths stamp best-effort AFTER the complete WS event; read-only `GET /profile/calibration-state`; 2 rounds — r1 caught the real profile-switch-mid-build misattribution → `activeProfileIdResolver` captures `IProfileRepository.ActiveId` at build accept and the stamp is skipped (Warning-logged) if it changed; r2 explicitly approved). Before it: **#822** (§63.4 delete hook — r2 caught my false selected-twin invariant → `IsTwinSelectedOnDaemon` guard), **#821** (§37.5 wizard unattended-shutdown — **§37.5 CLOSED** except deliberately-deferred items), **#820** (soft-warning altitude), **#819**+**#818** (logs/bug-report streaming CLOSED), **#817** (max-runtime cap), **#816** (clear-field), **#815** (§63.8), **#814** (§50 tiles), **#813** (§30 active-server), **#812** (§36.8 4b), **#811** (§42.5), #806–#810, the §42 epic #795–#805.
- **Currently working on:** **§77 planetary → `feature/planetary` incubation branch (2026-08-03, Joey's call).** The capture engine is PROVEN on hardware (Ceres 462M: 100% frames kept at 640×480@303 fps and 320×240@581 fps, sustained 3-minute runs to a T7 SSD, O_DIRECT, honest accounting; full-frame limited to ~63–70% kept by the shared USB3 bus) — but against the "Astrophotography Perfected" bar a shippable planetary FEATURE still needs the interactive experience (live high-fps preview + focus feedback, on-sky-validated pointing, RGB clip choreography): months of UX work. To keep the mainline focused, ALL §77 code (P1 engine, P2 surface, spike fixes, bandwidth tunes, unmerged P4 pointing) lives on `feature/planetary`, kept in sync with master; the four code PRs (#910/#911/#914/#915) are reverted on master; §77 design docs stay on master. Reopen trigger: DSO experience polished + P3 preview buildable to an excellent standard. PREVIOUS: **§18.I REST centering trigger** (branch `s18i-centering-endpoint`, SERVER) — the LAST PORT_TODO tail-sweep item: `POST /api/v1/platesolve/center` (`CenterRequestDto` ra_hours/dec_degrees, range-validated incl. NaN → 400) enqueues the §28 slew-and-center loop as a §65.5 background job (autofocus-sweep pattern): 202 + `BatchJobDto` with Location `/api/v1/jobs/{id}`, poll/cancel via the jobs endpoints, job total = profile `NumberOfAttempts` (clamped ≥1), one tick per completed solve attempt. Single-flight layered (one "center" job per type + `CenteringService`'s gate vs the §58.4/§35 recenters); work body maps `PlateSolverConfigurationException`/equipment faults onto the job runner's failure allow-list (Failed + honest, path-sanitized error_message — never a stranded "running" job). +9 tests (`CenteringEndpointTest`, real body through a live `InMemoryBatchJobService`); DTOs registered in `AraJsonSerializerContext` (incl. the previously-missing `PlateSolveRequestDto`/`PlateSolveResultDto`). **This closes the design-folder buildable tail — everything after is hardware/user/infra-gated.** PREVIOUS: **#841 §35 auto-resume pointing** MERGED @ `57064a490` (3 rounds — r1 lookup-phase OCE escape → CancelledByRelapse; r2 raw `Items` walk → `GetItemsSnapshot()`; r3 approved). Before it: **#840 §57 closers** @ `e6de98331`. 19 PRs this session (#823–#841).
- **Next (the open queue lives in `PORT_TODO.md`'s top half):** §45 polar alignment hardware close-out (the on-rig daemon spike: real solve-error budget, 2-pt vs 3-pt call; auto-binning; bolt-calibration nudge — the engine, endpoints, WS stream, and WILMA bullseye panel shipped 2026-07-30); ARA-side adoption of the openastro-guider#57 events once that upstream PR merges (see `PHD2-GAP.md`); §59.2–59.4 Smart Focus (ROADMAP part 3); libraw DSLR RAW + Live View star-annotation polish (ROADMAP part 5).
...
- **§63.17 guider equipment management in Ara Client (2026-07-28, in flight):** plan **#877** ✅ (PORT_PLAYBOOK §63.17 — six gated PRs closing the §63.12 unshipped rows so the daemon's web portal is never needed) → PR 1 **#878** ✅ (server: `get_equipment_choices`/`discover_alpaca_servers` wire classes + `GET /equipment/guider/choices` envelope + `POST /equipment/guider/discover` with a 60 s combined-sweep cap keeping the deliberately-synchronous endpoint inside client HTTP timeouts — review r1) → PR 2 **#879** ✅ (server: §63.17 selection fields `guider_camera/_camera_id/_mount/_aux_mount/_rotator/_alpaca_host/_alpaca_port` plumbed IGuiderSettings→GuiderSettings→Phd2SettingsDto→ApplyPhd2→openapi; selection setters join the §63.5 push FIRST in the daemon's documented order under the tested `RequiresDisconnectedEquipment` window predicate; `POST /guider/profile/push` 202 + `guider.profile_pushed`; review r1 caught the dead 422 path — `EnsurePHD2EquipmentConnected` returns false, never throws — now bool-checked → typed `GuiderRpcException` → 422 with FakeGuider coverage). PR 3 **#880** ✅ (client equipment panel: choices/discover/push APIs + pickers + wizard camera pick + eq.guider.equipment.* registry) → PR 4 **#881** ✅ (DELETE /darklibrary + guider.dark_library.invalidated on guide-camera change, pure GuideCameraChanged decision) → PR 5 **#882** ✅ (client delete-with-confirm + dismissible invalidation banner) → PR 6 ✅ (POST /restart fire-and-forget via the §63.3 supervisor — deliberately not connection-gated; guidance-copy update; USER_GUIDE; §63.12 table reconciliation; the #881-suggested invalidation-emission integration test). **§63.17 COMPLETE — all guider configuration now lives in Ara Client; the daemon web portal is never required.**
...
- §36 six add-on catalogs (Sh2/LDN/Barnard/vdB/Abell-PN/Arp; open-astro/sky-data @ 9ce09f7) + a seventh, `wr-stars` (717 Galactic WR stars, type `WR*`; sky-data @ 05404eb, #1107) + offline-first .deb seed bundle (seed-manifest.tsv + boot/download seed path) + Data Manager embedded in the Sky Data panel + removal confirm.
