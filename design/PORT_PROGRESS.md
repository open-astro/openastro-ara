...
- **Phase:** §38 sequence library + engine foundation — §38j placeholder hardening (j-1…j-9) + §38k engine wiring foundation (k-1…k-8). The §38k arc gets the JSON-side engine wired end-to-end against `HeadlessSequencerFactory` with structural-container, utility-instruction, and no-equipment-condition prototypes registered. The NINA → OpenAstroAra namespace remap in `JsonCreationConverter.GetType()` (§38k-6) closes the bug that made NINA-imported bodies fall to `UnknownSequence*` at the root.
- **Last merged on `port/ara`:** PR #296 (§38k-8 Conditions round-trip) — promoted to master via #297. The §38k arc through k-8 covers everything except equipment-bound instructions (TakeExposure, Slew, SetFilter, MoveFocuser, etc.), which need equipment-mediator stubs (per playbook §14.5 NSubstitute pattern for unit tests + §14.5.1 Alpaca simulators for integration).
- **Currently working on:** Nothing — §38k-8 is the no-equipment milestone. Next chunk genuinely changes scope (equipment-mediator stubs + DI surface).
- **Next substantive work:** **§38k-9+ equipment-bound instruction prototypes** — register `SetTracking` / `MoveFocuserAbsolute` / `TakeExposure` etc. as factory prototypes alongside their mediator dependencies. The cleanest start is a single mediator (e.g. `HeadlessTelescopeMediator`) + the smallest telescope-bound instruction (`SetTracking` or `ParkScope`), then expand outward. **Phase 14e Alpaca simulator pinning** per §14.5.1 (v0.4.0 already published by ASCOMInitiative — un-block the SHA-256 download + integration tests). OpenCvSharp4 + libraw wiring per §line-2105 to un-stub the `NotImplementedException`s in `Image/ImageData/`. `IXxxMediator → IXxxService` rename per §8.1 mapping table (cosmetic follow-up). v0.0.1-ara.1 release tag + RPi smoke test (user-blocked).
