...
- **Last merged:** PR #314 (§38k-9…12 tracking-doc refresh) on `master`. The §38k-13…18 equipment-mediator stub layer (PR #315) is in review. (Mediator stubs were retouched in #313: `GetDevice()` `null!` → `throw NotSupportedException` — behavior-neutral for the prototypes.)
- **Currently working on:** **§38k-13…18 (PR #315)** — completing the equipment-mediator stub layer: HeadlessCamera/FilterWheel/Rotator/Switch/Dome stubs + camera/guider/rotator/switch/dome instruction prototypes. OpenAstroAra.Test 434→479; 498 total; clean under TWAE.
- **Next substantive work:** the **profile-and-pipeline-bound instructions** deferred from §38k-18 — `Dither` + `SwitchFilter` (need `IProfileService` wired into the headless daemon), `SynchronizeDome` (needs `IDomeFollower`), and the full **`TakeExposure` capture path** (`IImagingMediator` stub + the image pipeline + exposure-info plumbing for frame type / gain / binning). Then the **Connect/Disconnect/SwitchProfile capstone** (needs the full mediator set incl. `IFlatDeviceMediator` + `IWeatherDataMediator` stubs). **Phase 14e Alpaca simulator pinning** per §14.5.1 (v0.4.0 already published by ASCOMInitiative — un-block the SHA-256 download + integration tests). OpenCvSharp4 + libraw wiring per §line-2105 to un-stub the `NotImplementedException`s in `Image/ImageData/`. `IXxxMediator → IXxxService` rename per §8.1 mapping table (cosmetic follow-up). v0.0.1-ara.1 release tag + RPi smoke test (user-blocked).
