...
- **Last merged:** PR #328 (§14e PR8 — Focuser device, Move) on `master` — capabilities + live runtime over REST, range-validated `Move`, `Halt`-before-disconnect. Follows #321–#327. The §38 sequence-engine thread (#313–#320) remains functionally complete.
- **Currently working on:** **§14e PR9 — Rotator device (Move).** `RotatorService` (real `IRotatorService`) connects to a discovered Alpaca Rotator, serves its live runtime (state + mechanical/sky angle, §32.4-cached) over REST, and rotates it via `MoveAsync` (202-Accepted background op — validates the `[0,360)` angle, `MoveMechanical`/`MoveAbsolute` off-thread per `UseSkyAngle`, refreshes the cache). Mirrors the now-hardened control-device template (generation supersede, single-flight refresh, `Halt`-before-disconnect, cache reset on adopt, extracted+unit-tested `IsAngleOutOfRange`). REST-only — the `IRotatorMediator` unification is the follow-up. Covered by 10 sim-free unit tests + a live `[Category("Integration")]` test (discover→connect→rotate→verify angle→disconnect). Next: FilterWheel/FlatDevice/Dome/Telescope/Camera/Guider.
