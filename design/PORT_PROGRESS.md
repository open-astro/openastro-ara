...
- **Last merged:** PR **#811** (§42.5 guider faults into the fault log @ `bc94daa27` — `GuiderService` records a `Guider`/`disconnected` fault-log row on the loss latch, stamps the policy action (or `notify_only`), resolves on observed reconnect; clean review, +4 tests incl. the FakeGuider end-to-end bench. **The last actionable §42.5 follow-up — the §42 fault architecture is fully closed out.**) Before it: **#810** (StarDetector cap early-out — #775 §64 trio complete), #809, #808, **#807** (§36 framing PA — all ROADMAP part-3 sequencer-instruction items closed), **#806** (§38 rotation fidelity), **the complete §42 epic #795–#805** (ROADMAP part 1 done).
- **Currently working on:** **§36.8 Tonight's Sky slice 4b** (branch `tonight-sky-overrides`) — the client side of the per-request optics/mosaic overrides the daemon has accepted since slice 4a: `TonightOverrides` model (query-param mapping, value equality), a session-scoped `tonightSkyOverridesProvider` the ranking provider watches (Apply/Reset refetch automatically), and the panel's what-if dialog (tune button, per-field blank-means-profile, validation mirroring the server caps, active-tint reminder). Scoping note recorded en route: §59.2–59.4 Smart Focus turned out ESSENTIALLY COMPLETE (the §59.4 selector, calibration store, Phase-2 one-frame runner, §59.10 collimation verdict are all shipped — ROADMAP/PORT_TODO corrected; only the asymmetry coefficient remains, and it's user-co-design). (Before it: **#811** §42.5 guider faults into the fault log @ `bc94daa27`; **#810** StarDetector cap early-out — the #775 §64 trio complete; #809, #808, #807, #806, the §42 epic #795–#805.) Next: part-6 client debt (canonical active-server provider — latent multi-server bug) or the part-5 repo-frame annotation guard.
