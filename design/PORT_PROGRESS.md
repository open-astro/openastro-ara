...
- **Last merged:** PR **#814** (§50 responsive dashboard tiles @ `92838289c` — `ResponsiveTileGrid` replaces the fixed-200 `Wrap` for Overview/Achievements tiles + milestone badges; 2 review rounds, r2 clean; also struck the stale custom-horizon ROADMAP entry, verified shipped as the per-azimuth sin LUT in `TonightSkyService.Rank`). Before it: **#813** (§30 active-server provider — move-to-end fix + all per-server call sites through `activeServerProvider`), **#812** (§36.8 slice 4b), **#811** (§42.5 guider fault log — §42 fully closed), #806–#810, the §42 epic #795–#805.
- **Currently working on:** **§63.8 poll-failure visibility** (branch `data-manager-poll-visibility`, ROADMAP part 4 — closes the #770 round-2 follow-up) — `PollBuildProgressAsync` gains per-build failure accounting behind the deliberate per-tick swallow: one Warning after 5 consecutive failed polls (`BuildProgressPollWarnAfterFailures`), plus a drain-time Warning when the whole build produced ZERO successful reads (covers builds shorter than the streak threshold); build isolation unchanged. New bench (all-failing progress RPC via a throwing FakeGuider factory → JSON-RPC error) proves exactly ONE warning per build, the build still completes, and no progress event fires. Next: part-6 wizard clear-field affordance, §37.5 safety extras, or logs streaming.
