...
- **Currently working on:** **§57 closers — watchdog abort reason + §57.5 latency logging** (branch `s57-latency-watchdog`, SERVER) — the PORT_TODO tail sweep (user asked "what's left in design/") surfaced these as buildable: (a) `StopSlew()` (only caller: the flip watchdog) now routes through the shared `AbortSlewCoreAsync(client, reason, rethrow)` — fire-and-forget/non-throwing for the flip-recovery path but with the full watch-noting races and lifecycle guarantees, so a watchdog-killed flip slew publishes `telescope.slew_aborted` w/ `reason: "watchdog"` and its complete is suppressed (E2E test); (b) §57.5 adapted: every abort logs a structured `LogAbortIssued` line (reason + server-side issue latency ms + episode-open) — kept in the LOG not the faults table (an abort is a measurement, not a fault). Both PORT_TODO entries ✅'d; ROADMAP §57 entry updated. NEXT from the tail sweep: §35 auto-resume pointing (re-center to checkpointed target before releasing), then §18.I REST centering trigger. PREVIOUS: **#839 §38 schema check + §42.2 row-3 N/A** MERGED @ `ff5ff9c7a` (clean round-1). 17 PRs this session (#823–#839).
