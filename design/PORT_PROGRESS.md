...
- **Last merged:** PR **#356** (§2105 render PR3 — `RenderedImage.Stretch` via a new public parameterized `Stretcher.Stf(targetBackground, shadowSigma)`; `AutoStf` = `Stf(0.25, 2.8)`) on `master` @ `f1158bd22`. See the **2026-06-11 session (#349–#356)** block under Completed for the full set.
...
- **Currently working on:** **§2105 in-memory render PR4 — `RenderedImage.Debayer` (full-resolution)**, then the remaining render stubs (`DetectStars`, `UpdateAnalysis`). §2105 PR1-3 (#354/#355/#356) un-stubbed the thin §65-wrapper methods + fixed a real AutoStf over-brightness bug; remaining stubs need real algorithm work (full-res debayer kernel, star detection) — tracked in PORT_TODO "§2105 remaining stubs".
