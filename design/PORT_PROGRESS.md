...
- **Last merged:** PR **#815** (§63.8 poll-failure visibility @ `ea8f0365b` — `PollBuildProgressAsync` per-build failure accounting: one Warning after 5 consecutive failed polls + a drain-time Warning on zero-success builds; 3 review rounds — r1 parity note adopted, r2 added the mid-build streak bench; closes the #770 round-2 follow-up). Before it: **#814** (§50 responsive tiles), **#813** (§30 active-server), **#812** (§36.8 slice 4b), **#811** (§42.5 — §42 fully closed), #806–#810, the §42 epic #795–#805.
- **Currently working on:** **§37 wizard clear-field affordance** (branch `wizard-clear-field`, ROADMAP part 6) — root cause confirmed: `POST /profiles` CLONES the active profile, and the wizard mappers' keep-on-blank meant an old rig's ASTAP paths / save directory / filename template could never be shed. New `ProfileDraft.clearedFields` + `ClearableField` keys; `WizardTextField` gains an `onCleared` reset suffix; screens 11+13 wire the four harmful fields (mark on reset, un-mark on type); `applyDraftToPlateSolve`/`applyDraftToStorage` write the section DEFAULT for marked fields. +4 mapper tests; 1484 client tests pass. Next: §37.5 safety extras or logs streaming (part 6).
