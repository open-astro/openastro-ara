...
- **Last merged:** PR #340 (§14e PR20 — Profile source-of-truth) on `master` — `StoreBackedProfileService` hydrates the live `ActiveProfile` from `IProfileStore`/`profile.json` at startup + on every settings PUT (new `IProfileStore.Changed`), via the unit-tested `ProfileStoreMapper`. Executing instructions now read user-edited settings. Follows #321–#339.
- **Currently working on:** **§38.7 — disk-shipped starter sequence templates.** Three real NINA-schema starter templates ship with the daemon (`OpenAstroAra.Server/templates/{lrgb-dso,narrowband-shoo,comet}.json`, copied to the build/publish output via csproj `Content Update`): full `SequenceTemplateDto` JSON (snake_case per the wire convention) whose `body` is an engine-faithful `$type` graph — `SequentialContainer` blocks with `LoopCondition` iterations, resolved `SwitchFilter` (FilterInfo `_name`/`_position`), `Annotation`/`WaitForTimeSpan`, and **NINA-verbatim `TakeExposure` capture nodes** (`NINA.Sequencer.SequenceItem.Imaging.TakeExposure` — deliberately the upstream `$type`, since the capture path isn't ported: they degrade to no-op `UnknownSequenceItem` today and resolve via the §38k-6 remap the day TakeExposure lands). `PlaceholderSequenceTemplateService` seeds them into `{profileDir}/sequences/templates/` on startup (copy-if-missing only — user edits never overwritten; ctor opt-out for the merge-behavior tests). `StarterTemplateTest` validates the exact shipped files in CI: DTO parse, §38.5 validation incl. capturable reachability, real-engine deserialization with registered types resolving (root container, 4 filter blocks with loop conditions + SwitchFilter on the LRGB template) — so schema drift fails CI, not a user's first run.
