...
- **Currently working on:** **§57 slice 2 — WILMA Stop Mount overlay + post-stop modal** (branch `s57-stop-mount-ui`, CLIENT) — `StopMountListener` wraps the app shell (inside SafetyAlarmListener): `telescope.slew_started` surfaces the §57.3 red panic banner over every tab (min 200×80 button, Space shortcut via CallbackShortcuts, commanded target RA/Dec shown, single-tap no confirmation), tap → `mountProvider.abortSlew()` with the overlay held until the daemon confirms (the event is truth), `slew_aborted` closes it and opens the §57.4 post-stop modal (halted position + Resume / Skip-this-target [skipCurrent+resume] / End-session [stop] against the run id stamped by the follow-up `sequence.paused` event; a no-run press explains instead of throwing), `slew_complete` + server-link-drop clear a stale overlay. +7 widget tests. Remaining §57: Verify-Position (capture+solve follow-up), §57.5 latency logging, flip-watchdog abort reason (PORT_TODO). PREVIOUS: **#836 §57 slice 1 (server)** MERGED @ `52428b128` after SEVEN review rounds (r1 idle-abort poisoning, r2 abort-vs-poll race + stale target, r3 unconditional pause + single-shot abort, r4 fresh halted position + retarget race, r5 consume-or-clear + MountStopHandler seam, r6 consumed-mid-failure lifecycle close, r7 approved); 14 PRs this session (#823–#836).
