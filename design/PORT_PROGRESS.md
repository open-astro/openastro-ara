...
- **Last merged:** PR **#820** (§37.5 soft-warning altitude @ `9b98e6e78` — `SiteSettingsDto.SoftWarningAltitudeDeg` + Tonight's Sky zero-point advisory tag on targets that never clear it; 2 rounds, r1 registry-gate fix: the §61 checker wants single-quoted Dart strings). Before it: **#819**+**#818** ("Logs + bug report streaming" CLOSED), **#817** (§37.5 max-runtime cap), **#816** (§37 clear-field), **#815** (§63.8), **#814** (§50 tiles), **#813** (§30 active-server), **#812** (§36.8 4b), **#811** (§42.5 — §42 fully closed), #806–#810, the §42 epic #795–#805.
- **Currently working on:** **§37.5 wizard unattended-shutdown coverage** (branch `wizard-unattended-shutdown`, ROADMAP part 6) — scoping found the "WILMA-offline auto-abort" consumer ALREADY SHIPPED (`UnattendedShutdownService`, §58.12: PausedAwaitingUser + unattended silence → park/warm/disconnect ladder; profile fields `unattended_shutdown_{enabled,wait_minutes}` + Settings panel + registries all exist) — the only gap was the WIZARD's safety screen. Screen 15 now carries the pair: on/off dropdown (Keep default / On / Off; picking Off clears the wait so no stale value reaches Save, mirroring the Ignore/auto-resume pattern) + wait-minutes field (1–720, blank keeps base), mapped through `applyDraftToSafety` onto the existing section fields. +1 mapper test; 1485 client tests pass, analyze clean. §37.5 is now CLOSED except deliberately-deferred items (per-weather actions — PORT_DECISIONS 2026-07-07; alarm knobs — device-local). Next: re-scan PORT_TODO/ROADMAP parts 5–7.
