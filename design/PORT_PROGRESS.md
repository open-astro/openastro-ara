...
- **Currently working on:** **§44.4 bandwidth cap** (branch `feat-44-4-bandwidth-cap`) — the last §44 open item: configurable Mbps cap on the backup puller (0 = unlimited; persisted in the device-local prefs beside toggle/folder), applied as inter-frame pacing (`paceFloor(bytes, mbps)` + a `pacer` seam so tests assert requested waits); the session's first pull doubles as the §44.4 link measurement ("link ≈ N Mbps" on the Settings status line). New Settings → Storage row + settings/help registry entries (gates 132/67). +3 controller tests; client suite 1407 green. Prior merged: **#740** §44 progress surfaces (@ `3142b8bde`), **#739** §35.1, **#738** §35.3, **#737** §48 client, **#736** §44 client, **#735** §48 server, **#734** §44 server.
