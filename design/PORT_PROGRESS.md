...
- **Last merged:** PR **#748** (§59.9 autofocus conditions deferral — `IAutofocusConditionGate` seam consulted by all five AF triggers, fail-open `DiagnosticsAutofocusGate` over §51 diagnostics, once-per-episode notification, `AutofocusAfterExposures` deferral latch + cadence re-anchor; **8 review rounds, every finding fixed same-round** incl. a back-to-back-AF cadence bug and an abandoned-read connection leak) on `master` @ `ac66bf356` — **the §59.5 arc is COMPLETE** (#746 trigger family → #747 live HFR feed → #748 deferral). Prior: **#747** live per-frame HFR analysis; **#746** trigger family + `IImageHistory` seam (also fixed the empty-`TriggerRunner` silent no-op and the never-registered ReconnectTrigger prototype); **#745** AlpacaFaultProxy flake fix; **#744** §37.5 wizard weather thresholds.
- **Currently working on:** **§43-2b(c) frames-catalog backup area** (branch `backup-areas-43-2c`) — every backup zip carries a consistent `db/openastroara.db` snapshot (§43.4; `BackupDatabase`, self-contained, degrades honestly to config-only on a catalog hiccup) + the §43.7 `FramesMetadataRows` manifest count; `restore_frame_metadata` swaps the catalog back (ClearAllPools + WAL/SHM sidecars set aside as a set); the checksum hash moved onto the restore worker (corrupt archive → failed clone-status, not a sync 422); WILMA restore dialog gains the default-OFF "Frame catalog (metadata)" checkbox; `restore_logs` documented reserved-by-design (not a §43.4 area).
