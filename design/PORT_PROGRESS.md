...
- **Currently working on:** **§64 Live View detection off the capture gate** (branch `liveview-detect-offload`, PORT_TODO #775 follow-up) — the live loop is split: `AcquireLiveFramePixelsAsync` (device half: settings → expose → poll → download → conversion) is all that still holds `_captureInFlight`; `PublishLiveFrame` (render/detect/JPEG + the frame write) runs off-gate, so a manual capture interleaves while a frame renders; render failures still count toward the self-stop but no longer reset `settingsApplied`. (Before it: **#808** §64 OSC annotation @ `970d719b7`, 1 round ✅; #807 §36 PA; #806 §38; the §42 epic #795–#805.) Remaining nearby: the `LiveViewMaxMarkers` real early-out (changes detector selection semantics — its own slice), repo-frame annotation seam, §59 Smart Focus remainder
