...
- **§38k-9+ equipment-bound instructions** — `SetTracking` (telescope), `MoveFocuserAbsolute` (focuser), eventually `TakeExposure` (camera). Each needs a mediator stub (`HeadlessTelescopeMediator` etc.) implementing the corresponding `IXxxMediator` interface from `OpenAstroAra.Equipment.Interfaces.Mediator`. Per playbook §14.5 line 1281, NSubstitute mocks suffice for unit tests; integration tests use real Alpaca simulators per §14.5.1.
