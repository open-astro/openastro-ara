...
- **Currently working on:** **fork in the queue — guider client done; deeper guider work now depends on the `openastro-guider` daemon's API (user says it's "almost ready").** Remaining guider items are **guider-daemon-dependent**: guider-d (§63.3 crash-detect + systemd-restart of `openastro-guider`), guider-e (§63.4/.5 — driving the daemon's `build_dark_library`/profile-push RPCs), and §45 polar-align (now the daemon's `POLAR_ALIGNMENT_DESIGN.md`, ARA drives it). **Recommend holding these until the daemon's API is frozen.** Meanwhile, **guider-INDEPENDENT v0.0.1 work** is available: Live View / §2105 image render (OpenCvSharp4 + libraw), §8.1 `IXxxMediator → IXxxService` rename (cosmetic), Phase 15 docs. Awaiting user steer on which to pursue.
