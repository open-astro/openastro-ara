...
- **Currently working on:** **§38 schema check + §42.2 row-3 N/A bundle** (branch `s38-schema-check-bundle`, SERVER tests/docs) — (a) two `SequenceJsonRoundTripTest` tests pin the §38 cross-boundary assumption: the daemon's canonical ObservableCollection wrapper `$type` tokens are byte-identical to WILMA's promotion constants (`itemsWrapperType`/`conditionsWrapperType`) AND the client-authored wrapper form deserializes — a future divergence fails CI instead of 422ing a Save; (b) §42.2 matrix row 3 (camera dew heater) recorded N/A-by-design on the `FaultPolicyMatrix` class doc (Alpaca ICameraV3 has no DewHeaterPower; dew rides a Switch port the read-back watch covers). ROADMAP §38 item struck; both PORT_TODO entries closed. PREVIOUS: **#838 §68 AlpacaBridge verify** MERGED @ `2fc7692a1` (clean round-1 approval; **§68 CLOSED** — one gap fixed: the chooser's post-setup empty state now carries the §68.2 install/systemctl guidance). Before it: **#837 §57 slice 2 Stop Mount UI** (4 rounds; **§57 CLOSED** except PORT_TODO follow-ups). 16 PRs this session (#823–#838).
