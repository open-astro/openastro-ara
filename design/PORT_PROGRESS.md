...
- **Last merged:** PR #321 (§14e Alpaca simulator harness) on `master` — pins + CI-downloads the ASCOM OmniSim (SHA-256, not committed), with a green `alpaca-sim-smoke` job proving the sim runs headless on linux-x64 and exposes a Camera over the HTTP management API. The §38 sequence-engine thread (#313–#320) remains functionally complete.
- **Currently working on:** **§14e PR2 — daemon discovery integration test.** Adds `AlpacaDiscoveryIntegrationTest` (`[Category("Integration")]`), which runs the daemon's `AlpacaEquipmentDiscoveryService.DiscoverAsync` against a live OmniSim and asserts the UDP-broadcast discovery path (port 32227) finds the simulated Camera + Telescope on `:32323` — the piece the smoke job (HTTP only) doesn't cover. A dedicated `alpaca-sim-integration` CI job starts the sim and runs `--filter TestCategory=Integration`; the solution-wide test run excludes that category. Next: real Alpaca-backed mediators (replacing the §38k stubs).
...
- ✅ **§14e discovery integration test (PR2)** — `AlpacaDiscoveryIntegrationTest` (`[Category("Integration")]`) runs the daemon's `AlpacaEquipmentDiscoveryService.DiscoverAsync` against a live OmniSim and asserts the UDP-broadcast discovery path (port 32227) surfaces the simulated Camera + Telescope advertising `:32323`. Self-gates via an HTTP probe of `:32323` (`Assert.Ignore` when no sim answers) so it skips cleanly in unit runs / on dev boxes; retries discovery up to 6× to absorb UDP timing flakiness. A dedicated `alpaca-sim-integration` CI job starts the sim and runs `--filter TestCategory=Integration`; the solution-wide `analyzer-gate` test run excludes that category (`TestCategory!=Integration`). Next: real Alpaca-backed mediators (replacing the §38k stubs).
