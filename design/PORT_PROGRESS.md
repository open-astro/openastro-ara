...
- **Last merged:** PR **#736** (§44 backup stream, WILMA client half — puller + Settings → Storage section + registries; four review rounds: persistence/claim-retry/visible-failures/watchdog, reclaim re-arm on queue success, bad-frame-doesn't-block-queue) on `master` @ `cd732e5c1`. Prior: **#735** §48 server prompt flow; **#734** §44 server half; **#733** §63.3 guider active-poll; **#732** §42.2 guider fault reactions; **#731** §35 SafetyReactionService; **#730** docs reorg.
- **Currently working on:** **§48 auto-flats, WILMA client half** (branch `feat-48-autoflats-client`) — `AutoFlatsPromptListener` at the shell level (mirrors the §27 ConnectionPolicyListener pattern: listens for `sequence.auto_flats_prompt`, a newer run's prompt replaces a stale dialog, barrier-dismiss = decide-later), the prompt dialog (panel-at-end / sky-at-twilight / not-tonight + "remember my choice"), `SequenceApi.decideAutoFlats` → `POST /sequences/{id}/auto-flats-decision`, and the new **Settings → Session → Calibration** panel exposing `calibration_capture_default` over the profile safety-policies round-trip (`CalibrationCaptureDefault` enum through model/notifier/ProfileApi mappers) + settings/help registry entries. +3 listener widget tests over the fake-socket WS harness; suite 1393 green.
- **Next (the open queue lives in `PORT_TODO.md`'s top half):** §45 polar alignment remaining phases (daemon spike → `PolarAlignService` state machine replacing the placeholder → §45.9 endpoints + WS events → the WILMA bullseye/arrows screen) once the user's openastro-guider upgrade lands; ARA-side adoption of the openastro-guider#57 events once that upstream PR merges (see `PHD2-GAP.md`); §44 progress surfaces + token-bucket cap; §35.1 weather-threshold triggers; §35.3 emergency stop; the §37.5 wizard safety/site extras now that the §35 engine enforces them; §59 Smart Focus + AF-trigger wiring; libraw DSLR RAW + Live View star-annotation polish (v0.1.0).
