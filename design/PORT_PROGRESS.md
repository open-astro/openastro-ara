...
- **Currently working on:** **§61.10 command palette slice 1 — tab navigation** (branch `command-palette-nav`, ROADMAP 7.6 committed row, client-only) — `SettingsSearchEntry` gains `tabIndex` (+ synthetic `nav.N` id); `buildSearchIndex()` prepends 4 "Go to <tab>" entries with intent keywords (planning/tonight, run/sequencer, live/capture, options/preferences); palette `_activate` switches `selectedTabIndexProvider` and pops; per-kind row icons (arrow/help/tune) + updated hint. +3 search tests; 1496 client tests green, analyze clean. Remaining §61.10 (ROADMAP row updated): actions, templates, atlas targets, equipment ops. PREVIOUS: **#827 client robustness sweep** MERGED @ `44eef7311`; **#826** @ `75bb27863`; **#825** @ `2c326ad0d`.
