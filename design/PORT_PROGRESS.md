...
- **Last merged:** PR **#794** (docs — version-based planning removed; `design/ROADMAP.md` is the consolidated design path). Before it, PR **#793** (§42.4 slice 4c — `sequence.instruction_failed` emitter: engine-raised `FailureEvent` once per FAILED transition incl. the retries-exhausted path, one event per failure occurrence incl. loop repeats, terminal drain barrier on all three terminal paths so the event always precedes the run's terminal event; **closes §42.4**; merged @ `68a6c3fd8`)
- **Currently working on:** **§42.5 fault log + REST** (branch `fault-log-425`) — `faults` table in the session DB written at the `EquipmentFaultHub` choke point, §42.3 reaction outcomes stamped onto the same row (`recovered` resolves it), run-session attribution via `ActiveRunSessionRegistry`, `GET /api/v1/faults` (+ `/{id}`) newest-first with device/kind/session/unresolved filters; then §42 WILMA surfaces → §42.2 remaining matrix rows → §42.3 generalized hot-reconnect
