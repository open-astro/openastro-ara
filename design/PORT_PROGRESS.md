...
- **Last merged:** PR #323 (§14e PR3 — first real Alpaca-backed device, `SafetyMonitorService`) on `master` — connects to a discovered Alpaca SafetyMonitor and serves live `State`/`IsSafe` over the §6 REST surface (§60.5 202-Accepted lifecycle, generation-based supersede, 11 review rounds). Builds on #321 (harness) + #322 (discovery test). The §38 sequence-engine thread (#313–#320) remains functionally complete.
- **Currently working on:** **§14e PR4 — unify `SafetyMonitorService` with the Sequencer's `ISafetyMonitorMediator`.** The same singleton now backs both the REST `ISafetyMonitorService` and the mediator (§8.1: one object, both interfaces), so `WaitUntilSafe` reads the **live** Alpaca device instead of the `HeadlessSafetyMonitorMediator` "not connected" stub. The mediator surface lives in a `SafetyMonitorService.Mediator.cs` partial; `GetInfo()` is the only real member (a bounded synchronous `IsSafe` read reusing the REST path's guards; never throws after Dispose since a running sequence may poll during shutdown), the rest mirror the stub. DI in Program.cs registers `SafetyMonitorService` once and resolves both interfaces + the mediator from it. Next: the §32.4 `cache_ttl` wrapper (serve `GetAsync`/`GetInfo` from a background-refreshed cache, gating replication), then roll the real-service pattern across the other device types.
