...
- **Currently working on:** **§31 time-sync server slice 1** (branch `s31-sync-verification`, ROADMAP part-8) — the planned "verification" found NO waterfall existed (endpoints/service/NMEA all absent; only the site round-trip). Built: `TimeSyncService` (§31.1 fresh-&-trusted gate = <1h + trust≥medium; §31.2 trust clamped to source ceiling — client/gps-mobile≤medium, manual=low; `ISystemClockSetter` seam with real `LinuxSystemClockSetter` via libc clock_settime under CAP_SYS_TIME, failure → offset tracked + Warning; location push writes through to profile site settings) + GET/POST `/api/v1/server/time-sync` (422 on unknown source) + `NmeaSentenceParser` (RMC/GGA, checksum-verified, talker-agnostic, void/no-fix rejected) ready for the GPS slice. +17 tests (9 service + 8 parser). Remaining §31: USB-GPS hosted wiring, WILMA waterfall UI. PREVIOUS: **#831 FITS keyword audit** MERGED @ `66113d201`; 9 PRs this session (#823–#831).
