...
- **Last merged:** PR **#729** (docs: Phase-15 close-out — fork CONTRIBUTING, user guide, ARA issue templates; PHD2-GAP status) on `master` @ `31d0b7483`. The arc before it: **#725–#728** — §36 Data Manager graceful-shutdown drain, §25.5.5 dome FindHome/AbortSlew/SetPark/SyncToAzimuth, §25.5.5 camera readout modes + cooler set-point read-back + pixel_size_y, §37.6 wizard recommended sky-data preset.
- **Currently working on:** repo docs reorganization (2026-07-07) — root guides moved to `docs/`, `design/` indexed via `design/README.md`, GAPS-ARA + HANDOFF archived, PORT_TODO split into open/done halves, this file's stale Current section replaced (the superseded snapshots are preserved under **Completed → Superseded status snapshots** below).
- **Next (the open queue lives in `PORT_TODO.md`'s top half):** §45 polar alignment remaining phases (daemon spike → `PolarAlignService` state machine replacing the placeholder → §45.9 endpoints + WS events → the WILMA bullseye/arrows screen); ARA-side adoption of the openastro-guider#57 events once that upstream PR merges (see `PHD2-GAP.md`); the §35 unsafe-reaction engine (prerequisite the §37.5 wizard safety extras are reclassified behind); §59 Smart Focus + AF-trigger wiring; libraw DSLR RAW + Live View star-annotation polish (v0.1.0).
