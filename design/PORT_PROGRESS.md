...
- **Last merged:** PR **#746** (§59.5 autofocus trigger family — the four remaining NINA AF triggers headless on the new `IImageHistory` seam + session-scoped `ImageHistoryService`; three review rounds surfaced and fixed real bugs beyond the port: WILMA-built trigger nodes shipped an empty `TriggerRunner` (silent no-op — fixed via `TriggerDef.runnerItems` for the AF/Dither family and an `[OnDeserialized]` re-bind for ReconnectTrigger), the ReconnectTrigger prototype was never factory-registered (every one degraded to `UnknownSequenceTrigger`), and a meridian-veto could permanently eat a filter-change refocus) on `master` @ `50e25a730`. Prior: **#745** AlpacaFaultProxy flake fix; **#744** §37.5 wizard weather thresholds; **#743** gstreamer CI deps; **#742** §35.5 alarm (§35 arc complete); **#741** §44.4 cap (§44 arc complete).
- **Currently working on:** **§59.5 live per-frame HFR analysis** (branch `frame-hfr-analysis`) — `CameraService` runs StarDetector on every captured LIGHT frame off the capture path (fire-and-forget post-registration), writes HFR + star count back via the new `IFrameRepository.UpdateAnalysisAsync`, broadcasts `frame.analyzed`, and feeds `ImageHistoryService.RecordImage` — making the HFR-drift trigger data-live and filling the library's HFR badges seconds after each frame. Frames under `MinStarsForAnalysis` (10) skip honestly. Client: the library's debounced refresh also rides `frame.analyzed`. Next in the arc: §59.9 diagnostics deferral.
- **Next (the open queue lives in `PORT_TODO.md`'s top half):** §59.9 diagnostics deferral for AF triggers (closing the §59.5 arc); §45 polar alignment remaining phases (daemon spike → `PolarAlignService` state machine replacing the placeholder → §45.9 endpoints + WS events → the WILMA bullseye/arrows screen) once the user's openastro-guider upgrade lands; ARA-side adoption of the openastro-guider#57 events once that upstream PR merges (see `PHD2-GAP.md`); §59.2–59.4 Smart Focus (v0.1.0); libraw DSLR RAW + Live View star-annotation polish (v0.1.0).
