...
- **Currently working on:** **§42.5 guider faults into the fault log** (branch `guider-faults-to-log`, flips the last actionable §42.5 follow-up) — `GuiderService` gains an optional `IFaultLogService`: the §42.2 latch records a `Guider`/`disconnected` row on fire (details distinguish link-down vs guide-camera-drop), `ReactToGuidingLossAsync` stamps the policy action (or `notify_only` idle) onto the same natural-keyed row, and `SetStateLocked`'s Connected branch resolves open rows (observed-reconnect semantics). End-to-end bench proves connect→resolve + drop→record→stamp through the FakeGuider. (Before it: **#810** StarDetector cap early-out @ `fdc3062f1` — the #775 §64 trio is complete; #809, #808, #807, #806, the §42 epic #795–#805.) Next: §59 Smart Focus remainder or remaining part-5 seams — re-read ROADMAP + PORT_TODO
