...
- **Last merged:** PR #320 (§38 precise instruction progress) on `master` — the §38 sequence-engine thread is now functionally complete (run + control + progress). **§14e Alpaca simulator harness (PR #321)** is in review: the foundation for real-equipment integration testing.
- **Currently working on:** **§14e (PR #321)** — pin + CI-download the ASCOM OmniSim simulators (SHA-256, not committed) + a smoke job that runs the sim on the linux-x64 runner and confirms its Alpaca API/Camera device. Next: the daemon `DiscoverAsync` integration test against the running sim, then real Alpaca-backed mediators (replacing the §38k stubs).
