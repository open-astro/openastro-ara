...
- **Last merged:** PR #329 (§14e PR9 — Rotator device, Move) on `master` — live mechanical/sky angle over REST, `[0,360)`-validated rotate. Follows #321–#328. The §38 sequence-engine thread (#313–#320) remains functionally complete.
- **Currently working on:** **§14e PR10 — FilterWheel device (change slot).** `FilterWheelService` (real `IFilterWheelService`) connects to a discovered Alpaca FilterWheel, serves its slots (position/name/focus-offset, read once like capabilities) + live runtime (state + current slot, §32.4-cached; ASCOM reports `Position == -1` while moving → `"moving"`/null) over REST, and selects a slot via `ChangeFilterAsync` (202-Accepted background op — validates the slot via extracted+unit-tested `IsPositionOutOfRange`, sets `Position` off-thread, refreshes the cache). Mirrors the hardened control-device template. REST-only — the `IFilterWheelMediator` unification (SwitchFilter instruction) is the follow-up. Covered by 9 sim-free unit tests + a live `[Category("Integration")]` test (discover→connect→read slots→change→verify→disconnect). Next: FlatDevice/Dome/Telescope/Camera/Guider.
