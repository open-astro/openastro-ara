...
- **Currently working on:** **§57 slice 1 — slew lifecycle events + panic-stop pause** (branch `s57-slew-events`, SERVER) — the §57 verify pass found `telescope.slew_started`/`slew_complete` were catalog-only (never published anywhere) and `/telescope/abort` didn't pause the sequencer (§57.4 step 2). Shipped: `SlewEventWatch` (observed IsSlewing transitions in the 2 s status poll, MountTrackingWatch pattern — every slew source covered for free: REST, sequencer mediator, flip, recovery, park/home; commanded targets noted at the REST + mediator slew sites ride `slew_started`; a §57.4 abort publishes `telescope.slew_aborted` IMMEDIATELY with the halted position and suppresses that episode's `slew_complete`; watch reset on connect/connection-lost), `/telescope/abort` now also calls `PauseActiveRunsAsync` (gate-arm; sequence.paused fires when the engine suspends). §57.6 safety-speed slews recorded N/A-by-transport (Alpaca ITelescope has no goto-rate control); §57.5 latency logging deferred until the button exists. +8 tests (7 pure watch + 1 scripted-mount E2E). NEXT §57: the WILMA Stop Mount overlay + post-stop modal (§57.2/57.3/57.4). PREVIOUS: **#835 §31 waterfall UI steps 4+5** MERGED @ `3dd466ce2` (3 rounds; §31 CLOSED except mobile-GPS user call + hardware field test); 13 PRs this session (#823–#835).
