...
- **Last merged:** PR **#789** (§42.2/§42.3 slice 1 — equipment fault spine + Alpaca disconnect detection: pure `DeviceConnectionProbe` (3 consecutive failed `Connected` probes per 2 s tick → Lost; success clears the streak; reset on reconnect), `EquipmentFaultHub`/`IEquipmentFaultSink` (log → `equipment.fault` WS → subscriber fan-out), one deliberate probe wired into all 8 pollable Alpaca device services with the check-and-observe folded into ONE critical section under each service's gate (round-2 TOCTOU finding), trip keeps `_device` remembered + publishes the fault; merged @ `2514e6ebb`)
- **Currently working on:** **§42.2/§42.3 fault-recovery arc — slices 2+3 paired: `FaultPolicyMatrix` + SafetyPoliciesDto policy fields + `FaultReactionService` (§42.3 hot-reconnect 0/5/15/30/60 ladder, pause-first for camera/mount, resume-on-recovery, pause / abort+best-effort-park give-up)** (branch `fault-reaction-4223`) — slices 2+3 of 6 Plan-scoped slices (1 detection spine ✓ → 2 policy matrix → 3 `FaultReactionService` + §42.3 hot-reconnect → 4 op/state-channel faults + `sequence.instruction_failed` emission → 5 §42.5 fault log + REST → 6 WILMA)
