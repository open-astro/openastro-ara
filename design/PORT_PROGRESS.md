...
- **Last merged:** PR **#823** (§30.7.4 e-4b-2 calibration_state record @ `0d4407f06` — the guider-slice `calibration_state` profile section (`dark_library`+`defect_map`, `{valid, last_built_at}`) wired through DTOs/normalizer/stores/snapshot/share-strip; both build-complete paths stamp best-effort AFTER the complete WS event; read-only `GET /profile/calibration-state`; 2 rounds — r1 caught the real profile-switch-mid-build misattribution → `activeProfileIdResolver` captures `IProfileRepository.ActiveId` at build accept and the stamp is skipped (Warning-logged) if it changed; r2 explicitly approved). Before it: **#822** (§63.4 delete hook — r2 caught my false selected-twin invariant → `IsTwinSelectedOnDaemon` guard), **#821** (§37.5 wizard unattended-shutdown — **§37.5 CLOSED** except deliberately-deferred items), **#820** (soft-warning altitude), **#819**+**#818** (logs/bug-report streaming CLOSED), **#817** (max-runtime cap), **#816** (clear-field), **#815** (§63.8), **#814** (§50 tiles), **#813** (§30 active-server), **#812** (§36.8 4b), **#811** (§42.5), #806–#810, the §42 epic #795–#805.
- **Currently working on:** **§43-2 async backup create** (branch `backup-create-async`, ROADMAP appendix items — the trigger fired 2026-07-08 when frames_metadata made archives catalog-sized) — `CreateZipAsync` now validates cheaply + synchronously (422 nothing-to-archive / NEW 507 disk-space pre-flight / NEW 409 create-in-progress with same-running-key idempotent re-accept) and packages on a fire-and-forget worker (wire shape unchanged: 202 + operation id); new poll-able `GET /api/v1/backup/create-status` (idle→running→done/failed, done carries snapshot_id) + best-effort `backup.create.started/complete/failed` WS events (optional IWsBroadcaster); create still queues behind a running restore on the shared gate. WILMA: `createBackup()` drops the interim 120 s read timeout, polls create-status to terminal (injectable interval/timeout/isCancelled), modal snacks the daemon's failure message. 72 backup server tests + 28 client backup tests green; new `BackupCreateWorkerTest` (8) + `BackupTestOps` helper migrating the 21 pre-worker call sites.
