...
- **Last merged:** PR **#740** (§44 progress surfaces — footer BackupStreamChip + per-frame per-target sync badges; one review round: SyncTarget scoping) on `master` @ `3142b8bde`. Prior: **#739** §35.1 weather thresholds; **#738** §35.3 emergency stop; **#737** §48 client; **#736** §44 client; **#735** §48 server; **#734** §44 server.
- **Currently working on:** **§35.5 audible safety alarm** (branch `feat-35-5-alarm`) — three generated loop tones bundled under `assets/audio/`, `SafetyAlarmController` (safety.unsafe / safety.emergency_stop → modal immediately + tone after the configurable silent delay, default 5 s; safety.safe auto-silences; flap-guarded; audioplayers at forced-max volume via a player seam), `SafetyAlarmListener` modal at the shell level with a Silence button, delay + tone device-local knobs in Settings → Notifications + 2 settings/2 help registry entries (gates 134/69). 5 controller tests over the fake-socket WS harness (incl. the Riverpod-3 active-listener requirement); client suite 1412 green. Vibration skipped — desktop-only app. Completes the §35 arc. Prior merged: **#741** §44.4 cap (@ `b092a5538`) closing the §44 arc, **#740** §44 surfaces, **#739** §35.1, **#738** §35.3, **#737** §48 client, **#736** §44 client.
- **Next (the open queue lives in `PORT_TODO.md`'s top half):** §45 polar alignment remaining phases (daemon spike → `PolarAlignService` state machine replacing the placeholder → §45.9 endpoints + WS events → the WILMA bullseye/arrows screen) once the user's openastro-guider upgrade lands; ARA-side adoption of the openastro-guider#57 events once that upstream PR merges (see `PHD2-GAP.md`);  the §37.5 wizard safety/site extras now that the §35 engine enforces them; §59 Smart Focus + AF-trigger wiring; libraw DSLR RAW + Live View star-annotation polish (v0.1.0).
