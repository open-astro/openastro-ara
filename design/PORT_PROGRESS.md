...
- **Last merged:** PR #331 (§14e PR11 — FlatDevice / CoverCalibrator) on `master` — §32.4-cached cover/light/brightness runtime over REST + `ApplyFlatPanelAsync` (cover + calibrator light, brightness-validated). Follows #321–#330. The §38 sequence-engine thread (#313–#320) remains functionally complete.
- **Currently working on:** **§14e PR12 — Dome (azimuth + shutter).** `DomeService` (real `IDomeService`) connects to a discovered Alpaca Dome and serves live runtime (`slewing|shutter_moving|error|parked|shutter_open|idle` state + azimuth + shutter-open/at-home/parked flags, §32.4-cached; `ShutterState` enum mapped to the summary) over REST, and drives it via four 202-Accepted background ops — `SlewAsync` (azimuth validated via extracted+unit-tested `IsAzimuthOutOfRange` [0,360)), `ParkAsync`, `OpenShutterAsync`, `CloseShutterAsync` — through a shared `RunControl` launcher; `AbortSlew` precedes disconnect. Mirrors the hardened control template. REST-only — mediator unification is a follow-up. Covered by 14 sim-free unit tests + a live `[Category("Integration")]` test (discover→connect→open shutter→slew→verify→disconnect). Next: Telescope/Camera/Guider.
