...
- **Currently working on:** **§42.2 mid-sequence guider fault flow** (branch `feat-42-guider-fault-flow`) — a guider link drop during a running sequence now executes the profile's `on_guider_lost` policy immediately (`pause_and_retry` default via the §35 `PauseActiveRunsAsync` machinery / `skip_target` via the new `SkipActiveRunsAsync` / `abort_sequence`), alongside the §63.3 process recovery; Critical notification + `guider.fault_action_taken` WS event when runs were affected, quiet when nothing was running; one reaction per disconnect episode (latch clears on the next successful connect). `GuiderRetryTimeoutSec` deliberately unconsumed until the §63.3 ARA-client auto-reconnect exists. +7 GuiderFaultReactionTest, +1 bench end-to-end drop→pause test, +1 engine SkipActiveRunsAsync test. Prior this arc: **#731** §35 unsafe-reaction engine (merged @ `ddf02cd39` — SafetyReactionService, 4-round review with flap hardening + liveness filter); **#730** docs/design reorg.
- **Next (the open queue lives in `PORT_TODO.md`'s top half):** §45 polar alignment remaining phases (daemon spike → `PolarAlignService` state machine replacing the placeholder → §45.9 endpoints + WS events → the WILMA bullseye/arrows screen); ARA-side adoption of the openastro-guider#57 events once that upstream PR merges (see `PHD2-GAP.md`); §42.2 mid-sequence guider fault flow + §63.3 active hang-detection poll (bench-testable, next up); the §37.5 wizard safety/site extras now that the §35 engine enforces them; §59 Smart Focus + AF-trigger wiring; libraw DSLR RAW + Live View star-annotation polish (v0.1.0).
