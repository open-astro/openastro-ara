...
- **Last merged:** PR **#730** (docs/design reorg — guides → `docs/`, `design/README.md` index + `design/archive/`, PORT_TODO open/done partition, apt.openastro.net release channel) on `master` @ `2d622e9b6`. Prior: **#729** Phase-15 close-out docs; **#725–#728** — §36 Data Manager graceful-shutdown drain, §25.5.5 dome FindHome/AbortSlew/SetPark/SyncToAzimuth + camera readout modes/cooler set-point read-back, §37.6 wizard recommended sky-data preset.
- **Currently working on:** **§48 auto-flats prompt flow, server half** (branch `feat-48-autoflats-prompt`) — the profile gains `calibration_capture_default` ("ask" default / panel_at_end / sky_at_twilight / never, §48.2, on SafetyPoliciesDto per §48.7's "inside the safety area"); run start announces per the token (`sequence.auto_flats_prompt` for ask, auto-`decided` for profile defaults); the SequencerService singleton now also serves `IAutoFlatsService` (§8.1 — the Phase-13.15 placeholder + its sketch DTO replaced inside the no-consumer window); on a COMPLETED run with a yes-choice the §39.5 `GenerateMatchingFlatsAsync` runs against the run's own session and panel_at_end auto-starts the generated sequence (sky_at_twilight generates + notifies — automated twilight timing is a tracked follow-up; NINA's FlatPanelFlats/SkyFlats instructions were never ported, so §39.5 is ARA's flat automation). Aborted/stopped runs never trigger flats. +4 engine tests. Prior this arc: **#734** §44 server half (@ `1ef8f5ae1`), **#733** §63.3, **#732** §42.2, **#731** §35, **#730** docs reorg.
- **Next (the open queue lives in `PORT_TODO.md`'s top half):** §45 polar alignment remaining phases (daemon spike → `PolarAlignService` state machine replacing the placeholder → §45.9 endpoints + WS events → the WILMA bullseye/arrows screen); ARA-side adoption of the openastro-guider#57 events once that upstream PR merges (see `PHD2-GAP.md`); §44 WILMA client half (Settings → Backup toggle + puller + footer progress); §45 polar-align phases 2+ once the user's openastro-guider upgrade lands; the §37.5 wizard safety/site extras now that the §35 engine enforces them; §59 Smart Focus + AF-trigger wiring; libraw DSLR RAW + Live View star-annotation polish (v0.1.0).
