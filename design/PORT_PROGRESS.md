...
- **Currently working on:** **§35 unsafe-reaction engine** (branch `feat-35-unsafe-reaction`) — `SafetyReactionService` polls the connected SafetyMonitor every 10 s and on safe→unsafe executes the profile's `on_unsafe` policy (`pause_and_park` default / `park_only` / `abort_and_park` / `ignore`): pause active runs at the instruction boundary + stop guiding + park, `safety.unsafe` fired BEFORE the action per §35.4 + `safety.action_taken` after; auto-resume-when-safe (unpark → read-back unparked → tracking on → release exactly the runs the engine paused; never a `PausedAwaitingUser` run) after the profile delay. New `ISequencerService.PauseActiveRunsAsync`/`ResumeRunsAsync` (daemon-automated — deliberately NOT §58.12 user activity). +13 SafetyReactionService tests, +2 engine-level sequencer tests. Closes the enforcement gap the §37.5 wizard safety extras were reclassified behind.
