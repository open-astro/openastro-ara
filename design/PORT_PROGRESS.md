...
- **Last merged:** PR **#806** (§38 CenterAndRotate rotation fidelity — `ICenteringExecutor.CenterAndRotateAsync`: NINA's rotate-first order (capture-solve → `Sync(solvedPA)` → `MoveRelative` by `FoldRotationDelta`, deltas folded into (-90°,+90°], looping to the profile's `RotationTolerance` bounded by its own `MaxRotationMoves=10`) then the §28 centre loop; 2 review rounds — r1 found the NumberOfAttempts coupling + missing direct executor tests, both fixed; merged @ `26ef90c54`). Before it: **the complete §42 epic, #795–#805** (ROADMAP part 1 done). Also corrected stale tracking: RunAutofocus was already fully wired (`AutofocusSweepService : IAutofocusExecutor`).
- **Currently working on:** **§36 framing position angle carried into runs** (branch `framing-pa-to-run`, closes the LAST ROADMAP part-3 sequencer-instruction item) — `buildTargetBlock`/`buildImagingRunBody` gain `positionAngleDeg` and emit a `CenterAndRotate` (Coordinates + normalized PA) IN PLACE of the `SlewScopeToRaDec` when set; threaded through `createImagingRun` from `stellarium_view._onPageEvent`'s `rotationDeg`; the dial's untouched 0° keeps the blind slew (no surprise solver requirement). Client-only slice. Next: ROADMAP part 5 imaging-pipeline items (OSC annotation/display wiring) or the §59 Smart Focus remainder — decide by re-reading ROADMAP parts 3/5 + PORT_TODO
