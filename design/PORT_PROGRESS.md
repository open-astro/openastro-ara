...
- **Currently working on:** **§44 backup stream, server half** (branch `feat-44-backup-stream`) — `BackupStreamService` replaces the placeholder: single-target slot (§44.3 — hostname-idempotent re-claim for crash recovery, stale takeover after 10 silent minutes), the §44.5 endpoint surface under `/api/v1/server/backup-stream/*` (status / claim / release / queue / ack; FITS bytes ride the existing frame download), the §44.6 frames columns (`sync_target`/`synced_at`/`sha256`, schema_version 4) with SHA-256 computed lazily + cached on first queue serve (auto-backfills pre-existing frames), unverified acks refused, takeover re-queues for the new target (per-desktop mirror). WILMA client half = follow-up slice (tracked). +7 BackupStreamServiceTest over a real temp catalog. Prior this arc: **#733** §63.3 poll+auto-reconnect (@ `ed87b95b3`), **#732** §42.2 fault flow, **#731** §35 engine, **#730** docs reorg.
- **Next (the open queue lives in `PORT_TODO.md`'s top half):** §45 polar alignment remaining phases (daemon spike → `PolarAlignService` state machine replacing the placeholder → §45.9 endpoints + WS events → the WILMA bullseye/arrows screen); ARA-side adoption of the openastro-guider#57 events once that upstream PR merges (see `PHD2-GAP.md`); §44 WILMA client half (Settings → Backup toggle + puller + footer progress); §45 polar-align phases 2+ once the user's openastro-guider upgrade lands; §48 auto-flats prompt flow; the §37.5 wizard safety/site extras now that the §35 engine enforces them; §59 Smart Focus + AF-trigger wiring; libraw DSLR RAW + Live View star-annotation polish (v0.1.0).
