...
- **Currently working on:** **§42.3 post-give-up linger re-adopt + disconnect-mid-wait harness** (branch `hot-reconnect-423`) — after a Disconnected episode's terminal action, `FaultReactionService.LingerForReadoptionAsync` polls at 120 s (bounded 30 attempts, test seams): while Error it re-dispatches the reconnect; on Connected (own dispatch or user cable re-seat) it resumes the runs the terminal pause left paused + publishes `readopted` + Info notification; a deliberate user disconnect (state Disconnected) stands the linger down. Plus `MediatorDisconnectMidWaitTest` — the 5-mediator (focuser/rotator/FW/mount/dome) disconnect-mid-settle → `SequenceEntityFailedException` regression harness owed from the #800 arc. **This closes ROADMAP part 1's §42.3 item.** Then §42.6 `affected_frames` population (last §42 epic item)
