...
- **Currently working on:** **§63 guider-c — `IGuiderMediator` unification** (build-passing WIP on branch `phase/14e-guider-c`, **NOT merged — awaiting a user transport decision**, see below). `GuiderService.Mediator.cs` makes the service also serve the Sequencer's `IGuiderMediator` (one singleton for both interfaces per §8.1, replacing `HeadlessGuiderMediator`) so the sequencer's `StartGuiding`/`StopGuiding`/`Dither` drive the **live** guider client. (guider-a folded in guider-b's RMS, so the queue skipped to -c.)
