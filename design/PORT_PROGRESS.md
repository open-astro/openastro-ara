...
- **Currently working on:** **§61.10 slice 2 — palette actions** (branch `command-palette-actions`, client-only) — `SettingsSearchEntry` gains `actionId` (id joins the getter chain); 5 action entries (library/calibration/stats/backup/wizard) with intent keywords; palette `_runAction` switch mirrors the app_shell top-bar launchers (routes via the ROOT navigator grabbed before the pop — the help-sheet pattern; BackupRestoreModal via showDialog); bolt icon for action rows. +2 search tests; 1498 client tests green, analyze clean. Remaining §61.10: equipment ops, templates, atlas targets (ROADMAP row updated). PREVIOUS: **#828 §61.10 slice 1 tab navigation** MERGED @ `85bf0e521` (2 rounds; r1: hardcoded indices → kPlanningTabIndex/kLiveTabIndex constants + app_shell asserts on all 4).
