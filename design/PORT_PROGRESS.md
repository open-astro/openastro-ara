...
- **Last merged:** PR **#738** (§35.3 emergency stop — daemon ladder + WILMA red button; four review rounds: symmetric failed_rungs, disconnected-probe, per-rung hung-driver timeout, abort-runs symmetry) on `master` @ `79d560cbf`. Prior: **#737** §48 client half; **#736** §44 client half; **#735** §48 server; **#734** §44 server; **#733** §63.3; **#732** §42.2; **#731** §35.
- **Currently working on:** **§35.1 weather-threshold triggers** (branch `feat-35-1-weather-thresholds`) — daemon: the §35 safety tick now also evaluates wind km/h (worse of sustained/gust), humidity %, and dew-delta °C over the connected ObservingConditions device (null-tolerant per sensor); a breach is UNSAFE through the same classifier/reaction/auto-resume machinery (combined verdict: any source unsafe wins, resume needs all safe); `safety.unsafe` gains a `reasons` array and the notification names the tripped threshold. New profile fields (`weather_triggers_enabled` default OFF, `max_wind_kmh` 36, `max_humidity_pct` 85, `min_dew_delta_c` 2.0). WILMA: Safety → Policies "Weather thresholds" section (master toggle + 3 numeric rows) + model/mapper + 4 settings/4 help registry entries (gates 131/66). Safety suite 23/23, client 1396. Per-trigger actions deferred by design (PORT_DECISIONS 2026-07-07). Prior merged: **#738** §35.3 emergency stop (@ `79d560cbf`), **#737** §48 client, **#736** §44 client, **#735** §48 server, **#734** §44 server, **#733** §63.3, **#732** §42.2, **#731** §35.
- **Next (the open queue lives in `PORT_TODO.md`'s top half):** §45 polar alignment remaining phases (daemon spike → `PolarAlignService` state machine replacing the placeholder → §45.9 endpoints + WS events → the WILMA bullseye/arrows screen) once the user's openastro-guider upgrade lands; ARA-side adoption of the openastro-guider#57 events once that upstream PR merges (see `PHD2-GAP.md`); §44 progress surfaces + token-bucket cap; the §37.5 wizard safety/site extras now that the §35 engine enforces them; §59 Smart Focus + AF-trigger wiring; libraw DSLR RAW + Live View star-annotation polish (v0.1.0).
