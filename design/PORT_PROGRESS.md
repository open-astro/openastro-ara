...
- **Last merged:** PR **#813** (§30 canonical active-server provider @ `ebad10461` — `SavedServerService.add` move-to-end fix (re-confirming an older rig makes it active; per-field metadata merge protects a bare host:port re-entry) + ALL per-server call sites (13 settings panels, 10 equipment device states, 8 stats screens, guider/backup/faults/library/sequence/ws/planetarium/wizard/help surfaces) routed through `activeServerProvider`/`activeServerFutureProvider`; 2 review rounds — r1 caught 29 files the `servers.last` grep missed (inline `select(list.last)` pattern), all migrated same-PR; +7 tests.) Before it: **#812** (§36.8 slice 4b what-if overrides), **#811** (§42.5 guider fault log — §42 fully closed), #806–#810, the §42 epic #795–#805.
- **Currently working on:** **responsive dashboard tiles** (branch `responsive-dashboard-tiles`, ROADMAP part 6) — `ResponsiveTileGrid` (LayoutBuilder → shared computed tile width, min 180/max 280/12 spacing; narrow→one full-width column, ultra-wide→capped) replaces the fixed-200 `Wrap` in the §50 Overview + Achievements sections and the milestone badges. +6 tests (pure width math + a layout pass). Also struck another stale ROADMAP entry verified shipped: custom-horizon in Tonight's Sky ranking (per-azimuth sin LUT in `TonightSkyService.Rank`, endpoint passes `GetCustomHorizon()`). Part-6's real remaining items: wizard clear-field affordance, §37.5 safety extras, logs streaming, planning-prose reconciliation. Next: pick from those or part-4 poll-failure visibility (#770 round-2 warn-once).
