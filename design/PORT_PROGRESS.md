...
- **Last merged:** PR **#745** (AlpacaFaultProxy bench flake root-caused + fixed — the test STUB's unguarded response write raced teardown; the proxy itself was innocent) on `master` @ `0bc4973db`. Prior: **#744** §37.5 wizard weather-threshold fields (completing the enforcement-backed §37.5 subset); **#743** gstreamer CI deps; **#742** §35.5 audible alarm **completing the §35 arc**; **#741** §44.4 bandwidth cap **completing the §44 arc**; **#740** §44 surfaces; **#739** §35.1; **#738** §35.3; **#737/#736/#735/#734** §48+§44 client/server.
- **Currently working on:** **§59.5 autofocus trigger family** (branch `af-triggers-59-5`) — the four remaining NINA AF triggers re-ported headless (`AutofocusAfterTimeTrigger` / `AutofocusAfterTemperatureChangeTrigger` / `AutofocusAfterHFRIncreaseTrigger` / `AutofocusAfterFilterChange`, from `840893eb8^`) on a new `IImageHistory` seam (Sequencer) implemented by the daemon's session-scoped `ImageHistoryService` (the §59 sweep records each completed run's temperature+filter); factory prototypes + Program.cs DI; WILMA trigger catalog grows the four defs **and** trigger runners now ship their action item (an empty `TriggerRunner` was a silent no-op — pre-existing gap fixed for Dither/AF-After-Exposures too). Follow-ups queued: live per-frame HFR analysis feeding `ImagePoints` (the HFR-drift trigger is data-starved until then), then §59.9 diagnostics deferral.
- **Next (the open queue lives in `PORT_TODO.md`'s top half):** §59.5 follow-ups (live per-frame HFR feed → §59.9 diagnostics deferral); §45 polar alignment remaining phases (daemon spike → `PolarAlignService` state machine replacing the placeholder → §45.9 endpoints + WS events → the WILMA bullseye/arrows screen) once the user's openastro-guider upgrade lands; ARA-side adoption of the openastro-guider#57 events once that upstream PR merges (see `PHD2-GAP.md`); §59.2–59.4 Smart Focus (v0.1.0); libraw DSLR RAW + Live View star-annotation polish (v0.1.0).
