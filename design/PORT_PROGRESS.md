...
- **Currently working on:** **§68 AlpacaBridge contract verification** (branch `s68-alpacabridge-verify`, CLIENT) — audit verdict: §68.1/68.2-wizard/68.4/68.5 all ✔ (details in ROADMAP §8); ONE gap fixed: the Alpaca chooser's post-setup empty state was a bare "No devices found" — it now carries the §68.2 guidance (install command + `systemctl status alpaca-bridge` + UDP-32227 subnet note), with the chooser moved onto the shared `equipmentDiscoveryApiFactoryProvider` seam (+1 widget test). The literal "critical notification on Equipment screen" reading deferred with reasoning (a bridge dying mid-session already surfaces via §42 per-device disconnect faults; a daemon-side bridge-liveness probe would be a new server feature for the only uncovered case). PREVIOUS: **#837 §57 slice 2 Stop Mount UI** MERGED @ `e6f810db7` (4 rounds — r1 remote-abort binding + SafeArea + link-drop test, r2 reconnect reconciliation + honest snackbar, r3 abortSlew guard bypass [moveAxis pattern] + multi-run modal actions, r4 approved). **§57 CLOSED** except PORT_TODO follow-ups (Verify-Position, §57.5 latency logging, flip-watchdog reason). 15 PRs this session (#823–#837).
