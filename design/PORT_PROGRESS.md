...
- **Last merged:** PR **#800** (§42.2 op-failure enforcement — all six peripheral op mediators throw `SequenceEntityFailedException` on definite failure (stall, device error, dispatched-but-never-confirmed-settled incl. mid-wait disconnects) after publishing the §42.4 fault, so `Attempts` retries / `sequence.instruction_failed` / `ErrorBehavior` finally engage; best-effort call sites guarded; 4 review rounds — r2 CaptureSolver restore + fire-and-forget switch wrapper, r3 rotator settle verdict, r4 focuser/rotator mid-wait-disconnect alignment; merged @ `6a8562ccc`. **Audit item 1 done.**) Before it: **#799** (§42.5 resolve-on-reconnect @ `64ab86c24`), **#798**/#797 (§42 WILMA surfaces), **#796** (has_more fix), **#795** (§42.5 fault log + REST).
- **Currently working on:** **§42.2 audit items 3+6 — switch re-command-once + solver exit codes** (branch `astap-recommand-422`) — `SwitchReadbackWatch` yields a one-per-episode `Recommand` verdict before faulting (the refresh loop re-issues the same value best-effort via a raw device write, settle window re-arms; a second exhaustion faults); `CLISolver` logs non-zero solver exit codes with a solver-specific `DescribeExitCode` mapping (ASTAP 1/2/16/32/33 — crash vs clean no-solution vs missing star DB finally distinguishable). Then audit items 2/4/5 (mount abort+park terminal, rotator drift watch, pause-on-persistent-op-fault) → §42.3 generalized hot-reconnect → §42.6 `affected_frames`
