...
- **Currently working on:** **blind-solver fallthrough #363, offer side** (branch `blind-solver-fallthrough`, ROADMAP part-10) — client-only: Settings engine dropdown lists ASTAP only (stored legacy astrometry.net/platesolve2 values render labeled '(not supported — ASTAP is used)'; wire enum + mapping untouched for compat); settings-registry enum → ['ASTAP'] + honest description; help entry rewritten. Registry gate + analyze clean, 1489 client tests green. The factory-side substitution warning shipped earlier (PlateSolverFactory.CreateAstapForRemovedAstrometryNet). PREVIOUS: **#825 §39 ListSessions keyset pagination** MERGED @ `2c326ad0d` (2 rounds; r1: field-attribution test gap + DISTINCT-combo dedup).
