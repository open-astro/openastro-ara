...
- **Last merged:** PR **#365** (§28 `CenteringService` — slew → solve → sync → re-slew) on `master` @ `4ced453eb`. Wires the ported `ICenteringSolver` behind `CenterOnTarget(Coordinates)`, building the capture sequence + `CenterSolveParameter` from the profile over the 5 live mediators. +3 tests. **This is the §28 "slew and center" capability + what the §58.4 flip recenter (in flight, below) now calls.** Prior: **#364** (§18.I solve-a-frame REST endpoint) + **#363** (`PlateSolveService` core). **The full plate-solve chain is wired: capture → catalog → solve endpoint → ASTAP.** **ASTAP `astap_cli` BUILT on the dev box** + fork build-fix pushed (`open-astro/ASTAP` branch `fix/fpc-3.2.2-build`). Earlier this arc: §59 curve-fits (#359-361), §58 trigger (#362). See the **2026-06-11 session** block under Completed.
...
- **Currently working on:** **§58.4 meridian-flip orchestration — `MeridianFlipExecutor`** (in flight, branch `phase/58-meridian-flip-executor`). The real `IMeridianFlipExecutor` (Server) replacing the throwing placeholder + the deleted WPF `MeridianFlipVM`: faithfully ports `DoMeridianFlip` headless — stop guiding → pass meridian (stop/wait/resume tracking) → flip slew (`ITelescopeMediator.MeridianFlip` + settle + best-effort dome sync) → recenter via `ICenteringService` (#365, best-effort) → auto-select guide star + resume guiding → settle → §58.5 side-of-pier verification. Failure path restores tracking + resumes guiding + halts the sequence; cancellation restores tracking + propagates. Re-focus deferred (live AF sweep unbuilt — logged when `AutoFocusAfterFlip` set). **Wires the `MeridianFlipTrigger` prototype into `HeadlessSequencerFactory`** (was unregistered) so a saved sequence's flip trigger resolves + executes. +10 tests (order matrix, guider-absent skip, recenter on/off, solve-fail best-effort, flip-false/throw failure-restore, cancellation, dome-sync). Mount-gated to live-validate; orchestration mock-tested. **§58.9 four-layer unattended safety (pre-flight check / in-slew watchdog / hard post-flip gate / park-on-failure) + §58.7 notifications + §58.8 first-flip confirm deferred → PORT_TODO.** Other open threads: **§45 polar-align** (guider-daemon RPCs not in its `jsonrpc_api.md`), **guider-e-2+** (profile-model+wizard), hardware-gated live AF sweep / flip + centering validation.
...
- **guider-e (§63.4/.5 profile + dark-library push) — guider-e-1 MERGED (#352); e-2 BLOCKED on a profile-model+wizard extension (no §63.5 source data in ARA's profile yet — tracked in PORT_TODO):**
  ...
  - **guider-e-2:** §63.5 profile-param push on connect — the `set_connected(false)` → push selectors/setup/algo → `set_connected(true)` → poll `get_connected` dance, mapping ARA `GuiderSettings` → the RPCs.
...
**Remaining after this session** (tracked in PORT_TODO): ~~§2105 render stubs~~ **DONE (#354–#358)**; guider-e-2+ (profile-model+wizard extension, then the push); §45 polar-align; the user/Pi-gated `v0.0.1-ara.1` release.
