...
- **Last merged:** PR **#812** (§36.8 Tonight's Sky slice 4b @ `5f9a628f5` — the panel's what-if optics/mosaic overrides dialog: `TonightOverrides` query-param model, session-scoped provider the ranking watches, blank-means-profile fields, server-cap-mirroring validation, active-tint; clean review, +13 tests. Also corrected stale tracking docs: §59.2–59.4 Smart Focus is ESSENTIALLY COMPLETE incl. the §59.4 selector + §59.10 collimation verdict — only the asymmetry coefficient remains, user co-design.) Before it: **#811** (§42.5 guider fault log — the §42 fault architecture fully closed out), #806–#810, the §42 epic #795–#805.
- **Currently working on:** **canonical active-server provider** (branch `active-server-provider`, ROADMAP part 6) — fixes the real multi-server bug: `SavedServerService.add` early-returned on a known server, so re-confirming an older rig never made it active (".last" stayed on whatever was saved most recently). Now a re-add moves the server to the end + refreshes metadata; `activeServerProvider` documented as THE canonical definition, new awaitable `activeServerFutureProvider` for providers that must not collapse loading→no-server; ALL ~24 `servers.last` call sites (13 settings panels' `_api()`, ws stream, tonight sky, site location, imaging tab, wizard discovery, solve panel, planetarium, alpaca chooser, run hydration, frame preview) migrated. +6 tests (real service round-trip via `setMockInitialValues` incl. move-to-end + metadata refresh; notifier fallback parity; active/awaitable provider picks). Next: re-scan ROADMAP parts 5–7 for the next buildable slice.
