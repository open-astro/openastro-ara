...
- **Last merged:** PR **#821** (§37.5 wizard unattended-shutdown coverage @ `34283207e` — screen 15 gains the on/off + wait-minutes pair onto the existing §58.12 fields; 2 rounds, r1 caught the dropdown not re-reporting step validity. **§37.5 CLOSED** except deliberately-deferred per-weather actions + device-local alarm knobs). Before it: **#820** (soft-warning altitude), **#819**+**#818** (logs/bug-report streaming CLOSED), **#817** (max-runtime cap), **#816** (clear-field), **#815** (§63.8), **#814** (§50 tiles), **#813** (§30 active-server), **#812** (§36.8 4b), **#811** (§42.5), #806–#810, the §42 epic #795–#805.
- **Currently working on:** **§63.4 guider profile delete hook** (branch `guider-profile-delete-hook`, ROADMAP part 4) — the lifecycle table's delete row: `DELETE /profiles/{id}` snapshots the profile name pre-delete and, on `Deleted`, fire-and-forgets `GuiderService.TryDeleteAraGuiderProfileAsync(name, id)` → maps to the `ara-<slug>-<id8>` twin (same `PHD2Guider.AraGuiderProfileName(name, id)` the connect path creates) → new `Phd2DeleteProfile` RPC (`delete_profile {name, delete_dark_files: true}`) via new `PHD2Guider.DeleteGuiderProfileAsync`. Best-effort by contract (never throws; disconnected → logged skip; ARA refuses deleting the ACTIVE profile so the twin is never the daemon's selected profile). Placeholder service returns false. +2 bench tests (FakeGuider records name + delete_dark_files=true — NOTE: FakeGuider OnRpc factories receive the WHOLE request, params under \"params\"; disconnected no-op). 12/12 guider integration tests. Remaining §63.4: clone/rename hooks (enhancement-grade — ROADMAP entry updated). Next: e-4b-2 calibration-state leftover or PORT_TODO re-scan.
