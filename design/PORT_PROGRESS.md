...
- **Last merged:** PR #336 (§14e PR16 — Dome mediator unification) on `master` — `DomeService` also serves `IDomeMediator` (five control ops via the bounded terminal-condition wait; dome-following stays the §38k-21 `IDomeFollower` stub). Third control-device mediator unification. Follows #321–#335.
- **Currently working on:** **§14e PR17 — Telescope mediator unification.** `TelescopeService` now also implements `ITelescopeMediator` (new `TelescopeService.Mediator.cs` partial; one singleton backs both, replacing `HeadlessTelescopeMediator`) — the largest mediator surface. `GetInfo()` populates what the registered instructions consume (Connected, AtPark/AtHome, `CanFindHome`/`CanPark`/`CanSetTrackingEnabled`, position + `Coordinates` in the mount's native epoch, and `TrackingModes` derived from the cached DriveRates + `CanSetTracking` for `SetTracking.Validate`). The long ops — `SlewToCoordinatesAsync` (with NINA→mount-epoch `Transform` so a J2000 target isn't off by precession on a JNOW mount, plus best-effort pre-slew tracking enable), `ParkTelescope`, `UnparkTelescope`, `FindHome` — drive the live device through the cancellation+wall-clock-bounded launcher and block until their terminal condition (settled-near-target + !Slewing, AtPark, !AtPark, AtHome). Prompt ops: `SetTrackingEnabled`/`SetTrackingMode` (DriveRate mapping; Stopped → Tracking=false; Custom unsupported), `StopSlew` (AbortSlew), `WaitForSlew`, `GetCurrentPosition` (cache, mount epoch). `TelescopeService` gains a read-once `EquatorialSystem` capture (reset on adopt) alongside its existing caps cache. Unconsumed members (MoveAxis/PulseGuide/Sync/topocentric slews/MeridianFlip/custom rates/snap port/DestinationSideOfPier) stay documented no-op stubs. So `SetTracking`/`UnparkScope`/`ParkScope`/`FindHome`/`SlewScopeToRaDec` execute against real hardware. REST surface unchanged. Covered by 17 sim-free mediator unit tests + a live mediator Unpark+SetTrackingMode+Slew added to the `[Category("Integration")]` test. Next mediator: Switch (`IWritableSwitch` population); FilterWheel/FlatDevice need `IProfileService`.
