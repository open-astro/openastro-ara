...
- **Last merged:** PR **#791** (§42.2 slice 4a — `MountTrackingWatch` (silent tracking drop: 3-tick streak, command-grace with commanded-intent gate against stale raced reads, slew/park guards, latch-through-rejected-reenable; REST FindHome classifies as park) + `CoolingDriftMonitor` (5°C/5min wall-clock sustained drift, 1°C hysteresis, latch-across-sensor-flap) + `ScriptedAlpacaDevice` TestHarness stub; merged @ `290ad2100`)
- **Currently working on:** **§42.4 slice 4b — op/write-channel fault detection: StallTimeout/OpError publishes at all 6 sequencer mediator op sites (focuser incl. settle-exhaustion, filter wheel incl. jam row, rotator, mount, dome, switch) + `SwitchReadbackWatch` (commanded-value read-back, settle window, tolerance-of-range, once-per-command-episode) + `SwitchValueTolerancePct` policy field + `switch.value_mismatch` WS** (branch `op-write-fault-424b`) — then 4c `sequence.instruction_failed` emission → 5 §42.5 fault log + REST → 6 WILMA
