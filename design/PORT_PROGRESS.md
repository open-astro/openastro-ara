...
- **Phase:** §38 sequence library + engine foundation — §38j placeholder hardening (j-1…j-9) + §38k engine wiring foundation (k-1…k-12). The §38k arc gets the JSON-side engine wired end-to-end against `HeadlessSequencerFactory` with structural-container, utility-instruction, and no-equipment-condition prototypes registered (k-1…k-8), then the first **equipment-mediator stubs + equipment-bound instruction prototypes** (k-9…k-12): SafetyMonitor/Telescope/Guider/Focuser mediators with `WaitUntilSafe`, `SetTracking`, the telescope-bound set (`UnparkScope`/`ParkScope`/`FindHome`/`SlewScopeToRaDec`), and the focuser set (`MoveFocuserAbsolute`/`Relative`/`ByTemperature`). The NINA → OpenAstroAra namespace remap in `JsonCreationConverter.GetType()` (§38k-6) closes the bug that made NINA-imported bodies fall to `UnknownSequence*` at the root.
- **Last merged:** PR #305/#306 (§38k-12 — `HeadlessFocuserMediator` + 3 focuser instructions) on `master`. Camera is the remaining equipment-bound gap (`TakeExposure` + `HeadlessCameraMediator`); the four mediator stubs and the telescope/focuser instructions have landed. (Mediator stubs were retouched in #313: `GetDevice()` `null!` → `throw NotSupportedException` — behavior-neutral for the prototypes.)
- **Currently working on:** Tracking-doc catch-up (this refresh, PR #314) — §38k-9…12 had merged while the doc still read §38k-8. After it lands, next chunk is the camera instruction (genuinely new scope: camera-info plumbing for frame type / gain / binning).
- **Next substantive work:** **§38k-13+ camera instruction** — register `TakeExposure` as a factory prototype alongside a `HeadlessCameraMediator` stub (`ICameraMediator`), adding exposure-info plumbing beyond the no-arg instruction shape used so far. **Phase 14e Alpaca simulator pinning** per §14.5.1 (v0.4.0 already published by ASCOMInitiative — un-block the SHA-256 download + integration tests). OpenCvSharp4 + libraw wiring per §line-2105 to un-stub the `NotImplementedException`s in `Image/ImageData/`. `IXxxMediator → IXxxService` rename per §8.1 mapping table (cosmetic follow-up). v0.0.1-ara.1 release tag + RPi smoke test (user-blocked).
...
- **§38k-13+ camera instruction** — `TakeExposure` (camera) is the next equipment-bound instruction, needing a `HeadlessCameraMediator` stub implementing `ICameraMediator` from `OpenAstroAra.Equipment.Interfaces.Mediator` (same stub pattern as §38k-9..12), plus exposure-info plumbing (frame type / gain / binning) beyond the no-arg shape used so far. Per playbook §14.5 line 1281, NSubstitute mocks suffice for unit tests; integration tests use real Alpaca simulators per §14.5.1. (`SetTracking` + `MoveFocuser*` + their mediators already landed in §38k-10/-12.)
