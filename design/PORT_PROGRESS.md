...
- **Phase:** v0.0.1 feature completion. The headless daemon drives real Alpaca equipment end-to-end (discover→connect→capture→§72 FITS→§28 catalog→§65 preview), runs saved sequences through the ported NINA engine, and guides via the external `openastro-guider` daemon (guider a/c/d done). Active threads: **§2105 in-memory render** (un-stubbing the inherited Image render path on SkiaSharp — PR1-3 merged), **guider-e** (§63.5 profile push — e-1 RPC classes + e-2 on-connect push & in-app editor merged #352/#371/#372/#373; e-3 profile-name mapping + e-4 dark-library remain), **§45 polar-align**. Remaining placeholders (polar-align, backup, data-manager, profile-share, Live View) are v0.1.0 per RELEASE_NOTES; the `v0.0.1-ara.1` tag + RPi smoke are the user/Pi-gated terminus.
...
- **Currently working on:** **guider-e-2 finalization** (branch `phase/guider-push-observability`) — the §63.5 push chain (#371/#372/#373) is merged; this folds in the #372-review-suggested observability (a push-summary INFO + a "dec-mode Auto, not pushed" Debug in `PHD2Guider.GuiderEngineConfig.cs`) and reconciles the tracking files (this stale "Currently working on" was still pointing at the already-merged #367). **§58 side-of-pier projection test matrix landed (#367)** — meridian flip is now trigger #362 + executor #366 + that projection coverage, functionally complete for the attended/auto flip. Other open threads: **§58.9 four-layer unattended safety + §58.7 notifications + §58.8 first-flip confirm + refocus-after-flip** (deferred → PORT_TODO); **§45 polar-align** (guider-daemon RPCs not in its `jsonrpc_api.md`), **guider-e-3** (profile-name mapping) **+ e-4** (dark-library), hardware-gated live AF sweep / flip + centering validation.
