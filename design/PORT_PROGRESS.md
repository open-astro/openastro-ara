...
- **Currently working on:** **§31 slice 4 — waterfall UI steps 4+5** (branch `s31-manual-time-entry`, CLIENT) — `TimeSyncSection` in Settings → Safety → Site (sync/source/trust, clock offset, known position, USB-GPS-detected rows; Retry + push-device-time actions; §31.1 step-4 plug-a-GPS guidance while unsynced) + `_TimeSyncManualDialog` (step 5: UTC pre-filled from device + optional lat/lng/alt, both-or-neither lat/lng validation, pushed as `manual` — server clamps to low trust; clock-set vs offset-tracked result surfaced). `TimeSyncState` model grew the full §31.3 GET fields (defensive parse, nullable alt) + `pushManual` on `TimeSyncClient`; `timeSyncStatusProvider`. ⌘K keywords time sync/gps/utc/clock → safety.site. +9 tests. Remaining §31: step 3 mobile GPS (geolocator dep = user call); live-dongle field test hardware-gated. PREVIOUS: **#834 USB-GPS worker** MERGED @ `2badf11c4` (4 rounds — r1 altitude-zeroing bug, r2 RMW-in-gate/shutdown-OCE/ordering-docs, r3 serial-collision → evidence-based not-a-GPS denylist + last-fix-device pinning, r4 approved); 12 PRs this session (#823–#834).
