...
- **Currently working on:** **guider-e-3 — §63.4 ARA-profile ↔ PHD2-profile name mapping** (branch `phase/guider-e3b-profile-wiring`). e-3a (#375) shipped the RPC classes + the pure `ara-<slug>` slug helper; e-3b (this PR) wires it into connect — `ResolveAraProfileSelection` (pure, unit-tested) + `EnsureAraGuiderProfileAsync` select-or-create on connect, before the §63.5 push, honoring the explicit `PHD2ProfileId` override. (Found `PHD2Guider.cs` is actually clean UTF-8 — the ISO-8859-1 © hazard is only `PHD2Methods.cs`.) Deferred to e-3c/v0.1.0 (PORT_TODO): slug-collision disambiguation + length cap, and the delete/clone/rename profile-lifecycle RPCs. **§58 meridian flip functionally complete** (trigger #362 + executor #366 + projection tests #367). Other open threads: **§58.9 four-layer unattended safety** (deferred → PORT_TODO); **§45 polar-align** (guider-daemon RPCs not in its `jsonrpc_api.md`), **guider-e-4** (dark-library), hardware-gated live AF sweep / flip + centering validation.
