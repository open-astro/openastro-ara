...
- **Last merged:** PR **#822** (§63.4 guider profile delete hook @ `f63923ca1` — `DELETE /profiles/{id}` snapshots the name pre-delete and on `Deleted` fire-and-forgets `GuiderService.TryDeleteAraGuiderProfileAsync(name, id)` → `delete_profile {name, delete_dark_files: true}` on the twin; best-effort, disconnected → logged skip; r2 caught my false invariant — profile select doesn't touch the guider, so the twin CAN be the daemon's selected profile → `IsTwinSelectedOnDaemon` guard + skip log. Remaining §63.4 clone/rename hooks are enhancement-grade, ROADMAP entry updated). Before it: **#821** (§37.5 wizard unattended-shutdown — **§37.5 CLOSED** except deliberately-deferred per-weather actions + device-local alarm knobs), **#820** (soft-warning altitude), **#819**+**#818** (logs/bug-report streaming CLOSED), **#817** (max-runtime cap), **#816** (clear-field), **#815** (§63.8), **#814** (§50 tiles), **#813** (§30 active-server), **#812** (§36.8 4b), **#811** (§42.5), #806–#810, the §42 epic #795–#805.
- **Currently working on:** **e-4b-2 calibration-state record** (branch `calibration-state-record`, ROADMAP part 4) — the §30.7.4 `calibration_state` profile section (guider slice: `dark_library` + `defect_map`, each `{valid, last_built_at}`): new `CalibrationStateDto`/`GuiderCalibrationEntryDto` wired through ProfileSnapshotDto (optional param) + normalizer back-fill (incl. partial blocks) + IProfileStore Get/Put (InMemory + File) + ProfileStoreSnapshot Capture/Apply (never-built profile clears live stamps) + ProfileShareService strip (donor's dark files aren't the recipient's); `GuiderService.RecordCalibrationBuilt` stamps best-effort AFTER the complete WS event on both build paths (CA1031 quiet, failure logged); read-only `GET /api/v1/profile/calibration-state`. +9 tests (7 store/normalizer/share + 2 bench: complete stamps + preserves sibling entry, failed build never stamps). Next: PORT_TODO top-half + ROADMAP parts 5–7 re-scan for the next buildable slice.
