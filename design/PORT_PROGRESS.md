...
- **Last merged:** PR **#796** (has_more pagination fix — the sentinel-consumed-before-count-check loop made notifications/diagnostics/frames lists report `has_more=false` on every page; minimal condition reorder + 3-page-walk regression tests; merged @ `69dae8032`). Before it, PR **#795** (§42.5 fault log + REST — `faults` table at the `EquipmentFaultHub` choke point, §42.3 outcomes stamped via natural-key upsert, `ActiveRunSessionRegistry` session attribution, `GET /api/v1/faults`; **closes §42.5**; merged @ `707370300`).
- **Currently working on:** **§42 WILMA surfaces, slice 1 — live fault chips + faults history client** (branch `wilma-faults-1`) — client `ActiveFaultsNotifier` folds `equipment.fault`/`fault_action_taken`/`connected` into a standing-fault set (recovery-tracked kinds hold red until `recovered` or a real reconnect; advisories amber with a 10-min TTL), blended worst-of into every top-bar equipment chip; plus `FaultRow` + `FaultsApi` over `GET /api/v1/faults` for the slice-2 feed/timeline. Then slice 2 (fault feed + §42.6 session timeline) → §42.2 remaining matrix rows → §42.3 generalized hot-reconnect
