...
- **Currently working on:** **§63.3 active hang-detection + auto-reconnect** (branch `feat-63-guider-active-poll`) — `PHD2Guider.PingAsync` (bounded `get_app_state` on a fresh per-call RPC connection) + a rescheduling ping loop in `GuiderService.ActivePoll.cs` (2 s guiding / 10 s idle; 3 consecutive failures converge on the shared `HandleLinkDownLocked` path — Error → §63.3 recovery → §42.2 fault flow); and the recovery pass now consumes its outcome — on `Recovered`, `TryAutoReconnectAsync` re-establishes ARA's client within the profile's `GuiderRetryTimeoutSec` grace window (finally consuming that field, per the #732 decision), with success/gave-up notifications; the auto-reconnect connect path deliberately does NOT supersede the recovery pass that issued it (`ConnectCoreAsync(supersedeRecovery:false)` — a user connect always does). +3 bench tests (wedge→fault-flow, streak reset, drop→recover→auto-reconnect end-to-end against FakeGuider). Prior this arc: **#732** §42.2 guider fault flow (@ `51705cf64`), **#731** §35 unsafe-reaction engine, **#730** docs/design reorg.
- **Next (the open queue lives in `PORT_TODO.md`'s top half):** §45 polar alignment remaining phases (daemon spike → `PolarAlignService` state machine replacing the placeholder → §45.9 endpoints + WS events → the WILMA bullseye/arrows screen); ARA-side adoption of the openastro-guider#57 events once that upstream PR merges (see `PHD2-GAP.md`); §45 polar-align phases 2+ against the local guider daemon (or §44 backup stream if the daemon isn't runnable yet); the §37.5 wizard safety/site extras now that the §35 engine enforces them; §59 Smart Focus + AF-trigger wiring; libraw DSLR RAW + Live View star-annotation polish (v0.1.0).
