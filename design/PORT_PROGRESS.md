...
- **Last merged:** PR **#357** (§2105 render PR4 — `RenderedImage.Debayer`, full-resolution bilinear) on `master` @ `861db061c`. See the **2026-06-11 session (#349–#358)** block under Completed for the full set. **#358 (§2105 render PR5 — `DetectStars`/`UpdateAnalysis`) is the in-flight sub-PR that closes the §2105 render thread.**
...
- **Currently working on:** **§2105 in-memory render PR5 — `RenderedImage.DetectStars` + `UpdateAnalysis`** (PR #358, in flight). New dependency-free `StarDetector` (background median+MAD → median+k·σ threshold → 8-connected blobs → flux-weighted centroid + HFR). **This is the LAST §2105 render stub** — once merged the in-memory render path is fully un-stubbed (#354–#358). Next thread: **§45 polar-align** (drives the guider daemon's polar-align API) — guider-e-2+ stays blocked on the profile-model+wizard extension.
