...
- **Last merged:** PR **#359** (§59.8 autofocus — Classic AF **parabolic** curve fit, `FocusCurveFit.FitParabolic` weighted LS → best-focus vertex + R²) on `master` @ `17:14Z`, 3 review rounds (caught a real numerical-conditioning bug at large focuser positions → pre-centred solve + scale-invariant singularity test). **PR #360 (hyperbolic fit + `FitBest` §59.8 fallback) is the in-flight follow-up.** Prior: **#358** (§2105 render PR5 — `DetectStars`/`UpdateAnalysis`; from-scratch `StarDetector` + atomic thread-safe analysis publish), **9 review rounds**, which **CLOSED the §2105 in-memory render thread** (all inherited render methods un-stubbed #354–#358; only libraw RAW decode + on-image star annotation remain, Live-View-gated v0.1.0). See the **2026-06-11 session (#349–#360)** block under Completed.
...
- **Currently working on:** **§59 autofocus — Classic AF hyperbolic fit + `FitBest`** (PR #360, in flight). `FocusCurveFit.FitHyperbolic` (`HFR = √(a² + b²·(x−c)²)`, linearised as a parabola in HFR² → reuses the conditioned solve) + `FitBest` (§59.8 selection: parabola, fall back to hyperbola when R² < 0.85 / unusable, keep the higher-R² usable fit). Pure math, +6 tests. Sub-PR 2 of §59 (parabolic #359 merged). **Remaining §59:** trendline fits (TREND*, pure math, next), the **live focuser V-curve sweep** + AF-sequence wiring (focuser-gated physical blocker — rig is camera-only, per PORT_TODO), Smart Focus (§59.2-4, likely v0.1.0). Other open threads: **§45 polar-align** (drives the guider daemon's polar-align API), **guider-e-2+** (blocked on the profile-model+wizard extension), **§58 meridian flip**.
