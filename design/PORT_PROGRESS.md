...
- **Currently working on:** **§31 slice 3 — USB-GPS self-sync worker** (branch `s31-usb-gps-worker`, SERVER) — `UsbGpsTimeSyncWorker : BackgroundService` (probe /dev/ttyUSB*+ttyACM* via `ISerialNmeaSource` seam; real `SerialPortNmeaSource` = System.IO.Ports 9600-8N1 w/ blocking ReadLine pushed off-path; per-device 10 s listen window; 2-min unsynced / 50-min synced cadence; per-device CA1031 skip boundary); first valid RMC → `TimeSyncService.ApplyGpsSyncAsync` (NEW public non-wire method, gps-internal/high, same plausibility window — bad GPS date refused, bad position dropped but time still applied; PushAsync refactored onto shared `ApplyCore`); System.IO.Ports 10.0.0 added to Server.csproj (Core already carried it transitively). +5 worker tests (25 total in the §31 fixtures). Remaining §31: waterfall UI steps 3/4/5; live-dongle field test hardware-gated. PREVIOUS: **#833 client push-on-connect** MERGED @ `c67772834`; 11 PRs this session (#823–#833).
