...
- **Last merged:** PR #335 (§14e PR15 — Rotator mediator unification) on `master` — `RotatorService` also serves `IRotatorMediator` (sky/mechanical moves via the hardened bounded-move path). Second control-device mediator unification. Follows #321–#334.
- **Currently working on:** **§14e PR16 — Dome mediator unification.** `DomeService` now also implements `IDomeMediator` (new `DomeService.Mediator.cs` partial; one singleton backs both, replacing `HeadlessDomeMediator`). Added a read-once capability cache + raw ASCOM shutter capture to `DomeService` so `GetInfo()` reports `CanFindHome`/`CanSetAzimuth`/… and the NINA `ShutterState` the instructions validate against (mapped from ASCOM). The five control ops — `OpenShutter`/`CloseShutter`/`Park`/`FindHome`/`SlewToAzimuth` — drive the live device and block on a bounded wait for their terminal condition (shutter Open/Closed, AtPark, AtHome, azimuth-reached + not-slewing), returning `true` only when reached, reusing the cancellation+wall-clock-bounded launcher from the focuser/rotator mediators. The dome-following surface (`EnableFollowing`/`SyncToScopeCoordinates`/`IsFollowingScope`) stays a no-op stub — software dome-sync is the separate `IDomeFollower` subsystem (§38k-21). So `OpenDomeShutter`/`CloseDomeShutter`/`ParkDome`/`FindHomeDome`/`SlewDomeAzimuth` execute against real hardware. REST surface unchanged. Covered by 8 sim-free mediator unit tests + a live mediator OpenShutter+Slew added to the `[Category("Integration")]` test. Next mediator: Telescope; Switch needs `IWritableSwitch` population; FilterWheel/FlatDevice need `IProfileService`.
