...
- **Currently working on:** **§31 client slice — push time on connect** (branch `s31-client-time-push`, client-only) — `TimeSyncApi` (GET state / POST client push, injectable Dio), `timeSyncOnConnectProvider` (`syncIfNeeded`: GET → if !synced POST device clock as client/medium; best-effort, failures logged+swallowed), `TimeSyncOnConnectListener` in the app_shell listener stack firing on every transition INTO connected (first connect + reconnects, the playbook cadence). +6 tests (3 API stub-adapter incl. push body shape, 3 notifier: pushes-when-unsynced, leaves fresh high-trust sync alone, swallows failures). 1505 client tests green. PREVIOUS: **#832 §31 server slice 1** MERGED @ `fcf6093d5` (2 rounds; r1: monotonic freshness + time/location plausibility bounds); 10 PRs this session (#823–#832).
