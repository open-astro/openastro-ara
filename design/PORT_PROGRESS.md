...
- **Last merged:** PR **#802** (§42.2 audit item 4 — `RotatorDriftWatch`: last commanded move (target + mechanical/sky domain) vs reported angle each refresh tick, circular distance ±0.5°, gated while moving/settling; persistent drift re-issues the exact move once then faults `ValueMismatch`; sync/reverse/failed-move/disconnect reset; 1 review round ✅, zero findings; merged @ `8776aae56`. **Audit item 4 done.**) Before it: **#801** (items 3+6 @ `08d8f75c2`), **#800** (item 1 @ `6a8562ccc`), **#799** (§42.5 resolve-on-reconnect), **#798**/#797 (§42 WILMA surfaces), **#796** (has_more fix), **#795** (§42.5 fault log + REST).
- **Currently working on:** **§42.2 audit items 2+5 — persistent-op-fault escalation** (branch `mount-terminal-pause-422`) — `OpFaultEscalator` (3 definite op faults / 10-min sliding window per device) feeds `FaultPolicyMatrix.Resolve(..., persistentOpFault:)`: Telescope → `AbortAndPark` (physical-safety row), Camera → `PauseSequence` (row 1b), peripherals stay notify-only; `FaultReactionService.ExecuteEscalationAsync` runs the terminal immediately (`escalated:*` fault-log stamp + WS action + Error notification); the camera capture path now publishes `StallTimeout`/`OpError` so its row is live. **This completes every buildable §42.2 audit item** (7 is §59.7 territory, 8 is an L-priority advisory, 9 is N/A by design). Then §42.3 generalized hot-reconnect → §42.6 `affected_frames`
