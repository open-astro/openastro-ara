...
- **Currently working on:** **§37.5 wizard safety extras** (branch `feat-37-5-wizard-safety`) — wizard screen 15 grows the now-enforced §35.1 weather-threshold fields: master dropdown (keep-default/on/off) + wind km/h, humidity %, dew-delta °C numeric fields (each nullable = keep base; thresholds clear when the master goes off so stale values never reach Save), mapped through the draft SafetyPolicies bag + `applyDraftToSafety`. Per-trigger actions stay deferred by design; alarm knobs are device-local and don't ride the profile. +2 save-mapping tests; suite 1419 green. Prior merged: **#742** §35.5 audible alarm (@ `bcc8afdff`) **completing the §35 arc**, **#743** gstreamer CI deps, **#741** §44.4 cap completing the §44 arc, **#740** §44 surfaces, **#739** §35.1, **#738** §35.3.
- **Next (the open queue lives in `PORT_TODO.md`'s top half):** §45 polar alignment remaining phases (daemon spike → `PolarAlignService` state machine replacing the placeholder → §45.9 endpoints + WS events → the WILMA bullseye/arrows screen) once the user's openastro-guider upgrade lands; ARA-side adoption of the openastro-guider#57 events once that upstream PR merges (see `PHD2-GAP.md`);  §59 Smart Focus + AF-trigger wiring; libraw DSLR RAW + Live View star-annotation polish (v0.1.0).
