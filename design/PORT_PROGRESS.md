...
- **Last merged:** PR **#747** (§59.5 live per-frame HFR analysis — StarDetector on every captured LIGHT frame via a single-reader bounded Channel worker preserving capture order, HFR+star-count write-back, `frame.analyzed` event, `ImageHistoryService` feed; two review rounds: the ordering race and the DropWrite/TryWrite silent-drop) on `master` @ `4b64d0bd6`. Prior: **#746** §59.5 trigger family + `IImageHistory` seam (3 review rounds — also fixed the empty-`TriggerRunner` silent no-op for WILMA-built AF/Dither/Reconnect triggers and the never-registered ReconnectTrigger prototype); **#745** AlpacaFaultProxy flake fix; **#744** §37.5 wizard weather thresholds; **#743** gstreamer CI deps; **#742** §35.5 alarm (§35 arc complete); **#741** §44.4 cap (§44 arc complete).
- **Currently working on:** **§59.9 autofocus conditions deferral** (branch `af-diagnostics-deferral`) — **closing the §59.5 arc**: all five AF triggers consult the new `IAutofocusConditionGate` seam before firing; `DiagnosticsAutofocusGate` defers while §51 diagnostics carries an open sky-condition issue (`clouds_passing`/`aperture_blocked`/`dew_formation`, enforcement-first), notifies once per episode, and fails OPEN on a broken/wedged read (bounded 2 s — diagnostics can never freeze focusing). Level-based triggers retry naturally; the edge-based `AutofocusAfterExposures` latches a deferred fire so the owed autofocus can't slip to exposure 2N. USER_GUIDE §5 grows the autofocus-triggers section. Prior in this arc: **#747** live per-frame HFR analysis (@ `4b64d0bd6` — HFR-drift trigger data-live, library badges fill live), **#746** the trigger family + `IImageHistory` seam.
