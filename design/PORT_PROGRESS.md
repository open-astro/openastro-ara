...
- **Last merged:** PR #316 (§38k-19/20 — last two device-mediator stubs; device set now 11/11) on `master`. §38k-21 (PR #317 — `HeadlessDomeFollower` + `SynchronizeDome`) is in review. (Mediator stubs were retouched in #313: `GetDevice()` `null!` → `throw NotSupportedException` — behavior-neutral for the prototypes.)
- **Currently working on:** **§38k-21 (PR #317)** — `HeadlessDomeFollower` stub + registered `SynchronizeDome` (deferred from §38k-18 on the `IDomeFollower` dep). OpenAstroAra.Test 485→491; 510 total; clean under TWAE.
- **Next substantive work:** the remaining deferred instructions all need a non-device dependency the headless daemon doesn't provide yet — and the cleanly-unblocked ones (camera/guider/rotator/switch/dome control + `SynchronizeDome`) are now all registered. **The gating prerequisite is wiring `IProfileService` into the headless daemon** (architectural — needs a single-source-of-truth decision vs. the existing `FileProfileStore`/`profile.json`; the playbook §8.1 says "keep" NINA's `IProfileService` as a singleton). That unblocks `Dither`, `SwitchFilter`, and the **Connect capstone** (`ConnectAllEquipment`/`ConnectEquipment`/`SwitchProfile`, + flip the `internal` `DisconnectAllEquipment`/`DisconnectEquipment` to `public` and register all five). Separately, the full **`TakeExposure` capture path** needs an `IImagingMediator` stub + the image pipeline + exposure-info plumbing. **Phase 14e Alpaca simulator pinning** per §14.5.1 (v0.4.0 already published by ASCOMInitiative — un-block the SHA-256 download + integration tests). OpenCvSharp4 + libraw wiring per §line-2105 to un-stub the `NotImplementedException`s in `Image/ImageData/`. `IXxxMediator → IXxxService` rename per §8.1 mapping table (cosmetic follow-up). v0.0.1-ara.1 release tag + RPi smoke test (user-blocked).
