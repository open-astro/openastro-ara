...
- **Last merged:** PR **#798** (§42 WILMA fault surfaces slice 2 — `FaultFeedNotifier` (cursor-paged newest-first history, WS-debounced refresh) behind the collapsible `FaultPanel` on the Imaging tab (header = live standing-fault roll-up); `sessionFaultsProvider` family + amber per-session fault badge/timeline dialog on Image Library cards; merged @ `2a659188a`. **The §42 WILMA-surfaces ROADMAP item is done** — #797 chips + #798 feed/timeline.) Before it: **#797** (slice 1 — live fault chips + history seeding, @ `5d0fe2ef1`), **#796** (has_more fix @ `69dae8032`), **#795** (§42.5 fault log + REST, **closes §42.5**, @ `707370300`).
- **Currently working on:** **§42.5 resolve-on-reconnect** (branch `fault-resolve-reconnect`) — `IFaultLogService.ResolveOnReconnectAsync` stamps `resolved_at` on a device type's unresolved `disconnected` rows from the `EquipmentEventPublisher` connected transition (fire-and-forget; FlatDevice/CoverCalibrator pair; `action_taken` untouched; tracking_lost stays recovered-only), closing the gave-up-then-manually-fixed dishonesty the #797 review arc surfaced. Then §42.2 remaining matrix rows (camera/mount/focuser/EFW mid-sequence reactions) → §42.3 generalized hot-reconnect → §42.6 `affected_frames` population
