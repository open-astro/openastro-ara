...
- **Currently working on:** **§39 ListSessions keyset pagination + batching** (branch `sessions-keyset-pagination`, ROADMAP part-10 appendix / PORT_TODO "#370 review" entry) — `SqliteCalibrationService.ListSessionsAsync`: 5-queries-per-session → 6 per page (batched `IN ($ids)` header/filters/coverage/profile; EXCEPT → null-safe NOT EXISTS, same semantics) + keyset cursor over raw `(MIN(captured_utc), session_id)` (deterministic tiebreaker; stable mid-pagination; legacy integer cursor still pages via OFFSET). +3 tests (mid-pagination stability, tie de-dup, legacy cursor) — 21/21 fixture green. PREVIOUS: **#824 §43-2 async backup create** MERGED @ `94d2836e9` (2 rounds; r1: pre-flight tree walk → O(1) stats, running-create check before pre-flight, re-accept echoes original AcceptedUtc; pre-merge full-suite flake noted on the PR — re-run fully green 2255/0).
