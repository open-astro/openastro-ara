...
- **Currently working on:** **§58 side-of-pier projection test matrix** (in flight, branch `phase/58-side-of-pier-tests`). Locks down the safety-critical flip astrometry in `OpenAstroAra.Astrometry.MeridianFlip` (`ExpectedPierSide` / `TimeToMeridian` / `TimeToMeridianFlip`) that the §58.2 `MeridianFlipTrigger.ShouldTrigger` decision depends on — it had **zero** coverage. +20 deterministic tests (JNOW coords so `Transform(JNOW)` is identity → no SOFA/precession/wall-clock dependency, cross-platform): the pier-side hour-angle projection incl. the ≥12 boundary + wrap, time-to-meridian mod-12, and both 12 h delayed-flip deferral branches (already-flipped pre-meridian + just-past-meridian-pier-matches) vs the no-pier baseline. **This closes the last §58 to-do** — meridian flip is then trigger #362 + executor #366 + this projection coverage. Other open threads: **§58.9 four-layer unattended safety + §58.7 notifications + §58.8 first-flip confirm + refocus-after-flip** (deferred → PORT_TODO); **§45 polar-align** (guider-daemon RPCs not in its `jsonrpc_api.md`), **guider-e-2+** (profile-model+wizard), hardware-gated live AF sweep / flip + centering validation.
