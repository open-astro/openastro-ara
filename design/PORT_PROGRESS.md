...
- **Last merged:** PR #332 (§14e PR12 — Dome / azimuth + shutter) on `master` — §32.4-cached state/azimuth/shutter runtime over REST + four 202-Accepted ops (slew/park/open/close shutter) via a shared `RunControl` launcher; shutter-fault wins state precedence. Follows #321–#331. The §38 sequence-engine thread (#313–#320) remains functionally complete.
- **Currently working on:** **§14e PR13 — Telescope (mount).** `TelescopeService` (real `ITelescopeService`) connects to a discovered Alpaca Telescope, serves read-once capabilities (`CanSlew/Sync/Park/Unpark/SetTracking/PulseGuide/FindHome` + supported sidereal rates) + §32.4-cached runtime (`slewing|parked|tracking|idle` state + RA/Dec + tracking/parked/at-home) over REST, and drives it via three 202-Accepted background ops — `SlewAsync` (goto via `SlewToCoordinatesAsync`, or sync via `SyncToCoordinates` when `Sync=true`; RA `[0,24)`/Dec `[-90,90]` validated via extracted+unit-tested `IsCoordinateOutOfRange`), `ParkAsync`, `UnparkAsync` — plus two prompt synchronous writes, `SetTrackingAsync` and `AbortSlewAsync` (§57 panic stop). `AbortSlew` precedes disconnect. Mirrors the hardened control template. REST-only — the `ITelescopeMediator` unification is a follow-up. Covered by 18 sim-free unit tests + a live `[Category("Integration")]` test (discover→connect→unpark→track→slew→verify→disconnect). Next: Camera/Guider/PolarAlign.
