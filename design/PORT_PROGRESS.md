...
- **Currently working on:** **§18.I plate-solving — solve REST endpoint** (PR #364, in flight). `POST /api/v1/platesolve/frames/{id}/solve` — loads a catalogued frame's pixels as `IImageData` (new `IFrameRepository.LoadImageDataAsync`, reusing the preview FITS read path) → `PlateSolveService.SolveImage` → `PlateSolveResultDto`. Lets a captured frame be solved over REST (for the user's night ASTAP validation). +tests on the result→DTO mapping. **ASTAP `astap_cli` BUILT on the dev box** (`~/astap/astap_cli`; fork build-fix pushed to `open-astro/ASTAP` branch `fix/fpc-3.2.2-build` — 2-line `{$mode objfpc}{$H+}` for FPC 3.2.2, also fixes the Pi `linux_aarch64` build). Live solve still needs a star DB + a night sky frame (user-gated). **PR #363 (`PlateSolveService` core, image-in→solution-out) MERGED** @ `20:31Z`. Next: the §28 centering loop (capture→solve→sync→re-slew) → §58.4 flip recenter. **§58 trigger (#362) merged.** Other open threads: **§45 polar-align** (guider-daemon RPCs not in its `jsonrpc_api.md`), **guider-e-2+** (profile-model+wizard), hardware-gated live AF sweep / flip validation.
