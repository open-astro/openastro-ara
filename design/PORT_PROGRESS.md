...
- **Last merged:** PR **#816** (§37 wizard clear-field affordance @ `5f1d81076` — `ProfileDraft.clearedFields` + `WizardTextField.onCleared` reset suffix on the four clone-trap fields (ASTAP paths, save dir, filename template); mappers write section defaults for marked fields; clean review). Before it: **#815** (§63.8 poll-failure visibility), **#814** (§50 tiles), **#813** (§30 active-server), **#812** (§36.8 slice 4b), **#811** (§42.5 — §42 fully closed), #806–#810, the §42 epic #795–#805.
- **Currently working on:** **§37.5 max-sequence-runtime cap** (branch `safety-max-runtime`, ROADMAP part 6, enforcement-first) — `SiteSettingsDto.MaxSequenceRuntimeMin` (int minutes, ctor default 0 = no limit, keeps old profile.json deserializing) + the sequencer's `WatchRuntimeCapAsync` watchdog (runs beside `Sequencer.Start` under a linked CTS, drained in a finally so a stop can't land on a later run; re-reads the cap each 60 s tick so mid-run Settings changes apply; graceful user-stop path → terminal `sequence.stopped` + one Warning notification; profile-read faults skip the tick). Client: `SiteSettings.maxSequenceRuntimeMin` + JSON mapping, Settings → Safety → Site row (+ settings/help registry entries), wizard screen 16 field + `applyDraftToSite` mapping. +2 server tests (cap stops gracefully with the notification via test seams; cap 0 never touches a run); 43/43 sequencer tests + 1484 client tests pass. Next: logs streaming or the remaining §37.5 fields.
