...
- **Last merged:** PR **#818** (§29.9 log-tail reverse byte-scan @ `6cfe95523` — `LogService.TailAsync` reads chunks backwards with byte-level carry, early-exits when the window fills; clean review). Before it: **#817** (§37.5 max-runtime cap), **#816** (§37 clear-field), **#815** (§63.8 poll visibility), **#814** (§50 tiles), **#813** (§30 active-server), **#812** (§36.8 4b), **#811** (§42.5 — §42 fully closed), #806–#810, the §42 epic #795–#805.
- **Currently working on:** **§54 download streaming** (branch `bugreport-stream`, ROADMAP part 6 "Logs + bug report streaming", second half — completes the bullet) — the daemon-log + bug-report downloads no longer buffer the body whole in client memory: `LogsClient.downloadLogTo(savePath)` / `BugReportClient.downloadTo(savePath, prepId)` stream via `dio.download` (0-byte bodies delete the empty file + throw), and the UI flow is destination-first: new `pickStreamSavePath` util (file_picker v12's `saveFile` REQUIRES the bytes, so path-only selection = `getDirectoryPath` + collision-safe `name (N).ext` uniquify) with a `savePathPicker` test seam on `SupportTab`/`BugReportCard`. Server side already streamed (BugReportService builds to a temp file). Tests: fakes updated to path-based signatures + a picked-path assertion; 1484 client tests pass, analyze clean. Next: remaining §37.5 fields (soft-warning altitude — needs a consumer) or re-scan PORT_TODO.
