...
- **Last merged:** PR **#739** (§35.1 weather-threshold triggers — thresholds in the safety tick + WILMA panel; three review rounds: combined-breach notification text, reasons-as-locals race, weather-only poll timer) on `master` @ `8d0176b19`. Prior: **#738** §35.3 emergency stop; **#737** §48 client; **#736** §44 client; **#735** §48 server; **#734** §44 server; **#733** §63.3; **#732** §42.2; **#731** §35.
- **Currently working on:** **§44 progress surfaces** (branch `feat-44-progress-surfaces`) — the backup stream's visibility layer: `BackupStreamChip` on the bottom status bar (hidden while disabled; connecting / N-pending / up-to-date / attention states with a tooltip carrying the session counts or the problem text) + per-frame cloud badges in the Image Library (`FrameListItemDto` gained optional `SyncedAt` read from the frames table's §44 `synced_at` column; the thumbnail shows cloud-done/cloud-queue only when a stream is configured — absence must not read as "unprotected"). 4 chip widget tests; client 1400, server frame/backup suites green. Prior merged: **#739** §35.1 weather thresholds (@ `8d0176b19`), **#738** §35.3 emergency stop, **#737** §48 client, **#736** §44 client, **#735** §48 server, **#734** §44 server.
- **Next (the open queue lives in `PORT_TODO.md`'s top half):** §45 polar alignment remaining phases (daemon spike → `PolarAlignService` state machine replacing the placeholder → §45.9 endpoints + WS events → the WILMA bullseye/arrows screen) once the user's openastro-guider upgrade lands; ARA-side adoption of the openastro-guider#57 events once that upstream PR merges (see `PHD2-GAP.md`); §44.4 token-bucket cap; the §37.5 wizard safety/site extras now that the §35 engine enforces them; §59 Smart Focus + AF-trigger wiring; libraw DSLR RAW + Live View star-annotation polish (v0.1.0).
