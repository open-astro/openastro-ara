...
- **Currently working on:** **§77 P2 — planetary capture surface** (branch `s77-p2-capture-surface`, SERVER): `PlanetaryCaptureService` (§77.2 arbitration — enter = camera `DisconnectAsync` + native takeover, refused via `ActiveRunSessionRegistry.HasAny` while any run is active; leave = SDK close + best-effort Alpaca reconnect from `CameraService.LastKnownDevice`), `UsbfsTuner` (`sudo set-usbfs-memory.sh`, new script + sudoers line in packaging/, RAM-scaled clamp(total/8 MB, 64, 1000)), `/api/v1/planetary` enter/leave/record-start/record-stop/status + 4 WS tokens + openapi schemas. +9 tests (`PlanetaryCaptureServiceTest`, Moq'd ICameraService + synthetic capture). PREVIOUS: **§77 P1 — planetary video capture core** SHIPPED **PR #910** @ `186ebfc80` (6 review rounds; CancelWrite + AbandonedFrames improvements over the closed AlpacaBridge PR #168 C++ reference) (branch `s77-p1-video-core`, SERVER): the C# engine per the amended §77.1 — `IVideoCapture` seam + `FrameRingBuffer` (POH arena, adaptive clamp(MemAvailable/4, 64–512 MB), CancelWrite fixes a producer-wedge latent in the C++ reference) + `SerWriter` (SER v3, Linux O_DIRECT via libc P/Invoke with aligned staging + honest buffered fallback, timestamp trailer, FrameCount patch) + `VideoRecorder` (capture/drain threads, honest accounting: captured == written + ring-dropped, SDK drops separate) + `ZwoVideoCapture`/`ZwoNative` (LibraryImport vs the AlpacaBridge-deb `libASICamera2.so`, §77.2 5 s retry-open with "still held by the bridge" error). +15 NUnit bench tests (`VideoCaptureEngineTest`, synthetic frame source; matrix ported from closed AlpacaBridge PR #168). PREVIOUS:  **§18.I REST centering trigger** (branch `s18i-centering-endpoint`, SERVER) — the LAST PORT_TODO tail-sweep item: `POST /api/v1/platesolve/center` (`CenterRequestDto` ra_hours/dec_degrees, range-validated incl. NaN → 400) enqueues the §28 slew-and-center loop as a §65.5 background job (autofocus-sweep pattern): 202 + `BatchJobDto` with Location `/api/v1/jobs/{id}`, poll/cancel via the jobs endpoints, job total = profile `NumberOfAttempts` (clamped ≥1), one tick per completed solve attempt. Single-flight layered (one "center" job per type + `CenteringService`'s gate vs the §58.4/§35 recenters); work body maps `PlateSolverConfigurationException`/equipment faults onto the job runner's failure allow-list (Failed + honest, path-sanitized error_message — never a stranded "running" job). +9 tests (`CenteringEndpointTest`, real body through a live `InMemoryBatchJobService`); DTOs registered in `AraJsonSerializerContext` (incl. the previously-missing `PlateSolveRequestDto`/`PlateSolveResultDto`). **This closes the design-folder buildable tail — everything after is hardware/user/infra-gated.** PREVIOUS: **#841 §35 auto-resume pointing** MERGED @ `57064a490` (3 rounds — r1 lookup-phase OCE escape → CancelledByRelapse; r2 raw `Items` walk → `GetItemsSnapshot()`; r3 approved). Before it: **#840 §57 closers** @ `e6de98331`. 19 PRs this session (#823–#841).
