...
- **Currently working on:** **§48.3 FlatPanelFlats — PR 1 of the native-flats arc** (branch `flat-panel-flats-48`) — the auto-exposure flat set: `FlatPanelFlats` instruction + `IFlatCaptureExecutor` seam (Sequencer), `FlatCaptureService` (Server: panel light via `IFlatDeviceService`, probe-to-ADU via the §59 `IAnalysisFrameSource` with a mean-ADU metric, saved FLATs via the real `IImagingMediator` pipeline, light restored on every exit path), factory prototype + DI. Arc remainder tracked in PORT_TODO (§39.5 generator switch + §48.7 flat_panel block + client catalog entry → SkyFlats + twilight timing).
