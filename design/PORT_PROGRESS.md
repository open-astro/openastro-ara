...
- **Currently working on:** **§44 backup stream, WILMA client half** (branch `feat-44-backup-stream-client`) — `BackupStreamApi` (status/claim/release/queue/ack + frame download, slot-lost as a typed exception) + `BackupStreamController` (claim → poll — `frame.complete`-nudged — → download → sha-256 verify via the new `crypto` dep → temp+rename store under `<root>/<server>/<session>/` → ack; capture-aware pause on `camera.exposure_*`; slot-lost re-claims once then disables with the holder surfaced — the reclaim flag deliberately clears only on a clean poll, a claim alone must not re-arm it or a flapping slot reclaims forever, caught by the hung test) + the Settings → Storage "Real-time frame backup" section with settings/help registry entries (gates green: 126/61 valid). +5 controller tests over a fake client + temp dir. Follow-ups (footer chip, per-frame icons, token-bucket bandwidth cap) tracked in PORT_TODO §44. Prior this arc: **#735** §48 prompt flow (@ `28741b269`), **#734** §44 server half, **#733** §63.3, **#732** §42.2, **#731** §35, **#730** docs reorg.
