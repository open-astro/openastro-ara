...
- **Last merged:** PR **#360** (§59.8 autofocus — **hyperbolic** fit + `FitBest` §59.8 selection) on `master` @ `17:43Z`; the dig into a review-flagged coverage gap proved the pure-R² fallback is near-unreachable on real curves (parabola sits at R²≈0.93 symmetric), so the rule was extracted into a directly-tested `SelectBest`. **PR #361 (Classic AF **trendline** fit) is the in-flight follow-up — completes all three §59.8 curve fits.** Prior: **#359** (parabolic fit, pre-centred solve fixing a real large-focuser-position conditioning bug), **#358** (§2105 `DetectStars`, **9 review rounds**, which CLOSED the §2105 render thread #354–#358). See the **2026-06-11 session (#349–#361)** block under Completed.
...
- **Currently working on:** **§59 autofocus — Classic AF trendline fit** (PR #361, in flight). `FocusCurveFit.FitTrendlines` — split at the min-HFR sample into two arms, weighted-LS line per arm, intersect for best focus; robust on rounded/flat-bottomed curves (reads the straight arms, not vertex curvature). +7 tests; also folds in #360's review nits (removed the hyperbolic squared-list alloc via a y-selector, test null-guards). Sub-PR 3 of §59 — **completes all three Classic AF curve fits** (parabolic #359, hyperbolic #360, trendlines #361). **Remaining §59:** the **live focuser V-curve sweep** + AF-sequence wiring (focuser-gated physical blocker — rig is camera-only, per PORT_TODO), Smart Focus (§59.2-4, likely v0.1.0). Other open threads: **§45 polar-align** (drives the guider daemon's polar-align API), **guider-e-2+** (blocked on the profile-model+wizard extension), **§58 meridian flip**.
