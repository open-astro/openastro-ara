...
- **Last merged:** PR #318 (§38k-22 — completed the §38k instruction-registration surface) on `master`. **§38 real execution engine (PR #319 — `SequencerService`)** is in review: sequences actually run for the first time. (Mediator stubs were retouched in #313: `GetDevice()` `null!` → `throw NotSupportedException` — behavior-neutral for the prototypes.)
- **Currently working on:** **§38 execution engine (PR #319)** — real `SequencerService` drives a deserialized body through NINA's inherited `Sequencer`; Start/Abort/Stop/GetRunState + WS lifecycle + §28 checkpoint are real. OpenAstroAra.Test 504→509; clean under TWAE.
