...
- **Last merged:** PR **#358** (§2105 render PR5 — `DetectStars`/`UpdateAnalysis`; from-scratch `StarDetector` + atomic thread-safe analysis publish) on `master` @ merge `16:40Z`, **9 review rounds** (caught real bugs: oversized-blob memory blowup, two missed cancellation paths, a 40 MB/call alloc, a 32-bit-ARM torn-double cross-thread read). **This CLOSES the §2105 in-memory render thread — all inherited render methods un-stubbed (#354–#358).** Only libraw RAW decode + on-image star annotation remain (Live-View-gated, v0.1.0). See the **2026-06-11 session (#349–#358)** block under Completed.
...
- **Currently working on:** **§59 autofocus — Classic AF curve fitting** (PR #359, in flight). `FocusCurveFit.FitParabolic` (`OpenAstroAra.Core/Model/`) — weighted least-squares parabola over `(focuser position, HFR)` points → best-focus vertex + R² + usable/in-range flags. Pure math, no hardware, +8 synthetic-V-curve tests. **Directly builds on #358's HFR** (the §59.8 feature §2105 unblocked). This is sub-PR 1 of §59; hyperbolic + trendline fits (nonlinear, §59.8 fallback) follow next. Live sweep orchestration + Smart Focus feature vectors defer (need a focuser — physical-blocker per PORT_TODO; user's Alpaca rig is camera-only). Other open threads: **§45 polar-align** (drives the guider daemon's polar-align API), **guider-e-2+** (blocked on the profile-model+wizard extension), **§58 meridian flip**.
