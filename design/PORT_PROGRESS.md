...
- **Currently working on:** **§63 guider/PHD2 — guider-a (real `GuiderService`), scoped + started, re-port deferred to a focused PR.** Discovery: the inherited `PHD2Guider.cs` (the `IGuider` impl that speaks the PHD2 JSON-RPC protocol) was **deleted in #242** (the net10.0 conversion) and is NOT in the tree — only the protocol primitives survive (`PHD2Methods`, `PhdEvents/*`, `PhdAppState`, `Phd2Profile`). Recovered the 1261-line class from `840893eb8^`; the WPF strip is small + done in exploration (drop `System.Windows.Media/Threading` (unused), `IWindowServiceFactory`+`SetupDialog`+`OpenPHD2FileDialog`, `[RelayCommand]`/`CommunityToolkit.Mvvm`, the `ImageSource Image` UI property). **But the recovered file predates three later refactors and won't compile as-is:** (1) §63/12h.4 reshaped guider settings — it uses `PHD2ServerUrl`, current `IGuiderSettings` exposes `PHD2ServerHost`/`PHD2ServerPort` (the TCP-connect path needs real adaptation, not a rename); (2) Phase 0.5p scrubbed `Notification` → route the ~12 `Notification.Show*` calls to `Logger`; (3) nullable-reference enforcement (`<Nullable>enable</Nullable>` + warnings-as-errors) flags ~11 CS8618 fields/events to annotate. **guider-a sub-PR = re-port PHD2Guider headless** (these adaptations) **then `GuiderService`** (REST connect/status, §63.2 state machine, §32.4 cache, RMS from the `GuideEvent` stream → `GuiderStateDto`) replacing `PlaceholderGuiderService`. WIP was reset to keep `master` clean; redo on a fresh `phase/14e-guider-a`.
