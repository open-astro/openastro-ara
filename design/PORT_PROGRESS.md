...
- **Last merged:** PR #343 (§14e capture-path PRa — real `CameraService`) on `master` — REST exposures drive the live Alpaca camera end-to-end into §72 FITS + the §28 catalog (10 review rounds; FK-safe manual session, capture mutex, double[,] bridged drivers, applied-settings header readback, DATE-OBS at exposure start). Follows #321–#342.
- **Currently working on:** **§14e capture-path PRb — `TakeExposure` re-port + camera/imaging mediator unification.** `OpenAstroAra.Sequencer.SequenceItem.Imaging.TakeExposure` is re-ported with the NINA-verbatim JSON surface (`ExposureTime`/`Gain`/`Offset`/`Binning`/`ImageType`/`ExposureCount`) so NINA-imported bodies and the §38.7 starter templates resolve via the §38k-6 remap; execution is ARA-native — it builds a `CaptureSequence` and awaits `IImagingMediator.CaptureImage`, discarding the returned `IExposureData` (the frame is persisted server-side; the WPF-era in-memory image pipeline stays §2105). `CameraService` now also serves `ICameraMediator` (live `GetInfo()` for Validate; `IsFreeToCapture` truthfully reflects the capture gate per the #315 note; raw `Capture`/`Download`/`LiveView` stay `NotSupported`) **and** `IImagingMediator` (`CaptureImage` maps the `CaptureSequence` onto the SAME §14e pipeline — shared `CaptureCoreAsync`, the sequencer WAITS on the in-flight gate instead of failing, genuine cancellation aborts the exposure and propagates, failure throws so the instruction's attempt policy engages; NINA `ImageTypes` map to §28 `FrameType`s, parent-container name becomes the catalog target). `HeadlessImagingMediator` stub added for factory/test prototype construction; `TakeExposure` registered in `WithDefaults`; DI dual-registers `CameraService` for all three interfaces. **A saved sequence with capture nodes now produces real FITS frames in the catalog.** +6 tests incl. a NINA-verbatim `$type` round-trip with property preservation and the starter-template filter blocks asserting resolved `TakeExposure` (713 sim-free green).
