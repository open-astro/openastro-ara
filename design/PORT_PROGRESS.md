...
- **Currently working on:** **client robustness sweep** (branch `client-robustness-sweep`, ROADMAP part-10 appendix ×2, client-only) — (1) Best Frames: `listFromJson` drops rows without a usable `frame_id` (missing/wrong-typed/empty; logged count) instead of degrading to `''`; (2) `LogsApi.tail`: non-array 2xx body throws FormatException (Support tab error state, the listSnapshots pattern), non-object rows dropped; `LogsApi` gained injectable Dio + new stub-adapter test file. 1493 client tests green, analyze clean. PREVIOUS: **#826 blind-solver fallthrough offer side** MERGED @ `75bb27863` (1 round, approved clean); **#825 §39 keyset pagination** @ `2c326ad0d`.
