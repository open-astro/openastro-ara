...
- **Currently working on:** **§64 OSC Live View annotation — the real fix** (branch `osc-live-annotation`, ROADMAP part 5) — `Debayer.SuperPixelStretchedWithLuminance` (one debayer pass → colour RGB + raw 16-bit CFA-weighted luminance on the same half-res grid), `JpegEncoder.EncodeColorAnnotated` (colour twin of the gray annotated encoder, shared downscale-first draw-and-encode tail), `RenderLiveFrame` OSC+annotate → detect on the luminance plane → colour overlay (markers land with no coordinate scaling). Flips the #775 PORT_TODO "OSC annotation" follow-up to done with the REAL fix (not the interim echo/reject). Next: remaining part-5 items (repo-frame annotation seam, live-view detection offload from the capture gate — PORT_TODO §64 follow-ups; OSC RenderBitmapSource display stays Live-View-gated) or the §59 Smart Focus remainder
