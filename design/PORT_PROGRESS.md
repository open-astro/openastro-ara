...
- **Currently working on:** **§35 auto-resume pointing** (branch `s35-autoresume-pointing`, SERVER) — the PORT_TODO tail-sweep item: `ISequencerService.GetActiveTargetCoordinatesAsync` walks the run's live plan for the RUNNING `IDeepSkyObjectContainer` (falls back to the plan's first) and returns `Target.InputCoordinates.Coordinates`; the auto-resume countdown re-centers via a new `Func<ICenteringService?>` resolver BETWEEN unpark and release — best-effort (no solver/target/fault → the old verify-pointing warning), bounded (`RecenterTimeout` 10 min → Failed, resume proceeds honestly), relapse-cancel mid-recenter restores the pause + re-parks (same path as the unpark-window relapse). Notification reports pointing honestly (confirmed / did-not-converge / verify); WS auto_resume payload gains `recentered`. +3 walker tests + 3 safety-engine tests. NEXT (last tail item): §18.I REST centering trigger (`POST /api/v1/platesolve/center`, 202 long-running + progress). PREVIOUS: **#840 §57 closers** MERGED @ `e6de98331` (clean round-1; watchdog abort reason + §57.5 latency line — §57 fully closed except Verify-Position). 18 PRs this session (#823–#840).
