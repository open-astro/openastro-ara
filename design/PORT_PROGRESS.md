...
- **Last merged:** PR **#797** (§42 WILMA fault surfaces slice 1 — live fault chips: `ActiveFaultsAccumulator`/`ActiveFaultsNotifier` fold `equipment.fault`/`fault_action_taken`/`connected` into a standing-fault set (recovery-tracked kinds red until `recovered`/reconnect, advisories amber w/ 10-min TTL) blended worst-of into every chip; history seeding from `GET /api/v1/faults?unresolvedOnly=true` on build + link-up with a timestamp-scoped clear guard + 24 h staleRedGuard; `FaultRow` + `FaultsApi`; 3 review rounds — r2 added the seed, r3 scoped the race guard; merged @ `5d0fe2ef1`). Before it: **#796** (has_more pagination fix @ `69dae8032`), **#795** (§42.5 fault log + REST, **closes §42.5**, @ `707370300`).
- **Currently working on:** **§42 WILMA surfaces, slice 2 — fault feed panel + §42.6 per-session timeline** (branch `wilma-faults-2`) — `FaultFeedNotifier` (cursor-paged newest-first history, WS-debounced live refresh) behind a collapsible `FaultPanel` on the Imaging tab (header = live standing-fault roll-up); `sessionFaultsProvider` family + an amber per-session fault badge/timeline dialog on Image Library cards. Then §42.2 remaining matrix rows → §42.3 generalized hot-reconnect (server resolve-on-reconnect follow-up tracked in PORT_TODO)
