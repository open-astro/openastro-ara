...
- **Currently working on:** **guider-d (§63.3 crash-detection + auto-restart)** — the `openastro-guider` daemon is now **complete + running** (user, 2026-06-11), so the guider-daemon-dependent queue is unblocked. guider-d adds active crash-recovery for the sibling `openastro-phd2` systemd unit: `IGuiderProcessSupervisor` (systemctl seam) + `GuiderRecoveryCoordinator` (backoff decision tree 1→5→15→30→60→120 s) fired from `GuiderService.OnConnectionLost`; Critical notification + Red diagnostic on non-recovery; safe no-op on non-systemd hosts. Active hang-poll + ARA-client auto-reconnect + §42.2 mid-sequence fault flow deferred (PORT_TODO). **Next:** guider-e (§63.4/.5 profile + dark-library push via the daemon's RPCs) → §45 polar-align (drives the daemon's `POLAR_ALIGNMENT_DESIGN.md`) → Live View / §2105 image render (SkiaSharp per the §26 reconsideration) → Phase 15 release.
