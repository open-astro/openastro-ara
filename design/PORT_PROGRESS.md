...
- **Last merged:** PR #341 (§38.7 — disk-shipped starter sequence templates) on `master` — three engine-faithful NINA-schema templates ship with the daemon and seed into the profile on first run; `StarterTemplateTest` validates the exact shipped files in CI. Follows #321–#340.
- **Currently working on:** **§14e — SOFA/NOVAS astrometry natives for non-Windows.** `scripts/build-astrometry-natives.sh` compiles the vendored C sources (`NOVAS31/` per the inherited vcxproj source set minus driver programs; all `SOFA/src` minus `t_sofa_c.c`) into `libnovas31`/`libsofa` shared libraries (`-D"__declspec(x)="` neutralizes the Windows-fork export macro; no vendored-source edits). `AstrometryNativeResolver` (registered idempotently from the `SOFA`/`NOVAS` static ctors — CA2255 forbids ModuleInitializer in libraries) maps the inherited P/Invoke names (`SOFAlib.dll`/`NOVAS31lib.dll`) to those binaries off-Windows, returning Zero for default probing otherwise so Windows behavior + the natives-absent DllNotFound surface are unchanged. `AstrometryNativeTransformTest` proves real J2000→JNOW precession (arcminute-scale shift, round-trip within arcsecond) and self-Ignores without the natives; the analyzer-gate CI job builds + stages them so CI always exercises the real path. **Verified live on macOS-arm64** (both tests pass against locally built dylibs). Cross-epoch slew targets in `TelescopeService` now transform for real wherever the natives ship; `.deb`/Docker packaging of the `.so` files rides with the release pipeline (Phase 15). The 1006 `[Platform("Win")]`-filtered astrometry fixtures can be un-filtered on linux CI as a follow-up.
