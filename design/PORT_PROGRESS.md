...
- **Currently working on:** **§44 puller minors** (branch `backup-stream-puller-minors`) — the three non-blocking #736 round-4 items in `backup_stream_state.dart`: server-switch listener drops claim/bookkeeping and best-effort releases the old slot; dead slot-lost rethrow removed from `_pullVerifyStore`; session-scoped stored-but-unacked memo so a failed ack never re-downloads a verified frame.
