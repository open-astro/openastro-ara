...
- **Last merged:** PR **#364** (§18.I plate-solve — solve-a-frame REST endpoint, `POST /platesolve/frames/{id}/solve`) on `master` @ `21:30Z`, 6 review rounds (caught a real solve-time NRE from a null profile + introduced a `PlateSolverConfigurationException` hierarchy → narrow 422). **The full plate-solve chain is now wired: capture → catalog → solve endpoint → ASTAP.** Prior plate-solve: **#363** (`PlateSolveService` core, image→solution). **ASTAP `astap_cli` BUILT on the dev box** + fork build-fix pushed (`open-astro/ASTAP` branch `fix/fpc-3.2.2-build`). **PR #365 (§28 centering service) is the in-flight follow-up.** Earlier this arc: §59 curve-fits (#359-361), §58 trigger (#362). See the **2026-06-11 session** block under Completed.
...
- **Currently working on:** **§28 centering — `CenteringService`** (PR #365, in flight). Wires the ported `ICenteringSolver` (capture → solve → sync → re-slew loop) behind `CenterOnTarget(Coordinates)`, building the capture sequence + `CenterSolveParameter` from the profile over the 5 live mediators (imaging/telescope/filterwheel/dome/dome-follower — all DI-registered). +3 tests (mocked solver + equipment). **This is the §28 "slew and center" capability + what the §58.4 flip recenter will call.** Mount-gated to live-validate (it syncs/re-slews); orchestration mock-tested. **Plate-solve chain MERGED:** `PlateSolveService` (#363) + solve endpoint (#364) — capture → catalog → solve → ASTAP. **ASTAP `astap_cli` BUILT** on the dev box + fork build-fix pushed (`open-astro/ASTAP` `fix/fpc-3.2.2-build`). Next after centering: §58.4 flip orchestration (executor impl). Other open threads: **§45 polar-align** (guider-daemon RPCs not in its `jsonrpc_api.md`), **guider-e-2+** (profile-model+wizard), hardware-gated live AF sweep / flip + centering validation.
