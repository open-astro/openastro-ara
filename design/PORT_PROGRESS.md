...
- **Last merged:** PR #334 (§14e PR14 — Focuser mediator unification) on `master` — `FocuserService` also serves `IFocuserMediator`; `MoveFocuser*` instructions drive the live device via a cancellation/wall-clock-bounded blocking move + settle-wait + direct read-back. First control-device mediator unification. Follows #321–#333.
- **Currently working on:** **§14e PR15 — Rotator mediator unification.** `RotatorService` now also implements `IRotatorMediator` (new `RotatorService.Mediator.cs` partial; one singleton backs both, replacing `HeadlessRotatorMediator`), reusing the hardened focuser-mediator move path: `GetInfo()` serves the §32.4 cache (never throws post-Dispose); `Move`/`MoveMechanical`/`MoveRelative` drive the live device (sky vs mechanical angle) with a cancellation+wall-clock-bounded blocking move, IsMoving settle-wait, and a direct angle read-back, returning the settled angle; `Sync` writes through; `GetTargetPosition`/`GetTargetMechanicalPosition` normalize to `[0,360)`. So `MoveRotatorMechanical` validates + executes against real hardware. REST surface unchanged. Covered by 7 sim-free mediator unit tests + a live mediator-move added to the `[Category("Integration")]` test. Next mediators: Dome/Telescope; Switch needs an `IWritableSwitch` population (heavier); FilterWheel's SwitchFilter + Dither need `IProfileService`.
