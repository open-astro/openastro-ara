...
- **Last merged:** PR #342 (§14e — SOFA/NOVAS astrometry natives off-Windows) on `master` — `scripts/build-astrometry-natives.sh` + `AstrometryNativeResolver`; cross-epoch transforms are real wherever the natives ship; CI builds + exercises them every run. Follows #321–#341.
- **Currently working on:** **§14e — Camera capture path PRa: real `CameraService` + capture→FITS→catalog pipeline.** Replaces `PlaceholderCameraService` (the tenth real device service, head of the capture path). Template surface: caps (sensor/pixel/gain/bin/exposure ranges + cooler flags) + §32.4-cached runtime (ASCOM `CameraState`→idle/exposing/downloading/error, CCD temp, cooler power/on, exposure progress), prompt `SetCooler`/`AbortExposure`. The substantive part: `StartExposureAsync` pre-allocates the frame id and returns immediately (§60.5 — the `PreviewUrl` 404s until the capture lands) while the background pipeline applies per-setting-guarded gain/bin/subframe, `StartExposure`, polls `ImageReady` (bounded exposure+60s margin, connection-supersede aware), downloads `ImageArray` (column-major `[x,y]` per Alpaca spec → row-major ushort with clamping; color 3-axis rejected in v1), writes a §72 `FitsImage` (UnsignedShort + IMAGETYP/EXPTIME/GAIN/BINNING/DATE-OBS/FILTER headers, atomic `Complete()` per §28.7) into the §29 save dir (`{SaveDirectory}/manual`, daemon-local fallback when unavailable) and registers the row via the **new `IFrameRepository.InsertAsync`** + lazily-created "manual capture" session (`EnsureManualCaptureSessionAsync`, reserve-then-insert under a lock) — at which point the existing §65 preview/thumbnail/§72 download endpoints serve the frame like any other. REST-only; the `ICameraMediator`/`IImagingMediator` unification (TakeExposure instruction re-port + registration, which also activates the §38.7 templates' NINA-verbatim capture nodes) is PRb. Covered by 10 sim-free `CameraServiceTest` cases (validation ordering, ImageArray transpose+clamp, state mapping, frames-dir fallback) + a live end-to-end `CameraConnectIntegrationTest` (OmniSim capture → catalog row + FITS on disk + §65 preview bytes).
