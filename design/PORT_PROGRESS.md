...
- **Last merged:** PR **#361** (§59.8 autofocus — **trendline** fit) on `master` @ `17:58Z` — **completes all three Classic AF curve fits** (parabolic #359, hyperbolic #360, trendlines #361); `FocusCurveFit` weighted LS on #358's HFR, ~29 tests, pure math. **PR #362 (§58 meridian-flip TRIGGER re-port) is the in-flight follow-up.** Prior: **#360** (hyperbolic + `FitBest`; the review dig proved the pure-R² fallback near-unreachable → directly-tested `SelectBest`), **#359** (parabolic, pre-centred solve fixing a real large-focuser-position bug), **#358** (§2105 `DetectStars`, **9 rounds**, CLOSED the §2105 render thread #354–#358). See the **2026-06-11 session (#349–#362)** block under Completed.
...
- **Currently working on:** **§18.I plate-solving — `PlateSolveService` (image-in → solution-out)** (PR #363, in flight). The solver backends + algorithms were ported in §0.5 but never wired into a callable service; `PlateSolveService.SolveImage` now builds the configured solver via `IPlateSolverFactory` (DI-registered) + a `PlateSolveParameter` from the profile, +2 tests (mocked solver). **This unblocks §58.4 flip recenter** (its mandatory recenter step needs plate-solve centering, which had no caller). **ASTAP integration is CLI, not an API** (org fork `open-astro/ASTAP`): `ASTAPSolver` shells out to the `astap_cli` binary at `ASTAPLocation` — no ARA code change; the ops work is building `astap_command_line` for Pi-arm64/macOS + a star DB (PORT_TODO). Next plate-solve slices: a solve REST endpoint, the centering loop. **§58 trigger (#362) merged** — flip *orchestration* (sub-PR 2) waits on §28 centering. Prior in-flight context: §59 curve-fits done (#359-361). Other open threads: **§45 polar-align** (guider-daemon RPCs not yet in its `jsonrpc_api.md`), **guider-e-2+** (profile-model+wizard), and hardware-gated live AF sweep / flip validation.
