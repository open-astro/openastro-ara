...
- **Last merged:** PR **#790** (§42.2/§42.3 slices 2+3 — `FaultPolicyMatrix` pure policy table (camera reconnect_then_pause / mount reconnect_then_abort_park / tracking reenable_then_pause / peripherals quiet-reconnect / guider null / unknown-token→default) + `FaultReactionService` (pause-FIRST for camera/mount, 0/5/15/30/60 s hot-reconnect ladder via `IEquipmentReconnector` + new `GetConnectionStateAsync`, resume-on-recovery, pause / abort+best-effort-park give-up, one episode per device type, `equipment.fault_action_taken` WS) + 4 SafetyPoliciesDto policy fields; merged @ `527f0b535`)
- **Currently working on:** **§42.2/§42.4 slice 4a — state-channel fault detectors: `MountTrackingWatch` (silent tracking drop; command-grace + slew/park guards) + `CoolingDriftMonitor` (5°C/5min wall-clock sustained drift, hysteresis clear)** (branch `state-fault-watch-424a`) — slice 4 of 6 split 4a/4b/4c per Plan (4a state-channel ← here → 4b op/write-channel: 8 mediator StallTimeout/OpError publish sites + `SwitchReadbackWatch` + `SwitchValueTolerancePct` + `switch.value_mismatch` → 4c `sequence.instruction_failed` emission → 5 §42.5 fault log + REST → 6 WILMA)
