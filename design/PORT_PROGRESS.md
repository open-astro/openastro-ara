...
- **Last merged:** PR **#737** (§48 auto-flats, WILMA client half — prompt dialog + Settings → Session → Calibration; clean first-round review) on `master` @ `d0abd34d4`. Prior: **#736** §44 client half; **#735** §48 server prompt flow; **#734** §44 server half; **#733** §63.3; **#732** §42.2; **#731** §35 SafetyReactionService; **#730** docs reorg.
- **Currently working on:** **§35.3 emergency stop** (branch `feat-35-3-emergency-stop`) — daemon: `EmergencyStopService` ladder (abort runs FIRST — nothing may start a fresh exposure behind the stop — then abort exposure → stop guiding → park → flat light off; best-effort rungs, honest `EmergencyStopResultDto`, single-flight double-press gate, `safety.emergency_stop` fires before the rungs + `safety.action_taken` after) behind `POST /api/v1/server/emergency-stop`, registered with the §35 optional-dep/resolver pattern, 6 Moq tests. WILMA: `ServerApi.emergencyStop()` + the persistent red bottom-status-bar button (confirm dialog → honest per-rung snackbar; a failed park says "MOUNT NOT REACHED", never pretends), 3 widget tests.
- **Next (the open queue lives in `PORT_TODO.md`'s top half):** §45 polar alignment remaining phases (daemon spike → `PolarAlignService` state machine replacing the placeholder → §45.9 endpoints + WS events → the WILMA bullseye/arrows screen) once the user's openastro-guider upgrade lands; ARA-side adoption of the openastro-guider#57 events once that upstream PR merges (see `PHD2-GAP.md`); §44 progress surfaces + token-bucket cap; §35.1 weather-threshold triggers; the §37.5 wizard safety/site extras now that the §35 engine enforces them; §59 Smart Focus + AF-trigger wiring; libraw DSLR RAW + Live View star-annotation polish (v0.1.0).
