...
- **Last merged:** PR #324 (§14e PR4 — `SafetyMonitorService` also backs `ISafetyMonitorMediator`) on `master` — one singleton serves both the REST service and the Sequencer mediator, so `WaitUntilSafe` reads the **live** device. Follows #323 (real REST service) + #321/#322 (harness + discovery test). The §38 sequence-engine thread (#313–#320) remains functionally complete.
- **Currently working on:** **§14e PR5 — §32.4 cache for the SafetyMonitor device read.** A background `Timer` reads `IsSafe` every 2s (`RefreshInterval`) while Connected and caches it; `GetAsync` (REST) and `GetInfo` (mediator) now serve the cached value under a single lock with **no per-poll blocking HTTP read** — removing the thread-per-poll accumulation flagged on #323/#324 and simplifying both reads. Cache seeded on connect; refresh guarded against overlap (`Interlocked`) and against faulting the daemon (timer-callback catch-all). This **unblocks replicating the real-service pattern** to the other 11 device types (Camera/Telescope/Focuser/…), which is the next thread.
