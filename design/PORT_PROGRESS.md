...
- **Last merged:** PR **#801** (§42.2 audit items 3+6 — `SwitchReadbackWatch` gains a one-per-episode `Recommand` verdict: the refresh loop re-issues the same value best-effort via a raw device write before the `ValueMismatch` fault fires; `CLISolver` logs non-zero solver exit codes with a solver-specific `DescribeExitCode` mapping (ASTAP 1/2/16/32/33 — crash vs clean no-solution vs missing star DB finally distinguishable); 1 review round ✅; merged @ `08d8f75c2`. **Audit items 3+6 done.**) Before it: **#800** (§42.2 op-failure enforcement, audit item 1 @ `6a8562ccc`), **#799** (§42.5 resolve-on-reconnect), **#798**/#797 (§42 WILMA surfaces), **#796** (has_more fix), **#795** (§42.5 fault log + REST).
- **Currently working on:** **§42.2 audit item 4 — rotator angle-drift watch** (branch `rotator-drift-422`) — `RotatorDriftWatch` (a `SwitchReadbackWatch` analog sharing `ReadbackVerdict`) remembers the last commanded move (target + mechanical/sky domain, recorded at the REST and mediator move sites) and the refresh tick compares the reported angle in that domain (circular distance, ±0.5°, gated off while moving/settling); persistent drift re-issues the exact move once, then faults `ValueMismatch`; sync/reverse/failed-move/disconnect reset the expectation. Then audit items 2+5 (mount abort+park terminal, pause-on-persistent-op-fault — likely one policy slice) → §42.3 generalized hot-reconnect → §42.6 `affected_frames`
