...
- **Last merged:** PR #339 (§14e PR19 — FilterWheel mediator unification + profile filter import) on `master` — `FilterWheelService` also serves `IFilterWheelMediator`; the wheel's filter list imports into `ActiveProfile.FilterWheelSettings.FilterWheelFilters` on connect (NINA semantics) so `SwitchFilter` resolves by name/position. Sixth and final owed mediator unification (FlatDevice/Weather have no consuming instruction). Follows #321–#338.
- **Currently working on:** **§14e PR20 — Profile source-of-truth.** `StoreBackedProfileService` replaces the `HeadlessProfileService` stub at the DI registration point: one live NINA `Profile` (the `ActiveProfile` instructions read at execution time) hydrates from `IProfileStore`/`profile.json` (the WILMA REST surface) at startup and re-hydrates on every settings PUT via a new `IProfileStore.Changed` event (raised outside the store lock by both `FileProfileStore.UpdateAndPersist` and the in-memory store). The unit-tested `ProfileStoreMapper` maps the sections with clean NINA counterparts: site→`AstrometrySettings` (lat/long/elevation), PHD2→`GuiderSettings` (host/port/dither/settle), autofocus→`FocuserSettings` (step/initial-offset/exposure/binning), storage→`ImageFileSettings` (path/pattern/`FileType` with fits-variant fallback), plate-solve numerics (incl. the arcsec→arcmin `Threshold` unit conversion), safety-policy meridian fields (`PauseTimeBeforeMeridian`/`Recenter`). Unmapped sections (notifications, diagnostics mode, stretch defaults, equipment auto-connect, and per-sequence dither/AF-orchestration fields) are documented store-only. Hydration failures degrade to previous values + a Warning (never throw into the PUT). Multi-profile management stays inert (single-profile v0.0.1 per §37); `HeadlessProfileService` kept for factory/test defaults. +10 sim-free tests (lifecycle: hydrate-at-construction / re-hydrate-on-change / dispose-unsubscribes; every section mapping; FileType + clamp helpers).
