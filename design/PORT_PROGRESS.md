...
- **Last merged:** PR #315 (§38k-13…18 equipment-mediator stub layer) on `master`. §38k-19/20 (PR #316) — the last two device-mediator stubs — is in review. (Mediator stubs were retouched in #313: `GetDevice()` `null!` → `throw NotSupportedException` — behavior-neutral for the prototypes.)
- **Currently working on:** **§38k-19/20 (PR #316)** — `HeadlessFlatDeviceMediator` + `HeadlessWeatherDataMediator`, completing the 11/11 device-mediator stub set. OpenAstroAra.Test 480→485; 504 total; clean under TWAE.
- **Next substantive work:** the **profile-and-pipeline-bound instructions** that need more than a device mediator (the device stubs are now all in place). (1) Wire **`IProfileService`** into the headless daemon → unblocks `Dither`, `SwitchFilter`, and the **Connect capstone** (`ConnectAllEquipment`/`ConnectEquipment`/`SwitchProfile` + flip the `internal` `DisconnectAllEquipment`/`DisconnectEquipment` to `public` and register all five). (2) **`SynchronizeDome`** needs `IDomeFollower`. (3) The full **`TakeExposure` capture path** (`IImagingMediator` stub + the image pipeline + exposure-info plumbing for frame type / gain / binning). **Phase 14e Alpaca simulator pinning** per §14.5.1 (v0.4.0 already published by ASCOMInitiative — un-block the SHA-256 download + integration tests). OpenCvSharp4 + libraw wiring per §line-2105 to un-stub the `NotImplementedException`s in `Image/ImageData/`. `IXxxMediator → IXxxService` rename per §8.1 mapping table (cosmetic follow-up). v0.0.1-ara.1 release tag + RPi smoke test (user-blocked).
...
- **§38k-19+ profile/pipeline-bound instructions** — the instructions deferred from §38k-13…18 because they need more than a device mediator: `Dither` + `SwitchFilter` (need `IProfileService` wired into the headless daemon), `SynchronizeDome` (needs `IDomeFollower`), and the full **`TakeExposure` capture path** (`IImagingMediator` stub + the image pipeline + exposure-info plumbing for frame type / gain / binning). Then the **Connect/Disconnect/SwitchProfile capstone**, which depends on the *full* mediator set including `IFlatDeviceMediator` + `IWeatherDataMediator` stubs (not yet built). Per playbook §14.5 line 1281, NSubstitute mocks suffice for unit tests; integration tests use real Alpaca simulators per §14.5.1. (All single-device + telescope-coordinated instruction prototypes + their mediator stubs landed in §38k-9…18.)
