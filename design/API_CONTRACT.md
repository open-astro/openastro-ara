...
**Reasoning:** the observed rate counts leaves, not loop passes (a SmartExposure of 60×120 s is two leaves) and its elapsed time includes pauses, so it was worst on exactly the loop-heavy sequences the daemon figure was built for; a total that counted disabled blocks the remaining figure credited read as progress that never happened; the retained total of a stopped run is the executed portion, since the stop marks everything unrun SKIPPED before the final walk; the estimator ran per capture tick on ARM64.
