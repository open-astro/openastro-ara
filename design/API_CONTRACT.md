...
- A seventh, `wr-stars` (717 Galactic Wolf-Rayet stars, Sheffield catalogue / Rosslowe & Crowther 2015, sky-data @ cc6373e), uses type `WR*`: `/dso-catalog` passes those rows regardless of magnitude so the client's offline search resolves any WR number, and the client ranker skips star types. `/catalogs/wolf-rayet` overlays them.
