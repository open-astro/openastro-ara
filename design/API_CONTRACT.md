...
- A seventh, `wr-stars` (226 Galactic Wolf-Rayet stars, van der Hucht 2001, sky-data @ a72784d), uses type `WR*`: `/dso-catalog` passes those rows regardless of magnitude so the client's offline search resolves any WR number, and the client ranker skips star types. `/catalogs/wolf-rayet` overlays them.
