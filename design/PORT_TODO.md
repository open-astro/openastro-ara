...
- **Precise `frames_completed` + `current_instruction_index`** (deferred in #319). The run-state reports `frames_total` from `SequenceBodyInspector`, but `frames_completed` and `current_instruction_index` are not yet tracked during real execution (the `IProgress<ApplicationStatus>` callback gives a per-current-item *description*, not an overall count or index). Both stay at their defaults (0 / null) today. Hook instruction-completion (root container `RemoveRunningItem`, or per-item status) to populate them; `SequencerService.RunState` already guards these fields under its lock so wiring them is write-only.
