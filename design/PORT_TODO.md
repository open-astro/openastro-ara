...
## Execution-engine TODOs (deferred — no execution engine exists yet)

The §38k work registered every instruction as a JSON-resolvable **prototype**; prototypes are only cloned/registered/validated, never run. These items become real when the sequence **execution engine** lands:
