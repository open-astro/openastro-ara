...
- **§36 Data Manager catalogs — durability + remaining slices.** PR (catalog repoint) points `hyg-stars`/`openngc-dso` at commit-pinned upstream raw URLs (HYG archived GitHub repo; OpenNGC GitHub) — immutable while the repo exists, but a repo deletion would break the download. Eventual robust home: a self-hosted snapshot (a release asset on an open-astro repo, or the org's own host) — needs the org to publish it; revisit before a public release. Also: `horizon-default` still points at the dead `data.openastro.net` `.tar.gz` (out of scope here — it's a horizon profile, not a catalog — but it should be repointed/removed too). Remaining catalog-overlay slices: (1) a server endpoint to serve an installed catalog as JSON rows {ra,dec,mag,name} (parse `{packageDir}/catalog.csv` — **delimiters + columns differ per package**: HYG `hygdata` is comma-separated with `ra`/`dec`/`mag`/`proper` columns, OpenNGC is **semicolon**-separated with different column names, so the parser must be per-catalog, not one-size-fits-all); (2) client `araAddCatalog` Aladin overlay + fetch, gated on installed catalog packages. Flagged while wiring the real-catalog download (PR4a-2); per-package delimiter raised by the #584 review. **Content-integrity (also #584):** downloads have no hash check — commit-pinned URLs make the content immutable while the repo exists, but a CDN/proxy hiccup or a future repoint could serve corrupt/wrong bytes undetected. Add a `Sha256` field to `DataPackageDto` + a post-download digest verify in `RunDownloadAsync` (low priority before first release; pairs naturally with the self-hosted-snapshot work).
