...
**§38 client editor — container depth (follow-ups to the container palette slice).** The editor can now add `Sequential`/`Parallel` containers and nest instructions inside them (built in the daemon's template shape, verbatim round-trip). Remaining to reach full NINA parity on containers: (1) **edit a container's `Name`** in the field panel (today it builds with the default NINA name and shows it in the tree, but there's no rename control — a leaf node never has `Name`, so the field editor needs a container-only Name field); (2) **loop conditions** — a Conditions sub-surface on a selected container to add/edit `LoopCondition` (iterations), `TimeCondition`, `TimeSpanCondition`, `AltitudeCondition`, `AboveHorizonCondition` (the `$type`s + wrapper are already known); (3) **triggers** — a Triggers sub-surface for `MeridianFlipTrigger`/`ReconnectTrigger`. Each is its own slice.
