...
- ⏳ **AOT-safe `JsonStringEnumConverter` per-enum.** The non-generic `JsonStringEnumConverter(JsonNamingPolicy)` constructor still trips IL3050 because the AOT-safe generic `JsonStringEnumConverter<TEnum>` doesn't accept custom naming policies. Currently suppressed with one `#pragma warning disable IL3050` in `Program.cs`. Only matters for full AOT publish (not yet enabled in CI per §14.3 deferred). Fix when §14.3 lands: enumerate 8 enums and register one converter per type with a custom `JsonNamingPolicy` set on each.
