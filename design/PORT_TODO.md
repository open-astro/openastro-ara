...
- **§54 bug-report — client must disclose the bundle contains the profile path (PII) before share.** The server `BugReportService.system-info.json` includes `profile_dir` (deliberate — aids "can't find profile" diagnostics, and the user downloads the bundle themselves). The absolute path can carry the OS username (e.g. `/home/alice/...`). The §54 client UI ("Send me a bug report") should tell the user the bundle contains their filesystem path / profile + logs BEFORE they attach it to a public issue tracker — a one-line disclosure near the download/share affordance. Server-side the field stays; this is a client-UX follow-up. Flagged by the #546 review.
