...
- **§68 gate altitude — CONFIRMED correctly placed at the REST endpoint (2026-06-13 finding).** `ConnectGatedAsync` gates `POST /equipment/{type}/connect`. **Update (2026-06-20): §52.1 daemon-side auto-connect-on-boot is now implemented** (`EquipmentSelectionStore` remembers the last device per type at the connect chokepoint; `EquipmentAutoConnectService` re-establishes each enabled device on boot) and it **routes through the same `ConnectGatedAsync` gate path** (it calls `IAlpacaBridgeHandshakeService.HandshakeAsync` and skips on `OutdatedBlock` / emits the warn event on `OutdatedWarn` before connecting), so a boot-time auto-connect cannot bypass the §68.1 handshake. The REST endpoint remains a connect path; auto-connect is the second. (The earlier note that "nothing acts on the auto-connect setting at startup" is now superseded.)
- **§68 gate logic duplicated on the boot path (minor refactor).** `EquipmentAutoConnectService.TryConnectAsync` mirrors the §68.1 block/warn decision from `EquipmentEndpoints.ConnectGatedAsync` (it can't reuse that overload verbatim because the REST one also persists to the selection store). A cross-reference comment flags it, but if the gate grows a third status the logic must change in two places — worth extracting a shared `AlpacaBridgeGate.Evaluate`-style helper eventually. (claude[bot] PR #552 review.)
