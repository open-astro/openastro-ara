...
- ✅ **§68.2 — dismissible "newer AlpacaBridge recommended" banner (DONE, client §68.2).** `AlpacaBridgeWarningBanner` + `alpacaBridgeWarningProvider` consume the `equipment.alpaca_bridge_outdated_warn` WS event and show a per-version-dismissible advisory banner on the Equipment settings panels. **Remaining:** surface the **503 `alpaca_bridge_outdated`** connect failure as the §68.2 modal ("AlpacaBridge too old — requires 1.2.0+") — blocked on the client gaining an **equipment-connect endpoint wrapper** (the client has discovery but not yet a POST `/equipment/{type}/connect` call; this banner is the first §68 client consumer). When that lands, parse the RFC7807 `code` and show the modal.
