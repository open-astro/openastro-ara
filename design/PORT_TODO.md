...
- **Connect/Disconnect/SwitchProfile capstone** (`ConnectAllEquipment`, etc.) — depend on the *full* 11-mediator device set, which is **now complete** (`HeadlessFlatDeviceMediator` + `HeadlessWeatherDataMediator` landed in §38k-19/20, PR #316). Remaining blockers before these register: (a) `ConnectAllEquipment` / `ConnectEquipment` / `SwitchProfile` need **`IProfileService`** wired into the headless daemon (same blocker as `Dither` / `SwitchFilter`); (b) `DisconnectAllEquipment` / `DisconnectEquipment` are declared **`internal`** in the Sequencer (their sibling `ConnectAllEquipment` is `public`) — flip them to `public` so the factory can construct them as prototypes. Register all five together as the capstone once `IProfileService` lands.
