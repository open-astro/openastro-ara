...
- **§38 editor — add a debug-mode assert for `$ref` entries in `Items.$values`.** `nina_dom.childrenOf` includes `{"$ref":"N"}` maps as children (they're `Map`s, pass `whereType`); `nodeAt`/`_rebuild`/`setField` would then treat a reference handle as a real node and write fields onto it, producing invalid Json.NET. The PR asserts tree-shaped NINA sequences never put `$ref` entries in a container's `$values` (reference handles appear on the referenced node via `$id`, not inline in the tree), which holds for daemon output. A `assert()` in `childrenOf` (debug builds only) would catch the assumption breaking early. Flagged by the #514 review (✅-approved, non-blocking).
