...
- **§38 editor catalog — release-mode message on a non-String-keyed default.** `instruction_catalog._deepCloneJson` keeps a debug `assert(value is Map<String,dynamic>)` for a clear message, then relies on the subsequent `entry.key as String` cast as the release-mode fail-fast (a bare `TypeError`). The whole catalog is `const Map<String,dynamic>` so this can't trigger today; if Phase 2 ever generates `InstructionDef`s with non-const map defaults, consider converting the assert to a release `throw` with a descriptive message. Flagged by the #515 review (low/future-risk, no current bug).
