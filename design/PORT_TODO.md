...
**§38 client editor — container depth (follow-ups to the container palette slice).** The editor can now add `Sequential`/`Parallel` containers, nest instructions inside them (daemon template shape, verbatim round-trip), ✅ **rename a container**, and ✅ **edit its loop conditions** (`LoopCondition` iterations + `TimeSpanCondition` duration — catalog + nina_dom ops + field-panel Conditions UI). Remaining to reach full NINA parity on containers: (1) **object-valued conditions** — `TimeCondition` (needs a `SelectedProvider` object) and the altitude conditions `AltitudeCondition`/`AboveHorizonCondition` (need a nested `WaitLoopData` object: coordinates + comparator + offset); these were deferred from the condition catalog. (2) **triggers** — `MeridianFlipTrigger` catalog landed (the `TriggerDef` model); still to do are the triggers nina_dom ops + state methods (mirror the conditions engine), the field-panel Triggers UI, and **`ReconnectTrigger`** (deferred — needs its `SelectedDevice` device-name set grounded). (3) **`TimeSpanCondition` field clamping** (flagged on #531): the `Hours`/`Minutes`/`Seconds` integer fields use a plain text formatter, so a user can enter negatives or Minutes/Seconds > 59 (the daemon validates, but there's no client clamp). `InstructionField` carries no min/max for the integer-text path yet — add min/max to `InstructionField` (and route integer fields through a clamped `_NumField`) when the duration editor is revisited. (4) **reserved-key guard consistency** (flagged on #532): `TriggerDef.build()` now rejects a field whose key collides with a base key it writes (`Parent`/`TriggerRunner`); apply the same guard to `ConditionDef.build()` (reserved `Parent`) and `InstructionDef.build()` (reserved `Parent`/`ErrorBehavior`/`Attempts`, plus the container keys) in a small consistency pass.
