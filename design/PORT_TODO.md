...
- **§38 editor — remove the superseded display-tree widgets.** The Sequencer tab (#~520) now renders the §38 raw-body editor (`SequenceEditorTree`/`SequencerPalette`/`SequenceFieldEditor` driven by `sequenceEditorProvider`). The old lossy display tree — `SequenceTree`, `InstructionEditor`, and the `SequenceController`/`selectedNodeIdProvider` in `state/sequencer/sequence_state.dart`, plus `parseNinaSequenceBody` if it has no other consumer — is no longer used by the tab. Left in place to keep the integration PR focused; delete it (and its tests) in a follow-up once nothing else references it (grep first — export/preview paths). Flagged during the tab-integration slice.
- **§38 coordinates editor — enforce the Dec ±90° cross-field boundary.** The RA/Dec field editor clamps each component independently (Dec degrees 0–90, minutes/seconds 0–59), but doesn't enforce the cross-field rule that at exactly ±90° the minutes/seconds must be 0 (−90:30:00 is an invalid declination). Low risk (the mount driver clamps, and it's an edit-time UX gap, not data corruption). When touched, special-case degrees==90 → force minutes/seconds to 0. Flagged by the #525 review.
