...
**§38 client editor — container depth (follow-ups to the container palette slice).** The editor can now add `Sequential`/`Parallel` containers, nest instructions inside them (daemon template shape, verbatim round-trip), and ✅ **rename a container** (field-panel Name editor, done in the container-rename slice). Remaining to reach full NINA parity on containers: (1) **loop conditions** — a Conditions sub-surface on a selected container to add/edit `LoopCondition` (iterations), `TimeCondition`, `TimeSpanCondition`, `AltitudeCondition`, `AboveHorizonCondition` (the `$type`s + wrapper are already known); (2) **triggers** — a Triggers sub-surface for `MeridianFlipTrigger`/`ReconnectTrigger`. Each is its own slice.
