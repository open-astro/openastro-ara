...
- **§38/§28.12 sequencer — `completedUtc` not populated from WS.** The live run-state WS slice (#508) folds `sequence.*` frames into the run state, but the WS payload (`SequencerService.EmitAsync`) carries only `sequence_id`/`run_id`/`state`/`current_instruction_index`/`frames_completed`/`frames_total` — not `completed_utc`. So when a `sequence.complete` arrives (esp. one triggered by another client, where the local `refresh()` never runs), the state flips to `complete` but `completedUtc` stays null and the completion time isn't shown. Fix options: add `completed_utc` (and ideally `current_target_name`/`started_utc`) to the server's WS payload, or re-`refresh()` once on a terminal WS event to pull the full DTO. Flagged by the #508 review.
