...
- ⬜ **§58** Meridian flip — sequencer trigger pending · 🟡 **§59** Autofocus — Classic AF curve fitting landed: parabolic (#359) + hyperbolic + `FitBest` §59.8 fallback selection (#360), `FocusCurveFit` weighted LS on #358's HFR; remaining: trendline fits, the live focuser V-curve sweep + AF-sequence wiring (focuser-gated), and Smart Focus (§59.2-4, likely v0.1.0).
