...
4. At a **phase boundary**, tag the PR's head immediately before merging it — `git fetch origin && git tag phase-N-complete origin/<branch> && git push origin phase-N-complete` (sub-phase tags `phase-N-<letter>-complete` where the sub-phase is a coherent milestone — judgment call). The tag goes on last because §19.1 requires it on the work being merged, and any review round that produces a fix commit moves that head.
