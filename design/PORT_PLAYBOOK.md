...
| `changes` | `Changed paths` | `ubuntu-latest` | classify the PR diff into `docs_only` / `dotnet` / `client` outputs (fails open: an error runs everything, and both steps are `continue-on-error` so a checkout flake or timeout leaves the job green with an annotation — #1024; job-level `continue-on-error` was measured on #1043 to still publish a red check) | never skipped |
