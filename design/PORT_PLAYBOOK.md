...
1. **Server — discovery & choices.** Wrap `get_equipment_choices` + `discover_alpaca_servers` (RPC classes exist unwrapped in `PHD2Methods.cs`); add `GET /api/v1/equipment/guider/choices` and `POST /api/v1/equipment/guider/discover`. Choices DTO carries camera/mount/aux-mount/rotator lists as reported by the daemon. FakeGuider coverage + openapi.yaml.
...
6. **Server+client — manual restart + polish.** `POST /api/v1/equipment/guider/restart` surfacing the existing §63.3 supervisor (idempotent per §60.5); replace the "guider is not configurable here" copy in `EquipmentEndpoints.cs`; USER_GUIDE/DEPLOY wording; reconcile the §63.12 table against shipped reality.
