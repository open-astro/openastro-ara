...
- ✅ **§43** Backup + restore — real `BackupService` (zip snapshots, §43-2 validated restore with atomic swap, snapshot download). · 🟡 **§44** Real-time backup stream — SERVER half real (2026-07-07: `BackupStreamService` — single-target slot, §44.5 claim/queue/ack surface, §44.6 frames columns + lazy sha256); the WILMA client half (toggle/puller/progress UI) is the remaining slice.
