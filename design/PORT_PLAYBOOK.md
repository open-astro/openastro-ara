...
- 🟡 **§58** Meridian flip — `MeridianFlipTrigger` decision logic re-ported headless (#362) behind the `IMeridianFlipExecutor` seam; remaining: the §58.4 flip orchestration (executor impl) + side-of-pier projection test matrix. · 🟡 **§59** Autofocus — **all three Classic AF curve fits landed** (parabolic #359, hyperbolic + `FitBest` §59.8 selection #360, trendlines #361), `FocusCurveFit` weighted LS on #358's HFR; remaining: the live focuser V-curve sweep + AF-sequence wiring (focuser-gated physical blocker), and Smart Focus (§59.2-4, likely v0.1.0).
