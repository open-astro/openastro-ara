...
4. At a **phase boundary**, tag the PR's head immediately before merging it — `git fetch origin && git tag phase-N-complete origin/<branch> && git push origin phase-N-complete` (sub-phase tags `phase-N-<letter>-complete` where the sub-phase is a coherent milestone — judgment call).
5. **Merge to `master`** with `gh pr merge --delete-branch` (squash for multi-commit PRs that should land as one logical change; merge commit where per-commit granularity matters — §19.1). **A PR carrying a phase or sub-phase tag must use `--merge`, not `--squash`:** a squash replaces the tagged commit with a new one and `--delete-branch` removes the branch, leaving the tag unreachable from `master` and invisible to `git describe master`. Pull `master` and continue.
...
**Reconciliation (2026-07-28, §63.17 complete):** everything below shipped under the `/api/v1/equipment/guider/*` namespace (not the planned `/api/v1/guider/*`): status/connect/start/stop/dither + dark-library/defect-map build/status/enabled (earlier arcs), and via §63.17: `GET /choices`, `POST /discover` (60 s combined-sweep cap), `POST /profile/push` (+ `guider.profile_pushed`), `DELETE /darklibrary` (+ `guider.dark_library.invalidated` on guide-camera change), `POST /restart`. Planned events `guider.lifecycle`/`guider.crashed`/`guider.restart_failed` were superseded by `guider.state` + `guider.process.failed`; `guider.event_passthrough` remains unbuilt (no consumer).
...
**Goal:** all guider configuration happens in Ara Client — equipment selection (guide camera, mount/aux-mount, rotator), Alpaca server binding, optics, and dark-library lifecycle — so the daemon's web portal is never required. Closes the unshipped rows of the §63.12 table.

Shipped surface note: the §63.12 table planned `/api/v1/guider/*`; what shipped is `/api/v1/equipment/guider/*`. §63.17 continues under the shipped namespace.
...
5. **Client — dark-library management UI.** Delete (with confirm) in the calibration dialog; "darks invalidated — rebuild" banner off the new WS event; library state surfaced in settings.
6. **Server+client — manual restart + polish.** `POST /api/v1/equipment/guider/restart` surfacing the existing §63.3 supervisor (idempotent per §60.5); update the "Guider is not Alpaca-discoverable" guidance copy in `EquipmentEndpoints.cs` (its detail text currently points users at host:port settings only); USER_GUIDE/DEPLOY wording; reconcile the §63.12 table against shipped reality.

Out of scope: §45 polar-align screens, structured EquipmentDisconnected routing (#58–61 follow-ups), multi-guider (§63.16).
