...
## 2026-07-07 — §44 backup stream server semantics

- **sha256 is computed lazily on first queue serve and cached**, not at capture time as §44.6 sketched. Same integrity property on the wire, zero capture-path risk, and it auto-backfills every frame captured before the feature existed (capture-time hashing would have left the whole historical catalog hashless). An unreadable file serves a null sha (entry listed, WILMA skips it) rather than failing the page.
- **The Phase-8 subscribe/claim-per-frame sketch was replaced by the §44.5 spec surface** (status/claim/release/queue/ack under `/api/v1/server/backup-stream/*`) — the wire had no consumers (WILMA has no backup-stream client yet), so the breaking change landed inside the no-consumer window like the `frames_*` rename precedent.
- **Slot semantics:** hostname-keyed; the same hostname re-claims idempotently (crash recovery per §44.9); a different hostname takes over only after the holder has been silent (no queue/ack traffic) past 10 minutes. A takeover re-queues frames synced to the old target — each desktop mirrors the full catalog.
- **Unverified acks are refused** — an ack with `sha256_verified: false` is a client bug; the frame stays queued rather than being marked synced on a failed integrity check.

## 2026-07-07 — §48 auto-flats prompt semantics

- **§39.5 generation IS the v0.0.1 flat automation.** NINA's FlatPanelFlats/SkyFlats instructions were never ported, so the prompt's "yes" choices route into `GenerateMatchingFlatsAsync` over the completed run's own catalog session — the ARA-native superpower the §48.1 dialog itself advertises. `panel_at_end` auto-starts the generated sequence; `sky_at_twilight` generates + notifies honestly (no twilight scheduler yet — auto-starting sky flats at completion time would shoot flats into a dark sky). The native instructions + §48.7 tuning blocks land together in v0.1.0 (enforcement-first).
