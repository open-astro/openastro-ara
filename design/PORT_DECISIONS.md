...
- **Encoded in:** `client/.../widgets/sky_atlas/aladin_view.dart` (the embed), `pubspec.yaml` (`webview_cef: ^0.2.2`). Offline-bundling the Aladin Lite JS (per §36.1, ~5 MB) + the universal-search "goto" + Tonight's-Sky projection wiring are follow-up §36 slices.
