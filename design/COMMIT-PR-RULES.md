...
2. **`master` is now a protected branch** (set up 2026-06-09). Required status checks (per the live `master protection` ruleset, re-read 2026-09-19 via `gh api repos/open-astro/openastro-ara/rules/branches/master` while landing the finer CI path gate): `Sanity (design docs)`, `Server (build + cross-publish + Docker)`, `Settings + Help registry gate`, and the three `Client (analyze + test)` OSes. **`Analyzer gate` and `review` are NOT required contexts** — an earlier revision of this line said they were (#1023; it also abbreviated the server context to `Server build` — context names are matched literally, so the names above are copied from the ruleset verbatim). **Decided 2026-09-19 (#1023): `Analyzer gate` stays non-required on purpose.** The reason is blast radius, not mechanics (job-level gating of a non-matrix required context works fine — `server-build` and `registry-gate` are gated that way): `analyzer-gate` is skipped on the *finer* `dotnet` classification, and playbook §19.1 keeps required contexts off that bucket on purpose, because a classifier bug there would mark a required context satisfied with nothing red. Promoting it would either widen that blast radius or force it back onto the coarse `docs_only` gate; it already fails the check suite on every code PR either way. That is what makes it safe for the path gate to skip `analyzer-gate` (and both Alpaca jobs, and `client-build`) on the finer `dotnet`/`client` classification: a skipped required context is counted by GitHub as satisfied, so only NON-required jobs are gated on anything other than `docs_only`. The `review` gate is enforced by the §19.1 merge rule below, not by the ruleset; strict/up-to-date required; PR-before-merge; force-push + deletion blocked; `enforce_admins=false` (solo-admin emergency override retained). This makes the §19.1 merge-gate **mechanical**, not just policy — a PR literally cannot merge until those checks are green — **or `skipping` because CI's path gate skipped them** (`ci.yml`'s `changes` job; a docs-only PR skips the build/test jobs per #1020, and playbook §19.1 accepts an attributable skip as clearance). Exception: the three `Client (analyze + test)` legs are matrix-expanded, so they are gated at *step* level and report **`pass`** with their steps skipped, never `skipping` — a matrix job skipped at job level never expands and its required contexts go unreported, blocking the PR forever (#1025). **Reminder:** a green `review` *check* is necessary but NOT sufficient — the gate requires the review *comment body* to carry **no unaddressed findings** (learned on #314, where a green check still had a stale-doc finding to fix). Read the review, don't just watch the status dot. **Before adding a context to this ruleset:** `codeql.yml` skips prose-only PRs with a workflow-level `paths-ignore` (#1022), which is only safe because `Analyze (csharp)` is not required — a filtered-out workflow never creates its check, so requiring it would leave every docs-only PR on "Expected — waiting for status" forever. Making CodeQL required means first moving it to a job-level gate like `ci.yml`'s (#1020). No test can see repo settings, so this line is the guard.
...
4. Opens PR **targeting `master`**
5. Review poll-and-fix loop runs (AI watches for the review comment; auto-fix trivial + correctness findings via new commits; reasoned replies for disagreements; out-of-scope items → `design/PORT_TODO.md`). If no review has appeared, the workflow is still running — AI waits and re-polls. There is no self-review fallback: `/review` does not satisfy this gate.
6. At a **phase boundary**, tags `phase-N-complete` (sub-phase tags `phase-N-<letter>-complete` where the sub-phase is a coherent milestone) on the PR's head **immediately before that PR merges** — a tagged PR merges with `--merge` (not `--squash`) so the tagged commit stays reachable from `master`. See playbook §22.1 step 4
7. **AI merges** once the §19.1 merge-gate clears (green CI + **review pass posted** — a `claude[bot]` review comment for the current head, with no unaddressed Defects; + ≥3 min quiescence + clean self-review against scope). Use `gh pr merge --delete-branch` — same-repo heads only; a fork head belongs to the contributor and is never deleted from here, so omit `--delete-branch` when `isCrossRepository` is true (squash for multi-commit PRs that should land as one logical change; merge commit where per-commit granularity matters — §19.1; **a PR tagged at step 6 always merges with `--merge`**, never squash, or the tag is orphaned) to remove the branch from origin in the same step. If any gate condition is ambiguous, AI posts `Held for human review @<user> — <reason>` and waits instead.
8. AI pulls updated `master` and starts the next sub-PR from there. **Whether the just-merged PR was the last in a phase** is determined by the phase sub-PR list in PORT_PLAYBOOK.md §3 plus the per-phase sub-split table in this document (e.g., `phase-12h` is the last under Phase 12's row); that determination is made at step 6, *before* the merge, since the tag goes on the PR's head. There is no separate promotion step — the merge to `master` *is* the integration.
...
| **12c** Imaging tab | Main image viewer, exposure controls, histogram, plate-solve overlay, §51 Health Indicator (always visible) + Diagnostic Panel, §64 Live View / loop imaging UI, §45 polar alignment iPolar-style continuous loop UI. (Framing/§47 mosaic moved to the Planning tab — see 12e + the reconciliation note below) | ~120 (was ~150; Framing/mosaic moved to 12e) |
...
3. **The reviewer re-runs automatically on each fix push** (`synchronize`). AI never invokes or @-mentions it, and never handles round-2 findings on its own authority — it waits for the new verdict on the current head before re-testing the gate.
...
- ✅ §19.1 git safety — branch allowlist now permits `port/ara` (integration) + flat-named `phase-N[<letter>]` sub-PR feature branches (e.g., `phase-0.5a`, `phase-12h`) plus a small set of named prep branches (e.g., `prep-ci`). **Merge policy revised 2026-05-23** in `prep-ci` PR #2: original "AI never merges, ever" rule replaced with "AI merges under a strict merge-gate" (green CI + reviewer quiescent ≥3 min + no unresolved findings + clean self-review). User retains override. Tag scheme extended to `phase-N-<letter>-complete` for sub-PRs. **Superseded (recorded 2026-09-19, #1032):** `port/ara` and the flat `phase-N<letter>` names were retired 2026-06-02; the live allowlist is `phase/<N>[-<letter>]-<short-name>`, the named prep branches (`prep-*`), `rules-*`, and `chore/<short-name>` — playbook §19.1 is authoritative.
