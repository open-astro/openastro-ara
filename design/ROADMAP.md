...
| ~~**Wizard 2.0 (§76)**~~ | SHIPPED 2026-08-03 (PRs #901–#907) | The five-screen setup wizard (equipment readiness, guiding decisions, finish provisions the guider + kicks darks) replaced the §37 18-screen flow on master. |
| **Planetary / lunar lucky-imaging (§77)** | playbook §77; INCUBATING on `feature/planetary` (2026-08-03) | Engine hardware-proven (sensor-limited SER capture, honest accounting, drop-free at planetary ROIs on a USB3 SSD; full-frame bus-limited). Feature-level work (live preview UX, on-sky pointing, RGB clips) continues on the branch, synced with master; mainline re-entry gated on the DSO experience being polished. Formerly IN PROGRESS: native-SDK SER capture in the daemon (P/Invoke of the AlpacaBridge-deb `.so`s — the one §52 exception), 100–500 fps ROI video, ring buffer + O_DIRECT SER writer, ephemeris-goto pointing, Live-tab planetary mode, sequencer clip plans. Slices P1–P6 in §77.5; P1 under way. |
...
| **General command palette** | §61.10 | Expand the ⌘K settings search into a full command palette — explicitly committed expansion. Slices 1+2 SHIPPED: tab navigation hits + 5 action commands (Open Image Library / Calibration / Stats, Back up & restore, Run profile wizard — the top-bar launchers as searchable commands). Remaining: equipment ops (park etc.), sequence templates, sky-atlas targets. |
...
- **§31** time + location sync — the "verification" found the waterfall was never built (only
  ...
  when the daemon reports a fresh, trustworthy sync). USB-GPS self-sync SHIPPED (step 2):
