...
public sealed partial class UnattendedShutdownService : IHostedService, IDisposable {
    ...
    /// must not stop the remaining equipment from being put to bed.
    /// </summary>
    [SuppressMessage("Design", "CA1031:Do not catch general exception types",
        Justification = "Best-effort by design: a fault in one step must not stop the rest of the shutdown/countdown.")]
    internal async Task ExecuteShutdownAsync(Guid sequenceId) {
        ...
        await DisconnectQuietly("filter wheel", _filterWheel is null ? null : ct => _filterWheel.DisconnectAsync(null, ct), summary).ConfigureAwait(false);
        await DisconnectQuietly("focuser", _focuser is null ? null : ct => _focuser.DisconnectAsync(null, ct), summary).ConfigureAwait(false);
        await DisconnectQuietly("rotator", _rotator is null ? null : ct => _rotator.DisconnectAsync(null, ct), summary).ConfigureAwait(false);
        await DisconnectQuietly("flat panel", _flatDevice is null ? null : ct => _flatDevice.DisconnectAsync(null, ct), summary).ConfigureAwait(false);

        // 4 — warm the cooler, then 5 — disconnect the camera.
        try {
            if (_camera is not null) {
                await WarmCoolerAsync(summary).ConfigureAwait(false);
        ...
        // during darkness — still below the alarm threshold).
        try {
            if (_notifications is not null) {
                await _notifications.CreateAsync(new NotificationDto(
                    ...
                    Title: "Unattended shutdown executed",
                    Message: "The sequence paused awaiting your attention and nobody responded within the "
                        + "configured window, so the equipment was put to bed: " + summaryText
    ...
    [SuppressMessage("Design", "CA1031:Do not catch general exception types",
        Justification = "Best-effort by design: a fault in one step must not stop the rest of the shutdown/countdown.")]
    private async Task WarmCoolerAsync(System.Text.StringBuilder summary) {
        if (_camera is null) return;
        var dto = await _camera.GetAsync(CancellationToken.None).ConfigureAwait(false);
        var runtime = dto?.Runtime;
        if (runtime is not { CoolerOn: true }) return; // nothing to warm
        ...
        while (setpoint < WarmTargetC && DateTimeOffset.UtcNow - started < WarmHardCap) {
            ...
            await Task.Delay(stepSleep, CancellationToken.None).ConfigureAwait(false);
        }
        await _camera.SetCoolerAsync(false, null, CancellationToken.None).ConfigureAwait(false);
