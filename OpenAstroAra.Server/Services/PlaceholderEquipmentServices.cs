...
public sealed class PlaceholderTelescopeService : ITelescopeService {
    ...
    public Task SetGuideRatesAsync(double rightAscensionDegreesPerSecond,
        double declinationDegreesPerSecond, CancellationToken ct) =>
        Task.FromException(new InvalidOperationException("Telescope service is not implemented."));
...
public sealed class PlaceholderGuiderService : IGuiderService {
    ...
    public Task<OperationAcceptedDto> RestartGuiderAsync(string? idempotencyKey, CancellationToken ct) =>
        Task.FromResult(PlaceholderEquipmentHelpers.Accepted("guider.restart", idempotencyKey));
