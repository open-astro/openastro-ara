...
/// <summary>
/// §38k-17 — headless no-op stub for <see cref="ISwitchMediator"/>.
/// Same pattern as the §38k-9 … §38k-13 stubs. Backs the
/// <c>SetSwitchValue</c> instruction prototype.
/// </summary>
public sealed class HeadlessSwitchMediator : ISwitchMediator {

    private static readonly SwitchInfo DisconnectedInfo = new() {
        Connected = false,
    };
    ...
    public SwitchInfo GetInfo() => DisconnectedInfo;
    ...
    public IDevice GetDevice() =>
        throw new NotSupportedException(
            "Headless switch stub has no backing IDevice; real Alpaca-backed wiring swaps in at the DI registration point once §14e Alpaca simulator pinning lands.");

    public event Func<object, EventArgs, Task>? Connected;
    public event Func<object, EventArgs, Task>? Disconnected;

    // ISwitchMediator addition — setting a switch value is a no-op with no
    // driver attached.
